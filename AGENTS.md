...
3. **`docs/state/2026-05-22-phase-21-m1.md`** — most recent close-out snapshot (Phase 21 M1 shipped 2026-05-22 — **binary distribution: `sov` ships as installable compiled binaries for `darwin-arm64` / `darwin-x64` / `linux-x64` via public `github.com/yevgetman/sov-releases` with `curl | bash` installer.** Two releases shipped today: `v0.2.0` (initial) + `v0.2.1` (splash/MCP version fix); `v0.2.1` is current latest. `~/.sov/bin/sov` + `~/.sov/bin/sov-tui` + `~/.sov/bundle-default/` layout. Runtime changes: `shippedBundlePath()` + `findTuiBinary()` learn `process.execPath`-relative discovery; `sov upgrade` auto-detects binary mode and re-runs the public installer; `src/version.ts` switched to build-time JSON import (the runtime `readFileSync(package.json)` broke in `bun --compile` mode); harness version threaded into splash via new `--harness-version` flag. ADRs P21-A + P21-B. Suite **1972/0/14** TS green; +14 from baseline; Go all packages green; lint+typecheck clean. macOS smoke PASS; **end-to-end `sov upgrade` 0.2.0→0.2.1 PASS** (validates the upgrade pipeline against a real release); Linux-binary structural check PASS; Linux container smoke deferred (Docker daemon issue on dev host); darwin-x64 deferred to first Intel-Mac beta user.) Predecessor: `docs/state/2026-05-21-ux-fixes-r5.md` (ux-fixes rounds 3-5 close-out). Read this BEFORE the build plan. Replaced each session — find the latest via `ls docs/state/*.md | sort -r | head -1`. The M11 audit report at `docs/state/2026-05-17-m11-parity-audit.md` and the M10 audit at `docs/state/2026-05-16-tui-parity-audit.md` both remain canonical.
