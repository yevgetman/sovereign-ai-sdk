...
3. **`docs/state/2026-05-22-phase-17-cron.md`** — most recent close-out snapshot (**Phase 17 — cron / scheduled jobs shipped 2026-05-22 evening; Phase 17 closed**). New `src/cron/` subsystem (types / schedule parser / atomic jobs CRUD / `CronRunner` with file-lock + fake-clock-injectable tick / executor factory / production glue) + `sov cron add | list | show | pause | resume | delete | run | tick` CLI surface + 60-second tick loop embedded in `buildRuntime` lifecycle (`.unref?.()` so tests don't hang; `cronEnabled: false` opts out). Each job runs in a **fresh session** via `AgentRunner` with auto-deny on permission asks (matches `sov drive` headless mode). Skill chaining via `expandSkillPrompt` (user-message injection, not system prompt); pre-agent scripts via `spawnSync` with interpreter inference + 120s default timeout + 16 KiB stdout cap. Local delivery to `<harnessHome>/cron/outbox/<jobId>/<ts>.txt`; `[SILENT]` first-line prefix (case-insensitive, post-trim) short-circuits the file write. Recursion guard: six cron CRUD tool names blocked in `SUBAGENT_EXCLUDED_TOOLS`. No ADRs (purely additive; design decisions captured inline in the plan at `docs/plans/2026-05-22-phase-17-cron-scheduled-jobs.md`). TS suite **2053/0/14** (+57 from morning's 1996 baseline). T11 (cut v0.3.0 release) follows. Predecessor: `docs/state/2026-05-22-semantic-suite-revival.md` (semantic test suite revived via new `sov drive` subcommand, 2026-05-22 late PM). Predecessor of that: `docs/state/2026-05-22-tui-tool-call-abstraction.md` (TUI tool-call abstraction + Fix A + Fix B, 2026-05-22 PM). Predecessor: `docs/state/2026-05-22-phase-21-m1.md` (Phase 21 M1 — binary distribution shipped 2026-05-22 AM). Read THIS file BEFORE the build plan. Replaced each session — find the latest via `ls docs/state/*.md | sort -r | head -1`. The M11 audit report at `docs/state/2026-05-17-m11-parity-audit.md` and the M10 audit at `docs/state/2026-05-16-tui-parity-audit.md` both remain canonical.
...
| File | What's in it |
|---|---|
| [`docs/backlog/post-phase-13-4.md`](docs/backlog/post-phase-13-4.md) | Open backlog (2 items): **#17 eval-gated auto-promote (P4, conditional)**; **#48 Phase 21 M2 release automation (P3, scheduled separately — GitHub Actions + optional Apple Developer code-signing)**. #47 closed 2026-05-22 evening (dead `transcript.go` removed). #40 closed M10.5; #41 + #43 + #44 + #45 + #46 closed 2026-05-19; #29 / #38 / #39 closed 2026-05-19. **Phase 16.1 closed with M13 (2026-05-20); Phase 21 M1 closed 2026-05-22.** |
...
| [`docs/plans/`](docs/plans/) | Implementation plans (executed; left as record). Latest: `2026-05-22-phase-17-cron-scheduled-jobs.md` (Phase 17 — cron / scheduled jobs; shipped + closed 2026-05-22 evening). |
