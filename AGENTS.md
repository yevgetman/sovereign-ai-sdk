...
3. **`docs/state/2026-05-22-tui-tool-call-abstraction.md`** — most recent close-out snapshot (TUI tool-call abstraction + two surgical UX fixes shipped 2026-05-22 PM). **Default tool rendering is now a single-line per `tool_result` (`Verb target stats ›`), matching the Claude mobile app aesthetic.** `ui.toolOutput.mode = 'detailed'` opts back into the bordered ToolCard (output capped to `inlineLines`, default 10). `-v / --verbose` is now wired and forwards as `--verbose-raw` (orthogonal raw escape hatch). Verb mapping owned in new `packages/tui/internal/components/compactline.go`. Status glyphs: `⚠` for permission denied, `✗` for runtime errors. Plus Fix A (silence `tui server listening` stderr) + Fix B (switch dark chroma style to `monokai`). Spec at `docs/specs/2026-05-22-tui-tool-call-abstraction-design.md`; no new ADRs (single-user UX polish). Suite **1982/0/14** TS green (+10 from Phase 21 M1's 1972 baseline); Go all packages green (+29 new tests); lint+typecheck clean. Predecessor: `docs/state/2026-05-22-phase-21-m1.md` (Phase 21 M1 — binary distribution shipped 2026-05-22 AM; `~/.sov/bin/sov` v0.2.1 latest; ADRs P21-A + P21-B). Read THIS file BEFORE the build plan. Replaced each session — find the latest via `ls docs/state/*.md | sort -r | head -1`. The M11 audit report at `docs/state/2026-05-17-m11-parity-audit.md` and the M10 audit at `docs/state/2026-05-16-tui-parity-audit.md` both remain canonical.
