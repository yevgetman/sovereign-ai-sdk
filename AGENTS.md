...
3. **`docs/state/2026-05-20-m13.md`** — most recent close-out snapshot (Phase 16.1 M13 shipped 2026-05-20 — **terminalRepl removal complete; Phase 16.1 closed.** Deleted `src/ui/terminalRepl.ts` (2334 LoC) + 9 REPL-only `src/ui/*` modules + their tests + `src/cli/replDeprecation.ts` + `src/cli/surfaceResolver.ts` + readline asker bits from `src/permissions/prompt.ts`. Dropped `--ui` flag, `SOV_UI` env, `ui.surface` config field. main.ts boot flow collapsed from ~65 lines to ~13. Missing binary = hard error (ADR M13-01); schema rejects `ui.surface` (ADR M13-02); CLI/env handlers gone (ADR M13-03); surfaceResolver deleted (ADR M13-04). **4 ADRs M13-01..04**; suite **1949/1949** green (−124 active tests matching deleted test files); 4/4 boot-decision smoke pass; independent 4-Opus parity audit clean.) Predecessors: `docs/state/2026-05-19-m12.md` (M12 REPL deprecation warning), `docs/state/2026-05-19-m11-5.md` (M11.5 inline picker card), `docs/state/2026-05-17-m11.md` (M11 default-flip; preserved). Read this BEFORE the build plan. Replaced each session — find the latest via `ls docs/state/*.md | sort -r | head -1`. The M11 audit report at `docs/state/2026-05-17-m11-parity-audit.md` and the M10 audit at `docs/state/2026-05-16-tui-parity-audit.md` both remain canonical.
