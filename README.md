...
**Semantic test suite (2026-05-03)** — opt-in LLM-judged behavior tests under `tests/semantic/`. Each test spawns the real `sov` binary in an isolated sandbox, captures the transcript, and asks an LLM judge whether per-test must-satisfy / should-not criteria were met. Default judge is the local `claude` CLI (subscription, no API tokens); `--judge anthropic-api` falls back to the SDK. Pluggable backend design — adding a new backend (codex, `sov`-itself, OpenAI judge) is one new file. Strictly additive: zero edits to `src/`, never imports from `src/`, file names don't match `*.test.ts` so `bun test` is unaffected. Run with `bun run test:semantic`. Coverage (20/20 pass): Bash/Read/Edit/Write happy paths and error reporting, `/help` dispatch, write-then-verify workflow, directory enumeration, anti-fabrication on missing files and zero search results, prompt-injection resistance, verify-not-trust-user-claims, four permission cases (deny rule blocks, allow rule permits, deny-wins-within-layer, bypass-mode-honors-deny), Glob/Grep search-tool dispatch, and `@file` reference expansion. See [`tests/semantic/README.md`](tests/semantic/README.md).
