...
**Semantic test suite (43/43 pass).** Opt-in LLM-judged behavior tests. Default judge is the local `claude` CLI (subscription, no API tokens). Coverage spans 9 tool-dispatch cases (including the Phase 12.5 envelope-recovery case), 6 slash-command pipeline paths (including `/context-budget` and the skill-args propagation regression guard), 6 permission cases (including `mcp__server` server-prefix denial), 4 refusal cases, 2 context-expansion cases, 2 MCP cases, 2 hook cases, 1 self-doc/harness-info case, 1 router case, 1 secret-redaction case (defense-in-depth on Write/Edit/NotebookEdit), 1 `/security-audit` skill case, 2 sub-agents cases (Phase 13 — registry discoverability + live end-to-end delegation), and 6 workflow/multi-turn cases (including end-to-end `/compact` and `/rollback`). Run with `bun run test:semantic`. See [`docs/semantic-testing.md`](docs/semantic-testing.md) for the full inventory and run policy.
...
**Next high-leverage targets** per the build plan: **Phase 13.2** (task system for parallel workers — long-running background sub-agents with `task_create` / `task_list` / `task_get` / `task_stop` / `task_output` over the Phase 13 scheduler), **Phase 13.3** (background review daemon — propose-then-promote memory/skill changes from trajectories), **Phase 13.4** (continuous-learning observation stream + instinct corpus, derived from ECC). See `~/code/sovereign-ai-docs/harness/docs/runtime/harness-build-plan.md` for the full plan.
