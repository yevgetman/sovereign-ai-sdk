...
**Semantic test suite (54/54 pass).** Opt-in LLM-judged behavior tests. Default judge is the local `claude` CLI (subscription, no API tokens). Coverage spans 10 tool-dispatch cases (including pool-separation guard for `memory_propose`/`skill_propose`), 6 slash-command pipeline paths, 6 permission cases, 4 refusal cases, 2 context-expansion cases, 2 MCP cases, 2 hook cases, 1 self-doc/harness-info case, 1 router case, 1 secret-redaction case, 1 `/security-audit` skill case, 2 sub-agents cases, 4 task-system cases, 6 review-system cases (`/review` list/show/consolidate/activity/unknown-verb/bare-call), and 6 workflow/multi-turn cases. Run with `bun run test:semantic`. See [`docs/semantic-testing.md`](docs/semantic-testing.md) for the full inventory and run policy.
...
**Next high-leverage targets** per the build plan: **Phase 13.4** (continuous-learning observation stream + instinct corpus, derived from ECC). See `~/code/sovereign-ai-docs/harness/docs/runtime/harness-build-plan.md` for the full plan.
