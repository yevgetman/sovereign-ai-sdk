...
**Semantic test suite (2026-05-03)** — opt-in LLM-judged behavior tests under `tests/semantic/`. Each test spawns the real `sov` binary in an isolated sandbox, captures the transcript, and asks an LLM judge whether per-test must-satisfy / should-not criteria were met. Default judge is the local `claude` CLI (subscription, no API tokens); `--judge anthropic-api` falls back to the SDK. Pluggable backend design — adding a new backend (codex, `sov`-itself, OpenAI judge) is one new file. Supports both single-turn and multi-turn cases (`prompt: string | string[]`). Strictly additive: zero edits to `src/`, never imports from `src/`, file names don't match `*.test.ts` so `bun test` is unaffected. Run with `bun run test:semantic`. Coverage (30/30 pass): tool dispatch + error reporting (Bash/Read/Edit/Write/Glob/Grep), slash-command pipeline (`/help` local; `/commit` and `/init` prompt-commands; markdown-skill invocation via `<cwd>/.harness/skills/`), six permission cases (deny, allow, deny-wins-within-layer, bypass-honors-deny, virtual-tool-name Bash→Read mapping, local-overrides-project layer precedence), four refusal cases (missing file, zero search results, prompt-injection resistance, verify-not-trust-user-claims), `@file` expansion, directory enumeration, and five multi-turn cases (cross-turn memory, refinement-after-tool-result, error-recovery, /compact-preserves-key-facts, /rollback-restores-parent-session). See [`tests/semantic/README.md`](tests/semantic/README.md).
