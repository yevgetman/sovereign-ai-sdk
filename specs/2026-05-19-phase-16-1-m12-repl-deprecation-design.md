...
**Predecessor:** M11 default-flip (`docs/07-history/state/2026-05-17-m11.md`), M11.5 inline picker card (`docs/07-history/state/2026-05-19-m11-5.md`).
...
- README + `docs/03-cli-reference/usage.md` updated to call `--ui repl` deprecated and point at the M13 removal timeline.
...
`docs/03-cli-reference/usage.md` — similar treatment in whatever flag/section covers `--ui`.
...
**Integration via boot smoke (extended):** the existing M11 boot-decision smoke harness (`docs/07-history/state/2026-05-17-m11-smoke/run-smoke.ts`) captures `--ui repl` / `SOV_UI=repl` / `ui.surface=repl` scenarios. M12 adds an extra assertion to each that the deprecation warning string appears on stderr; and at least one scenario with `SOV_NO_DEPRECATION_WARNING=1` asserts the warning is absent. Re-run captures the new transcripts to `docs/07-history/state/2026-05-19-m12-smoke/` for the close-out.
