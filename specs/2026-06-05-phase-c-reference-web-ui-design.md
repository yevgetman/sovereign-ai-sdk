...
**Parent roadmap:** `specs/2026-06-05-run-anywhere-harness-roadmap-design.md` (Phase C / module M8). **Depends on Phase A** (gateway, v0.6.18) **+ Phase B** (multi-client transport / `?follow` / reconnect, v0.6.19).
...
- `docs/03-cli-reference/usage.md` — "Open the web UI" (run `sov gateway`, browse to `http://host:port/`, paste the token).
- `docs/02-architecture/runtime-architecture.md` — note the bundled reference web client surface.
...
TDD where it applies (the serve route); the e2e is the behavioral proof. Full gate green. Update `docs/03-cli-reference/usage.md` + `docs/02-architecture/runtime-architecture.md` + state snapshot + `CLAUDE.md`/`AGENTS.md` pointer (don't touch the soak banner) + testing-log. Commit/push; `sov upgrade`; cut a release (the binary now serves the UI). Per `docs/05-conventions/autonomous-feature-builds.md`, executes immediately into the plan with no approval gate.
