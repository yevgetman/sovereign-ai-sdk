...
**Relationship:** Builds the **multi-channel gateway** differentiator named in ADR H-0010 (`~/code/sovereign-ai-docs/harness/decisions/0010-...`). Orthogonal to and parallel with the learning-loop soak (`docs/07-history/state/2026-06-04-learning-loop-spike-phase-1.md`) — the learning layer rides on top unchanged. Governed by `docs/05-conventions/autonomous-feature-builds.md`.
...
Ordering rule (per your "prioritize with respect to dev effort"): do the **cheapest high-leverage unlocks first**, respecting hard dependencies, so value lands early and each phase builds on the last. Effort is in subagent **dispatches** + rough token magnitude (per `docs/05-conventions/estimation.md`; subagent pace per the calibration memory).

### Phase A — Secure remote gateway (M1) · ~5–7 dispatches · ~250–350K
**✅ Shipped v0.6.17 (2026-06-05).** The `sov gateway` long-lived entrypoint serves the native HTTP+SSE protocol off-loopback with bearer auth + configurable host bind (loopback default) + CORS + a refuse-to-boot-when-exposed-without-auth guard; the TUI/serve/drive paths are byte-unchanged (auth/CORS are options-gated middleware). Security-reviewed **secure-to-ship**. See `specs/2026-06-05-phase-a-secure-remote-gateway-design.md` (spec), `plans/2026-06-05-phase-a-secure-remote-gateway.md` (plan), and `docs/07-history/state/2026-06-05-phase-a-gateway.md` (close-out).
...
**✅ Shipped v0.6.19 (2026-06-05).** The session event bus (`src/server/eventBus.ts`) is now **multi-subscriber** (fan-out to a `Set`) with a **bounded replay ring** (`gateway.eventBufferSize`, default 512); `GET /sessions/:id/events` serves **`Last-Event-ID` reconnect** (header or `?lastEventId` query — replays `seq >` cursor then live) + an optional **`?follow=true`** persistent stream (stays open across turns); the **bus lifecycle moved per-turn → per-session** (disposed on session disposal; full shutdown reclaims all). The default per-turn single-client contract is unchanged (`sov drive` byte-compatible). Correctness-reviewed (core robust) + 4 hardening fixes. Idle-session bus eviction deferred to Phase D. See `specs/2026-06-05-phase-b-multi-client-transport-design.md` (spec) and `docs/07-history/state/2026-06-05-phase-b-transport.md` (close-out).
...
**✅ Shipped v0.6.20 (2026-06-06).** A real, polished **browser chat client** served by the gateway — a single self-contained page (`src/server/webui.html`, inline CSS + vanilla JS, no framework, no build) embedded into the binary (text-import) + served OPEN at `GET /` / `/ui` by `buildAppWithRuntime` (before the `/sessions/*` bearer-auth). It drives the same HTTP+SSE protocol as the TUI (token-prompt auth with the token client-side only; `?follow` fetch-stream; `Last-Event-ID` reconnect); renders streaming text, collapsible thinking, tool cards, inline permission Approve/Deny, auto-reconnect, new-chat, cancel. Same-origin → no CORS. Validated by a real-browser Playwright e2e (real model; tool-use → permission → Approve; reconnect — incl. fixing an SSE header-flush bug, `a77ed32`) and reviewed **SECURE + CORRECT** (XSS-clean, token-safe, reconnect correct). Two known-minor LOW follow-ups deferred (duplicate permission card on mid-turn reconnect-replay; SSE parser splits on LF). See `specs/2026-06-05-phase-c-reference-web-ui-design.md` (spec, D1–D8), `plans/2026-06-05-phase-c-reference-web-ui.md` (plan), and `docs/07-history/state/2026-06-06-phase-c-webui.md` (close-out).
...
**✅ Shipped v0.6.21 (2026-06-06).** The gateway became the **persistent backbone**: `SessionSupervisor` (`src/server/sessionSupervisor.ts`) owns a periodic, `unref`'d **idle sweep** that gracefully + transparently reclaims a session's in-memory state (context + bus) once it is NOT turn-active, has NO subscribers, and is idle past `gateway.idleSessionTimeoutMs` (default 30 min, swept every `gateway.idleSweepIntervalMs`, default 5 min) — the durable SQLite row stays, so the next request **lazily rebuilds** it (restart-resume). New lifecycle routes `GET /sessions` (list + `live`/`turnActive`/`subscribers` annotations) + `DELETE /sessions/:id` (FK-safe `SessionDb.deleteSession` → 204), and a `POST /sessions` opt-in **concurrency cap** (`gateway.maxConcurrentSessions`, default 0 = unlimited — sweep-then-429). **Decision D1: extend the gateway** (the dormant `src/daemon/` skeleton stays dormant); gateway-scoped, so the TUI / `sov serve` / `sov drive` paths are byte-unchanged. Hardened with an in-flight sweep guard + an awaitable `stop()` that drains on shutdown (no DB-read-after-close race). Reviewed **READY TO SHIP** (no Critical/High); two known-LOWs documented (a tiny TOCTOU in `sweep()`, gated by the 30-min idle floor; a pre-existing `buildMockRuntime` test-isolation smell). Closes Phase B's deferred idle-bus-eviction (D9) + the interactive-buses-accumulate carry-forward. Ships a **systemd + launchd service-install** story (X2). See `specs/2026-06-06-phase-d-persistent-supervisor-design.md` (spec), `plans/2026-06-06-phase-d-persistent-supervisor.md` (plan), and `docs/07-history/state/2026-06-06-phase-d-supervisor.md` (close-out).
...
**✅ Shipped v0.6.22 (2026-06-06).** The gateway became **multi-user**: `gateway.principals: [{ id, token, name? }]` (XOR the single `gateway.token`; `id` validated as a safe path segment) maps bearer tokens → named principals; `resolvePrincipal` (`src/server/principals.ts`) constant-time-compares against every principal (no early-exit timing leak) and the principal-aware auth middleware attaches the resolved principal to the request context — with **no anonymous bypass** (a resolving token is required on every request, including loopback, in principals mode). Sessions gained a nullable `owner_id` column (SessionDb migration 4→5); `POST /sessions` stamps the caller and **every `/sessions/:id/*` route returns 404 on owner mismatch** (existence-hiding; covers messages/turns/events/approvals/cancel/compact/commands/skills/DELETE), with `GET /sessions` owner-filtered. Memory + learning gained an owner-derived per-user namespace (`$HARNESS_HOME/users/{id}/{memory,learning}/…`, validated at the path boundary) — **two-layer isolation** (authz + owner-derived scoping). Scope is **within-org / single-trust-domain** (trusted-but-separate users); hostile cross-tenant / managed-multi-tenant isolation is **founder-reserved**. The default `sov` (TUI) / `sov serve` / `sov drive` surfaces configure no principals → byte-unchanged. The hard **adversarial security review** found + fixed two cross-user leaks (C1 Critical — the `memory` tool ignored the user scope; H1 High — the compaction child dropped the owner; `d7559a8`) and returned **SECURE-TO-SHIP** (A cannot reach B's sessions/memory/learning). Known v1 limits: operator-side traces + fine-tune trajectories aren't per-user-partitioned (operator-only, never API-served); the admin learning CLI operates on the legacy top-level corpus. See `specs/2026-06-06-phase-e-multi-user-design.md` (spec, D1–D8), `plans/2026-06-06-phase-e-multi-user.md` (plan, E-T1…E-T9), and `docs/07-history/state/2026-06-06-phase-e-multi-user.md` (close-out).
...
Per `docs/05-conventions/autonomous-feature-builds.md`, **incrementally**:
1. This roadmap spec (committed) is the organizing artifact.
2. For each phase in order: write the detailed spec (`specs/`) → self-review → write the build plan (`plans/`) → execute autonomously (subagent-driven) → ship (docs + tests + commit/push + `sov upgrade` + release) → let the shipped phase inform the next phase's spec.
