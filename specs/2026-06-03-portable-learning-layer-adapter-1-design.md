...
- In-repo kickoff: `plans/2026-06-03-learning-loop-spike-kickoff.md`
...
| **D7** | Persist scope (Phase 1) | Define `PersistPort`; adapter #1 binds it to FS using the existing `src/learning/paths.ts` layout. Add a **Persist-backed instinct reader** for Recall, sharing a **pure serde** with the existing (synchronous) `InstinctStore` — so the new read path is portable without rippling an async refactor through `InstinctStore`'s ~8 callers. The full `InstinctStore`→`PersistPort` migration + observations/trajectory sealing + a mock-host `PersistPort` are Phase 2. |
...
2. Plan — `superpowers:writing-plans` produces `plans/2026-06-03-learning-loop-spike-phase-1.md` (checkbox tasks for `superpowers:subagent-driven-development`).
3. Subagent-driven implementation per `docs/05-conventions/subagent-policy.md`: **Opus default; Sonnet only for trivially mechanical, fully-specified tasks; never Haiku.**
4. TDD throughout (≥80% coverage). Pre-commit gate: `bun run lint && bun run typecheck && bun run test` — all green.
5. Atomic commits, autonomous push to `master`. `sov upgrade` + cut the next binary release in-session once runtime code changes (per `docs/05-conventions/cutting-releases.md`).
