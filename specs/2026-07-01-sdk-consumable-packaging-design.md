...
>
> **Amendments from grounding (2026-07-01, pre-plan — a 6-dimension code sweep verified against HEAD `0bff267`; these refine the "how," scope is unchanged):**
> - **Deep-subpath exports are MANDATORY (supersedes §8's "deep paths are internal, omitted from the tarball").** The wrapper imports **102 distinct open modules across ~345 edges from 94 files**, but the `.` barrel only covers ~29. So `@yevgetman/sov-sdk` ships **all compiled `dist`** and exposes **both** `.` → `dist/sdk.js` (the frozen, semver'd **public** surface) **and** `./*` → `dist/*.js` (every module — **internal/unstable, not semver-covered**, but shipped so the wrapper + ~250 root tests can deep-import). `files: ['dist','src','LICENSE','README.md']` *(amended at Phase 3, see below)*. `@yevgetman/sov-protocol` exposes only `.`.
> - **Tarballs ship `src/` alongside `dist/` (ratified 2026-07-02, Phase-3 build + review).** The dual-condition exports map (`bun` → `src/*.ts` for the no-build in-repo dev loop; `types`/`import` → `dist` for Node consumers) means an INSTALLED Bun consumer resolves the `bun` condition inside the tarball — so the source tree must ship. The packages are MIT open-core *by construction* (boundary lint + Bun-residual guard + tarball allow-list = triple guard), so §9.2's intent — no proprietary leakage, no stray files — is preserved; the tarball assertion excludes tests/configs and pins both entry forms. Reviewer-adjudicated ACCEPT over pack-time exports-stripping (repo≠artifact divergence) or dropping the bun condition (reintroduces build-before-test staleness).
> - **SDK runtime deps are SIX, not one (corrects §6):** `zod`, `yaml`, `@modelcontextprotocol/sdk`, `@anthropic-ai/sdk` (existing value imports) + `picomatch` (replaces `Bun.Glob().match`) + `tinyglobby` (replaces `Bun.Glob().scanSync`). `@yevgetman/sov-protocol` has **zero** runtime deps.
> - **Bun-API shims (Node-compat, §6):** the 5 `Bun.spawn` call sites collapse onto **one** `node:child_process`-backed shim that returns the **already-defined** `SpawnedProc`/`SpawnFn` shape (`src/runtime/executorPort.ts:21-36`) via `Readable.toWeb()`; `Bun.Glob` → `picomatch` (pure match, security-relevant — parity-tested) + `tinyglobby` (fs scan); `Bun.serve`+`Bun.file` in `StaticSiteValidateTool` → `node:http` + `fs.createReadStream`. `bun:sqlite` confirmed **absent** from the open graph.
> - **`version.ts` move gotcha:** moving it makes `../package.json` resolve to the SDK's own `0.1.0`, which would change what `sov --version`/`/health` prints. The wrapper gets its **own** version source so `sov --version` stays on the harness `0.6.x` line; the SDK's `version.ts` legitimately reports the SDK's `0.1.0`.
> - **Boundary lint must be rebased** to `^packages/sdk/src/` + `^packages/protocol/src/` (else the `^src/`-anchored rule matches nothing and silently passes); post-split the invariant is also **structural** (an open package has no dependency on the wrapper). **`engines.node = '>=20'`** (JSON import attributes + global fetch). The test retarget (~250 files) is a first-class work item, not a footnote.
...
Each open package ships **compiled `dist/*.js` + `dist/*.d.ts`** with a conditional `exports` map (`types`/`import`/`default`), a `files` allow-list (dist + src + LICENSE + README — **the open source tree ships by design** (bun-condition resolution, see amendments); no tests/configs, no proprietary files), its own semver line, an MIT `LICENSE`, and a README/quickstart. The wrapper depends on both via `workspace:*` and retargets its imports of open modules to the package names. `bun run lint && typecheck && test` and the boundary lint stay green throughout.
...
5. **Publish-readiness + docs — then STOP.** README/quickstart + a written semver/stability policy per package; API docs/examples (promote `examples/embed`); an `npm pack` tarball-contents assertion (dist + src + LICENSE + README, per the ratified amendment); `npm publish --dry-run` green for both open packages; a `PUBLISHING.md` runbook. **Do not run the real publish or flip the repo public** — CEO's button.
...
2. Each open package **builds** (`tsc` emit → `dist/*.js` + `*.d.ts`) and **packs** (`npm pack`) to a tarball whose contents are dist + src + LICENSE + README **only** (the open source tree ships by design — see amendments) — asserted by a tarball-contents test (no tests/configs, no proprietary module).
