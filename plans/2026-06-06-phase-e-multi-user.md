...
> **For agentic workers:** REQUIRED SUB-SKILL: `superpowers:subagent-driven-development`. Checkbox steps. Executes per `docs/05-conventions/autonomous-feature-builds.md` — no approval gates. Read the cited files first to match exact current signatures, then TDD (red → green → commit). **Security-critical phase — negative isolation tests are mandatory, not optional.**

**Goal:** Multiple distinct users share one self-hosted gateway with isolated sessions, memory, and learning, per `specs/2026-06-06-phase-e-multi-user-design.md`.
...
**Files:** `docs/03-cli-reference/usage.md`, `docs/02-architecture/runtime-architecture.md`, `docs/06-testing/testing-log.md`, `docs/07-history/state/2026-06-06-phase-e-multi-user.md`, roadmap spec (mark Phase E shipped), `CLAUDE.md`+`AGENTS.md` (state pointer; **DON'T touch the soak banner**; `diff` empty), `package.json`.
- [ ] `docs/03-cli-reference/usage.md`: "Multi-user gateway" — configure `gateway.principals` (id/token/name; XOR the single `token`), the trust model (within-org; not hostile-multi-tenant), owner-only sessions, per-user memory + learning, the no-anonymous-bypass rule. Curl examples with two principals.
- [ ] `docs/02-architecture/runtime-architecture.md`: the identity + per-user scoping layer (token→principal, owner_id, owner-derived memory/learning namespace; two-layer isolation).
- [ ] State snapshot `docs/07-history/state/2026-06-06-phase-e-multi-user.md` (match recent style): what shipped, the within-org scope + founder-reserved managed-tenant line, the two-layer isolation, the security-review verdict, the known v1 limitation (traces/trajectories not per-user-partitioned), test count, version (v0.6.22), roadmap (A–E shipped; **F channels remains**), learning soak continues.
