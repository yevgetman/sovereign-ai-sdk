...
| `docs/07-history/state/2026-05-23-phase-2-task-routing.md` | Close-out snapshot. |
...
| `docs/03-cli-reference/usage.md` | Add "Routing observability" subsection. |
| `docs/06-testing/testing-log.md` | Append Phase 2 entry. |
...
**Files:**
- Create: `docs/07-history/state/2026-05-23-phase-2-task-routing.md`.
- Modify: `docs/03-cli-reference/usage.md` (add "Routing observability" subsection), `docs/06-testing/testing-log.md` (append), `CLAUDE.md` + `AGENTS.md` (byte-identical, update state pointer).

State snapshot mirrors `docs/07-history/state/2026-05-23-phase-1-task-routing.md` structure: HEAD, chain since predecessor, suite numbers, ADRs (none — purely additive), Phase status, what shipped, behavioral notes, open follow-ups (Phase 2.5 conditionals + profile presets), postmortem-rule compliance.
...
Pass: bump `0.4.1 → 0.5.0` (minor — four new SSE event types are a notable feature surface for downstream consumers). Cut via `bun run scripts/release.ts` per `docs/05-conventions/cutting-releases.md`. Smoke `~/.sov/bin/sov upgrade && sov --version`.
