...
Spec: `specs/2026-06-15-multi-agent-workflows-design.md`. Decisions: declarative deterministic engine + path-granular locking; parallel fan-out is the headline. Autonomous build (spec → plan → execute → ship, no gates).
...
**W6 — Bundle example + docs (owner E).** `bundle-default/workflows/review.yaml` (a real, useful fan-out→verify→synthesize example) + `docs/03-cli-reference/usage.md` (a Workflows section) + `docs/04-extending/extending.md` (authoring recipe) + a semantic/behavioral smoke.
