...
**Goal:** Update `CLAUDE.md` boot block, `README.md`, `docs/03-cli-reference/usage.md`, and any code comments that reference `terminalRepl.ts:NNN` line numbers (now stale).
...
- [ ] **Step 4: Update `docs/03-cli-reference/usage.md`.**
...
The boot block currently references `docs/07-history/state/2026-05-19-m12.md`. After T12 lands the new snapshot, this points there. **Defer this step until T12.** Mark this step `- [ ]` and revisit at the start of T12 (T12 will both write the snapshot AND update CLAUDE.md in the same commit).
...
For each false claim or stale reference, report file:line and the offending text. Don't report state snapshots, ADRs, plans, or postmortems under docs/07-history/state/ or docs/07-history/postmortems/ or plans/ — those are historical records and should be preserved.
...
**Files:** No source changes. Smoke outputs go to `docs/07-history/state/2026-05-19-m13-smoke/`.
...
Create `docs/07-history/state/2026-05-19-m13-smoke/README.md` summarizing the four scenarios with PASS/FAIL marks.

- [ ] **Step 7: Commit.**

```bash
git add docs/07-history/state/2026-05-19-m13-smoke && git commit -m "$(cat <<'EOF'
...
**Files:**
- Create: `docs/07-history/state/2026-05-19-m13.md`
- Modify: `DECISIONS.md`
- Modify: `CLAUDE.md`
- Modify: `AGENTS.md` (byte-identical mirror)
- Modify: `docs/08-roadmap/backlog/post-phase-13-4.md` (sync line)
- Modify: `docs/06-testing/testing-log.md` (append entry)

- [ ] **Step 1: Write `docs/07-history/state/2026-05-19-m13.md`.**

Follow the style of `docs/07-history/state/2026-05-19-m12.md`. Include:
...
- [ ] **Step 5: Sync `docs/08-roadmap/backlog/post-phase-13-4.md`.**
...
Add a newest-first entry to `docs/06-testing/testing-log.md` matching the format of recent entries. Include: date, scope (M13 close-out), suite count, smoke results, any caveats.
...
```bash
git add docs/07-history/state/2026-05-19-m13.md DECISIONS.md CLAUDE.md AGENTS.md docs/08-roadmap/backlog/post-phase-13-4.md docs/06-testing/testing-log.md && git commit -m "$(cat <<'EOF'
...
Per the repo's standing rule (`docs/05-conventions/sov-upgrade.md`), any change under `src/` must be followed by `sov upgrade` to keep the global binary current.
...
- [ ] 4/4 smoke scenarios PASS per `docs/07-history/state/2026-05-19-m13-smoke/`.
- [ ] `git status` is clean.
- [ ] `CLAUDE.md` and `AGENTS.md` byte-identical (`diff CLAUDE.md AGENTS.md` returns no output).
- [ ] `docs/06-testing/testing-log.md` has a newest-first M13 entry.
- [ ] `docs/07-history/state/2026-05-19-m13.md` exists and is referenced from CLAUDE.md's Session boot block.
