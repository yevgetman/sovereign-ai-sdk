...
## ADR M11.5-01 — Picker as a side-effect on CommandContext (capability detection)

Decision: M11.5 adds an optional `requestPicker(config: PickerOpenConfig)` method to `CommandContext` (defined in `src/commands/types.ts`). Picker-driven commands (`/model`, `/resume`, `/export`) check its presence: when defined (server mode — the route at `src/server/commandContext.ts:178-188` populates it), the command emits a `pickerOpen` side-effect on the response envelope and returns an empty string. When undefined (REPL surface — `terminalRepl.ts` doesn't populate it), the command falls back to the legacy in-process `pick()` flow at `src/ui/picker.ts`. The TUI receives the side-effect and renders an inline `PickerCard` matching the reference UX (`~/Desktop/goodux.png`).

Rationale: Decouples picker *state* (server: what to pick, what the options are) from picker *rendering* (client: how to draw the card). Pre-M11.5, `/model` no-args ran `pick()` which writes raw-mode escape sequences to stdout. The Bubble Tea TUI was simultaneously painting its own layout via the alternate-screen buffer; the two render loops collided producing the broken layout in `~/Desktop/ux1.png`. The capability-detection shape (`if (ctx.requestPicker)`) mirrors the existing `cleanupPhantomReviews` pattern in `CommandContext` (`/review activity` uses the same conditional), keeps the REPL legacy path untouched per Postmortem Rule 1, and means future picker-driven commands automatically benefit without per-command branching at the server route. The shape generalizes — `/model`, `/resume`, `/export` migrated in T3 + T7; any future picker (e.g., `/skills install`, `/memory edit`) gets the protocol for free.
...
Decision: M11.5 ships the inline `PickerCard` for the TUI surface only. The readline REPL surface continues to use the in-process `pick()` flow at `src/ui/picker.ts`. The two surfaces have different picker *implementations* but the same user-visible *behavior* — a user on either surface can select a model, resume target, or export format.

Rationale: M12 (terminalRepl deprecation) is the natural endpoint for unifying picker implementations. Building a `PickerCard`-equivalent for the readline REPL today would require either a sub-process invocation of the Go TUI binary or building a render loop inside `src/ui/terminalRepl.ts`. Both are substantial work for a surface that M13 will retire entirely. Capability-detection (ADR M11.5-01) keeps both paths working independently with no coupling. Postmortem Rule 1 is honored — `src/ui/terminalRepl.ts` is not modified by M11.5.

Alternatives rejected: (a) Migrate the REPL to render via `PickerCard` too — substantial work for a soon-to-be-retired surface; risks regressions on a working code path; needs a render loop. (b) Retire `/model`, `/resume`, `/export` from the REPL entirely — they're documented and used today; removal is a regression. (c) Hard-deprecate the REPL in M11.5 (skip M12) — conflates milestones; M12 is the proper deprecation discussion per ADR M11-03's scope-discipline principle.

Status: implemented (M11.5 — by design; REPL CommandContexts in `src/ui/terminalRepl.ts` and `src/cli/dispatchCommand.ts` do not populate `requestPicker`). Spec: §2 (Out of scope), §4.1 (capability detection), Risk #2 in §9.

## ADR M11.5-03 — Stateless server: resolution via fresh dispatch, not continuation tokens

Decision: When the user selects an item from the inline `PickerCard`, the Go TUI dispatches `/<command> <value>` as a fresh slash command via the existing M10.5 `POST /sessions/:id/commands` route. The server holds NO suspended-command state across the picker round-trip — the original command's call to `requestPicker` returned synchronously after recording the side-effect; the second dispatch is independent.

Rationale: Statelessness is the simplest invariant. The arg-form of every picker command (`/model claude-sonnet-4-6`, `/resume <uuid>`, `/export md`) already worked pre-M11.5 — the picker is just collecting the arg interactively. Continuation tokens would require server-side state keyed by `(sessionId, pickerId)` with timeout / cleanup / restart semantics, plus a new endpoint to resolve them. Long-polling has its own complications (HTTP timeouts on slow user selection, complications for the existing request/response model). Fresh dispatch reuses the existing route, idempotent semantics, and gives correct behavior even if the user opens multiple TUIs against the same session (each gets its own picker; selections don't cross-contaminate).

Alternatives rejected: (a) Continuation tokens (server stores `{ pickerId, originalCommand, originalArgs, resumeFn }`; client POSTs the chosen value to a `/pickers/:id/resolve` route) — stateful server, complicates timeout / cleanup / restart, gains nothing the fresh-dispatch path doesn't already give us. (b) Long-poll: server holds the request open while the picker is rendered, client streams the selection back over the same connection — HTTP timeouts, complicates the dispatcher's request/response model.

Status: implemented (M11.5 — `packages/tui/internal/app/app.go`'s picker-key handler at the `tea.KeyEnter` branch calls `dispatchCommandCmd(m.baseURL, m.sessionID, cmdName, value)`). Spec: §3 (architecture diagram), §6 (ADR M11.5-03). Smoke verified: Agent B's `agent-b-modelchanged.json` confirms the second dispatch returns the normal `modelChanged` side-effect with no picker state on the server.

## ADR M12-01 — REPL deprecation warning fires on explicit opt-in; not on missing-binary fallback

Decision: The M12 deprecation predicate is `resolution.surface === 'repl'`, NOT `effectiveSurface === 'repl'`. When the missing-binary fallback at `src/main.ts:221-230` flips `effectiveSurface` from `'tui'` to `'repl'`, `resolution.surface` stays `'tui'` and the M12 warning stays silent. The existing M11 "sov-tui binary not found" warning still fires for that path — it's the right diagnostic for that situation; M12 doesn't need to pile on.

Rationale: Users hit the missing-binary fallback when they haven't trusted the postinstall (`bun pm -g trust @yevgetman/sov`) or don't have Go ≥ 1.24 on PATH. They didn't choose REPL — they chose TUI (or the default) and the harness softly downgraded them. Warning them about REPL deprecation in addition to the existing fallback line would be punitive and confusing ("I asked for the TUI, why are you telling me REPL is deprecated?"). Reading `resolution.surface` (the user's chosen surface) instead of `effectiveSurface` (the post-fallback value) makes the predicate correctly distinguish "I chose REPL" from "system gave me REPL."
...
Decision: `SOV_NO_DEPRECATION_WARNING=1` suppresses the M12 warning. There is no `ui.suppressDeprecationWarning` config field, no per-session "I've seen this" acknowledgement state, no profile-level toggle. The env var is the only mechanism.

Rationale: This deprecation is temporary by design. M13 removes the REPL entirely, at which point the warning becomes moot — keeping any config field around past that point would be dead schema surface that future maintainers have to either keep wired (cost) or migrate users away from (cost). An env var has no namespace claim, is shell-local, easy to set per-invocation or in a shell rc, and disappears when the env-var name stops being checked. The env-var name is also discoverable from the warning text itself — users who see the message and don't want it just copy/paste the env-var name into their shell rc; no docs hunt required.

The strict-equal-`'1'` semantics are intentional: `'0'`, empty string, `'true'`, `'yes'` etc. do NOT suppress. This is documented in the warning text indirectly (the user sees `=1` and types `=1`) and explicitly in the unit-test suite (`tests/cli/replDeprecation.test.ts`). Treating any truthy-looking value as suppression would make accidental shell-rc typos silently disable the warning.
