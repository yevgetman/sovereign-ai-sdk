...
**Tests (updated, RED-aware).** `tests/config/subscriptionExecutor.test.ts` — accepts all four modes incl. `bypass`; still rejects unknown tokens incl. `bypassPermissions`. `tests/runtime/subprocessExecutor.test.ts` — new posture block: unset defaults to `--dangerously-skip-permissions`; constrained modes emit `--permission-mode` and NO dangerous flag; `bypass` emits the dangerous flag and omits `--permission-mode`. `tests/config/catalog.test.ts` — permissionMode choices assert `['bypass','plan','acceptEdits','default']` with `bypass` leading. Docs: `docs/usage.md` (config example + permissionMode bullet + ToS-boundary note), the spike spec got a dated UPDATE note (default flipped; original design rationale preserved), and the CLAUDE.md/AGENTS.md index line corrected (was "bypass rejected").
...
Closed a catalog gap: the `subscriptionExecutor` block (the opt-in headless Claude Code sub-agent executor, v0.6.27) was in `SettingsSchema` + `docs/usage.md` but never added to the curated config catalog (`src/config/catalog.ts`), so it was editable ONLY by hand-editing `~/.harness/config.json` — it never appeared in `/config` or `sov config`. Added a **Subscription executor** group (placed after Task routing, its mutually-exclusive sibling) covering all six fields (enabled / engine / binary / permissionMode / timeoutMs / maxTurns).
...
**Docs.** `docs/usage.md` MCP security notes now document the redirect behavior (auth headers dropped on a cross-origin hop; configure the canonical same-origin endpoint; prefer `bearerToken`/`Authorization` / `SOV_MCP_<ALIAS>_TOKEN`).
...
Arc C of the ecosystem-openness work, per `docs/specs/2026-06-08-remote-mcp-transport-design.md`. Opens the harness to remote/hosted MCP servers (Streamable HTTP + legacy SSE) alongside the existing stdio servers. **No release** (the controller reviews + releases). Built RED→GREEN; committed atomically; pushed `origin/master`.
...
**Gate.** `bun run lint` clean (biome, 690 files) + `bun run typecheck` clean (`tsc --noEmit`) + `bun run test` **3217 pass / 0 fail / 14 skip** (+20 from the ~3197 baseline; no new failures, no regressions). Go: `cd packages/tui && go build ./... && go test ./...` — all packages green (build exit 0; app/components/render/style/theme/transport all `ok`). The pre-existing AGENTS.md guard-scanner `[WARN]` lines (the installer `curl | bash` doc snippet) are unrelated. Docs updated: `docs/usage.md` (`/skills import` verb + "Porting a Claude Code skill" + `:`-glob caveat + `/skill` enforcement note + API route table) and `docs/extending.md` (`allowedTools` enforced on `/skill`; `SkillTool` advisory; importing CC skills).
...
**Docs.** `docs/usage.md` — added a mutual-exclusion note (with cross-reference) to BOTH the task-routing section and the subscription-executor section. `docs/specs/2026-06-08-subscription-executor-spike.md` — added an "Interaction with task routing (mutual exclusion)" section (the conflict, the parse-time decision, and the future "subscription-as-a-routing-lane" compose explicitly out of scope).
...
5. `docs/backlog/post-phase-13-4.md` first paragraph — "freshest concretely-specced next phase is Phase 21" updated to "freshest closed phases are Phase 17 + Phase 18; next concretely-specced phases are Phase 21 M2 plus demand-gated 16.5/19/20 channels."
...
**ADRs:** none new. Phase 18 is purely additive; all design decisions are captured inline in the plan at `docs/plans/2026-05-23-phase-18-openai-api-server.md` (D1-D14 + OQ1-OQ5 in the "Locked design decisions" table at the top). No foreground refactor, no surface removal, no architectural pivot.
