...
**Spec:** [`docs/specs/2026-05-21-binary-distribution-design.md`](../specs/2026-05-21-binary-distribution-design.md) — 7 ADRs locked (P21-01..07). Read it first; this plan executes against it.
...
| `docs/state/2026-05-22-phase-21-m1.md` | M1 close-out snapshot |
...
| `docs/testing-log.md` | Smoke-pass entries (macOS + Linux); release attempt entry |
...
**Files:** none in private repo; appends to `docs/testing-log.md` in Task 12.
...
Status: implemented (Phase 21 M1 — Task 1 commit). Plan: `docs/plans/2026-05-22-phase-21-binary-distribution.md`.
...
Status: implemented (Phase 21 M1 — Task 2 commit). Plan: `docs/plans/2026-05-22-phase-21-binary-distribution.md`.
...
```markdown
**Plan:** `docs/plans/2026-05-22-phase-21-binary-distribution.md` (in the harness repo).
```
...
**Files:**
- Create: `docs/state/2026-05-22-phase-21-m1.md`
...
**ADRs:** P21-A (binary-install asset discovery), P21-B (upgrade-mode detection). The 7 spec ADRs (P21-01..07) live in `docs/specs/2026-05-21-binary-distribution-design.md` — those are design decisions, not runtime-local ADRs.
...
Edit `docs/testing-log.md`. Insert a new entry at the TOP (after the intro paragraphs, before the existing first entry):
...
```markdown
3. **`docs/state/2026-05-21-ux-fixes-r5.md`** — most recent close-out snapshot ...
```

Replace with:

```markdown
3. **`docs/state/2026-05-22-phase-21-m1.md`** — most recent close-out snapshot (Phase 21 M1 shipped 2026-05-22 — binary distribution: Bun `--compile` for darwin-arm64/darwin-x64/linux-x64; Go cross-compile sibling; public `sov-releases` repo with `install.sh`; `sov upgrade` auto-detects binary mode; v0.2.0 live; macOS + Linux smokes green). Predecessor: `docs/state/2026-05-21-ux-fixes-r5.md` (ux-fixes rounds 3-5 close-out). Replaced each session — find the latest via `ls docs/state/*.md | sort -r | head -1`.
```

Also update the "Current state" table row pointing at the snapshot. Find:

```markdown
| [`docs/state/2026-05-21-ux-fixes-r5.md`](docs/state/2026-05-21-ux-fixes-r5.md) | **Canonical current-state snapshot — ...
```

Add a new row ABOVE it pointing at the new snapshot:

```markdown
| [`docs/state/2026-05-22-phase-21-m1.md`](docs/state/2026-05-22-phase-21-m1.md) | **Canonical current-state snapshot — Phase 21 M1 shipped 2026-05-22.** Binary distribution: per-platform Bun `--compile` + Go cross-compile + side-car bundle-default + public `sov-releases` repo with `curl ... | bash` installer. `sov upgrade` auto-detects binary mode. v0.2.0 live; macOS + Linux smokes pass; darwin-x64 deferred to first beta-user report. ADRs P21-A + P21-B locked. TS suite **1958+/0/14**; Go unchanged. |
```
...
Edit `docs/state/2026-05-22-phase-21-m1.md`. Replace the `<SHA after Task 11 commits ...>` placeholder at the top with the actual current short SHA:
...
```bash
git add docs/state/2026-05-22-phase-21-m1.md \
        docs/testing-log.md \
        docs/backlog/post-phase-13-4.md \
        CLAUDE.md AGENTS.md
git commit -m "$(cat <<'EOF'
docs: Phase 21 M1 close-out — state snapshot + testing log + index pointers

State snapshot at docs/state/2026-05-22-phase-21-m1.md replaces the ux-fixes
