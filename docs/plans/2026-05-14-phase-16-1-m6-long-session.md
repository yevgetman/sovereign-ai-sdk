...
**Goal:** Wire three subsystems — **microcompaction** (per-part `tool_result` clearing during long turns), **full compactor** (`/compact` slash command + threshold-triggered `shouldCompactProactively`), and **context-overflow auto-recovery** (`createClearedChildSession` retry path) — into the Phase 16.1 split-process architecture. Three prereq boxes flip in `docs/backlog/phase-16-rebuild-prereqs.md` (rows 7, 8, 15). `--ui tui` reaches feature parity with terminalRepl on the surfaces a long session needs first.
...
**Spec references:**
- `docs/specs/2026-05-13-phase-16-1-tui-rebuild-design.md` §10 (M6 row), §13 (open Qs deferred to plan)
- `docs/backlog/phase-16-rebuild-prereqs.md` rows 7 (Compactor), 8 (Microcompaction), 15 (Context-overflow auto-recovery)
- `docs/postmortems/2026-05-12-phase-16-revert.md` Rules 1–4 (terminalRepl untouched; coexistence; audit before flip)
- `docs/plans/2026-05-14-phase-16-1-m5-user-noticed.md` — M5 plan for the wiring-into-server pattern this milestone repeats
...
| `docs/backlog/phase-16-rebuild-prereqs.md` | Flip checkboxes for rows 7, 8, 15 with `(M6 — 2026-05-XX)` annotation. |
...
| T7 | `tests/cli/tuiLauncherIntegration.test.ts`, `docs/backlog/phase-16-rebuild-prereqs.md`, `DECISIONS.md`, `docs/state/<date>.md`, `CLAUDE.md`, `AGENTS.md` | `docs/state/<date>.md` | (extends integration test with three scenarios) |
...
- Create: `docs/state/2026-05-XX.md` (today's date if M6 lands today, else the close-out date)
...
Create `docs/state/2026-05-XX.md` (today's date, or the close-out date if M6 spans into the next day) using `docs/state/2026-05-14.md` as the template. Required sections:
...
Then move `docs/state/2026-05-14.md` → `docs/state/archive/2026-05-14.md` (or whatever the existing archive convention requires — verify by reading the `docs/state/archive/` directory).

- [ ] **Step 6: Update `CLAUDE.md` + `AGENTS.md` state-snapshot pointer**

Both files reference `docs/state/2026-05-14.md` in the session-boot list and the doc-index table. Update both to the new dated file. After editing both, verify byte-identical mirror:
...
In `docs/testing-log.md`, prepend (newest-first) a 2026-05-XX entry covering: M6 build pass; the three new unit-test files; the integration-smoke extension; suite delta (1897 → 1897+N); any regressions caught + fixed during the build.

- [ ] **Step 9: Commit and run sov upgrade**

```bash
git add tests/cli/tuiLauncherIntegration.test.ts docs/backlog/phase-16-rebuild-prereqs.md DECISIONS.md docs/state/ CLAUDE.md AGENTS.md docs/testing-log.md
git commit -m "$(cat <<'EOF'
...
> `sov upgrade` is required because M6 touched `src/` and `packages/tui/`. See `docs/conventions/sov-upgrade.md`.
...
5. **Prereq box numbers** — rows 7, 8, 15 verified against `docs/backlog/phase-16-rebuild-prereqs.md` at plan-write time.
