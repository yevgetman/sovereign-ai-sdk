...
**Docs:**
- `docs/backlog/phase-16-rebuild-prereqs.md` — flip the three M4 checkboxes to `[x]`.
- `docs/usage.md` — add an "ui tui flag coverage" table.
- `docs/testing-log.md` — append M4 close-out entry.
- `docs/state/2026-05-14.md` (new) — M4 close-out snapshot, supersedes 2026-05-13.
...
- Create: `docs/state/2026-05-14.md`
- Archive: `docs/state/2026-05-13.md` → `docs/state/archive/2026-05-13.md`
- Modify: `CLAUDE.md`, `AGENTS.md`
- Modify: `DECISIONS.md`

- [ ] **Step 1: Flip the three M4 checkboxes**

Edit `docs/backlog/phase-16-rebuild-prereqs.md`. Find rows 6 (Session DB persistence), 9 (Preflight checks), 23 (Full CLI flag forwarding) and change their checkboxes from `[ ]` to `[x]` with the close-out shape used by prior milestones (commit SHA + date next to the box if the doc's existing pattern uses that — match whatever's there).

- [ ] **Step 2: Add the `--ui tui` flag-coverage table to `docs/usage.md`**
...
Edit `docs/testing-log.md`. Newest-first ordering — prepend at the top a date-stamped entry summarizing M4 changes, the test count delta, manual smoke results, and any open follow-ups. Match the structure of prior entries.

- [ ] **Step 4: Archive prior snapshot**

```bash
git mv docs/state/2026-05-13.md docs/state/archive/2026-05-13.md
```
...
to:
```
3. `docs/state/2026-05-14.md` — **most recent close-out snapshot** (Phase 16.1 M4 shipped). Read this BEFORE the build plan to know what shipped, what's open in the backlog, and where to start. Replaced each session.
```

Update the Documentation Index section the same way — `docs/state/2026-05-14.md` becomes the canonical row; `docs/state/archive/2026-05-13.md` joins the archive list. Update the "Phases — where we are" section to reflect M4 close.
...
```bash
git add docs/backlog/phase-16-rebuild-prereqs.md docs/usage.md docs/testing-log.md docs/state/2026-05-14.md docs/state/archive/2026-05-13.md CLAUDE.md AGENTS.md DECISIONS.md
...
Append to `docs/testing-log.md` (under the M4 entry from Step 3): a bulleted list summarizing the 11 manual checks + their outcomes. Commit.
...
**Spec coverage check.** M4 owns three subsystems per `docs/specs/2026-05-13-phase-16-1-tui-rebuild-design.md` §10 row:
...
Plan complete and saved to `docs/plans/2026-05-14-phase-16-1-m4-critical-correctness.md`.
