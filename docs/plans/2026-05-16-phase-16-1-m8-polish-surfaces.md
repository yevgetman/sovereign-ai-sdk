...
**Spec references:**
- `docs/specs/2026-05-13-phase-16-1-tui-rebuild-design.md` §9 (M8 group row), §10 (M8 row in milestone sequence), §13 (open Qs deferred to plan)
- `docs/backlog/phase-16-rebuild-prereqs.md` rows 14 (Router), 16 (Capture/Replay), 17 (@file), 18 (Subdir hints), 19 (Skill-as-slash), 20 (Skill visibility), 21 (Goodbye), 22 (Stall), 24 (Expand)
- `docs/backlog/post-phase-13-4.md` item #30 (server-mode `subagentDefaultProvider`/`subagentDefaultModel` not specialized for router mode — closes in T1)
- `docs/postmortems/2026-05-12-phase-16-revert.md` Rules 1–4 (terminalRepl untouched; coexistence; audit before flip)
- `docs/plans/2026-05-14-phase-16-1-m6-long-session.md` — M6 plan establishing the per-route slash-command pattern (M6 T5 `/compact` route + M6 T6 TUI intercept)
- `docs/plans/2026-05-15-phase-16-1-m7-hermes-layer.md` — M7 plan establishing the SessionContext per-session pattern; M8 extends a couple of SessionContext fields (subdirectoryHintState in T3)
- `docs/state/2026-05-15.md` — M7 close-out + post-close hardening snapshot (where M8 boots from)
...
| `docs/backlog/phase-16-rebuild-prereqs.md` | Flip checkboxes for rows 14 (Router), 16 (Capture/Replay), 17 (@file), 18 (Subdir hints), 19 (Skill-as-slash), 20 (Skill visibility), 21 (Goodbye), 22 (Stall), 24 (Expand) with `(M8 — 2026-05-XX)` annotation. |
| `docs/backlog/post-phase-13-4.md` | Close item #30 (router-mode default subagent provider/model in server build). |
...
| `docs/state/2026-05-XX.md` (close-out date) | New close-out snapshot — supersedes `docs/state/2026-05-15.md`. |
...
| T8 | `tests/server/integration/m8Full.test.ts`, `docs/backlog/phase-16-rebuild-prereqs.md`, `docs/backlog/post-phase-13-4.md`, `DECISIONS.md`, `docs/state/<date>.md`, `CLAUDE.md`, `AGENTS.md` | `tests/server/m8Full.test.ts` | (extends integration test with full M8 sweep) |
...
```bash
git add src/server/runtime.ts tests/server/runtime.router.test.ts docs/testing-log.md
...
```bash
git add src/server/runtime.ts tests/server/runtime.capture.test.ts tests/server/runtime.replay.test.ts docs/testing-log.md
...
- Create: `docs/state/2026-05-XX.md`

**Spec / inventory pointers:**
- `tests/server/m7Full.test.ts` — M7 integration smoke pattern; M8's smoke follows the same shape.
- `docs/state/2026-05-15.md` — M7 close-out snapshot; M8 supersedes.
...
In `docs/backlog/phase-16-rebuild-prereqs.md`:
...
In `docs/backlog/post-phase-13-4.md`, mark #30 closed with the T1 commit SHA.
...
```bash
mv docs/state/2026-05-15.md docs/state/archive/
```

Write new `docs/state/2026-05-XX.md` covering:
...
6. **24/24 prereq boxes:** After M8, every box in `docs/backlog/phase-16-rebuild-prereqs.md` is `[x]`. M9 visual polish + M10 parity audit are the gates to M11 default flip. ✓
...
Hardening findings → file as backlog items in `docs/backlog/post-phase-13-4.md` per the M7 hardening precedent.
