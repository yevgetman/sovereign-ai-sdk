...
Spec: `specs/2026-06-08-plugin-system-design.md` (§4 security model S1–S7, §5 staged scope, §11 founder decisions). Plan: `plans/2026-06-08-plugin-system-v1.md` (T1–T9). No new in-repo ADRs (built per ADR H-0010, canonical in `sovereign-ai-docs`); no bundle changes.
...
`bun run lint` clean (740 files) · `bun run typecheck` clean · `bun run test` **3555 pass / 0 fail / 16 skip** (clean `HARNESS_HOME`; 3571 across 390 files, ~61 s). Docs: `docs/03-cli-reference/usage.md` (`/plugins` section), `docs/02-architecture/runtime-architecture.md` (plugin layer + consent/integrity model), `docs/04-extending/extending.md` (authoring a plugin).

---

## Catch-up — the wave since Phase F (v0.6.23 → v0.6.34), un-snapshotted until now

These shipped between snapshots. Each has a spec under `specs/`; none has its own state snapshot (this is their record):
