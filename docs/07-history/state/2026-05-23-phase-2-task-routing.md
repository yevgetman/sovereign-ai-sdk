...
(From Phase 2 implementation. The Phase 2.5 / Phase 3 items below are documented in the spec at `specs/2026-05-23-multi-provider-task-routing-design.md`.)
...
8. **Append Phase 2 entry to `docs/06-testing/testing-log.md` (T11).** Followup task in this Phase 2 close-out chain.
9. **Cut v0.5.0 binary release with Phase 2 in it (T12).** Phase 2 is runtime-affecting (`src/`, `packages/tui/`); per `docs/05-conventions/cutting-releases.md` a release must be cut in the same session as the runtime changes so `~/.sov/bin/sov` picks them up.

## Postmortem-rule compliance check

The Phase 16.1 revert's Rules 1-4 (`docs/07-history/postmortems/2026-05-12-phase-16-revert.md`) apply primarily to foreground-surface refactors. Phase 2 is purely additive — no existing surface removed, no behavioral change to existing flows when `taskRouting.enabled: false` (the default) — so most rules don't engage:
