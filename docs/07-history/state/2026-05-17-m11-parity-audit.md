...
**Predecessor:** [`docs/07-history/state/2026-05-16-tui-parity-audit.md`](./2026-05-16-tui-parity-audit.md), [`docs/07-history/state/2026-05-16-m10-5.md`](./2026-05-16-m10-5.md)
...
- `0b528f3` — `README.md` + `docs/03-cli-reference/usage.md` doc updates
...
### `README.md` + `docs/03-cli-reference/usage.md` (doc updates)

`git diff d2de19b..HEAD -- README.md docs/03-cli-reference/usage.md` reviewed. The `--ui` flag description updated correctly:
...
- `docs/03-cli-reference/usage.md:78-80` — Section header updated with M11 default-flip note + alternative opt-out paths.
- `docs/03-cli-reference/usage.md:107` — bare `sov` entry table row updated to mention default `--ui tui` + fallback behavior.
...
- **Active docs (`specs/`, `plans/` from M11, `docs/07-history/state/2026-05-1[7-]`):** correctly updated.
- **Stale docs:**
  - **LOW finding — `docs/05-conventions/sov-upgrade.md:19`** still says "If Go is missing, the install succeeds and `sov --ui repl` (the default) still works". This is now inaccurate post-M11 (bare `sov` defaults to `--ui tui`; if `sov-tui` is missing, the new M11 auto-fallback fires). Recommend updating this convention file in the M11 close-out commit so newer sessions reading the convention don't get a stale picture.
  - Historical references in `plans/2026-05-1[3-6]-*.md`, `specs/2026-05-13-*.md`, `docs/06-testing/testing-log.md`, and `docs/07-history/state/archive/` correctly preserve the pre-M11 default in narrative form. These should NOT be retroactively edited — they're history.
...
| HIGH | 0 | — |
| MEDIUM | 0 | — |
| LOW | 1 | `docs/05-conventions/sov-upgrade.md:19` stale "`sov --ui repl` (the default)" reference + minor `sov chat --ui repl` references in `src/server/commandContext.ts:98,100` could mention bare-`sov` opt-out for clarity |
...
1. **Update `docs/05-conventions/sov-upgrade.md:19`** to reflect the M11 default-flip — change `"sov --ui repl" (the default)` to `"bare sov" (which defaults to --ui tui as of M11, with auto-fallback to readline REPL when sov-tui is missing)`.
