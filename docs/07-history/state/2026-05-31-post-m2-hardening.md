...
**Predecessor:** [`docs/07-history/state/2026-05-25-phase-21-m2.md`](2026-05-25-phase-21-m2.md) (Phase 21 M2 — release automation, v0.6.0).

## What this snapshot is

A **hardening / UX-polish run, not a new phase.** No phase opened or closed between Phase 21 M2 (2026-05-25, v0.6.0) and now. 57 commits and 13 patch releases (v0.6.1 → v0.6.13) shipped through the M2 CI tag-push pipeline — which doubled as ongoing validation that the automation works end-to-end. This file exists so `ls docs/07-history/state/*.md | sort -r | head -1` surfaces the real runtime tip (v0.6.13) rather than the v0.6.0 M2 snapshot.
...
- **TUI global style guide + routing-color fix (v0.6.3 → v0.6.4).** New `packages/tui/internal/style/style.go` (`style.S.*`) layer separating immutable layout/spacing/glyph/brand-color tokens from theme-switchable colors (`bf38e92`, v0.6.4) — now a HARD rule (see `docs/05-conventions/tui-style-guide.md`). The same morning replaced `t.Primary` "basic blue" with the fixed sky-300 `#7dd3fc` in delegator/routing output (`6bff455`, v0.6.3).
...
- **Visual TUI QA / VHS loop (v0.6.9 → v0.6.10) — new capability.** Agent-driven screenshot harness: `bun run visual [name]` drives `sov-tui` under VHS+ttyd and writes PNGs the agent can `Read`. Multi-screenshot scenarios (tool/edit/permission scenes), Terminal.app color calibration. Convention: `docs/05-conventions/visual-tui-qa.md`.
...
- No new ADRs — the run is fixes + a brand-color token change, all decisions captured in commit messages, the testing log, and `docs/05-conventions/tui-color-rendering.md`.
