...
Captured by `bun docs/07-history/state/2026-05-19-m12-smoke/run-smoke.ts`. Focused on the M12 predicate (ADR M12-01): the deprecation warning fires when the user explicitly opted into REPL via `--ui repl` / `SOV_UI=repl` / `ui.surface=repl`, but stays silent on missing-binary fallback or default-TUI.
