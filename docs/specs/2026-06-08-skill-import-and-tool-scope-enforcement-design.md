...
TDD; full gate (`bun run lint && bun run typecheck && bun run test`); update `docs/usage.md` (the `/skills import` verb + a "porting a Claude Code skill" note + the B enforcement behavior) and `docs/extending.md` (skill `allowedTools` is now enforced on the `/skill` path); append `docs/testing-log.md`; atomic commits (A separate from B); push; `sov upgrade`; cut a release.
