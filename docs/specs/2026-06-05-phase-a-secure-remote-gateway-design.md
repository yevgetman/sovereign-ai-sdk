...
**Parent roadmap:** `docs/specs/2026-06-05-run-anywhere-harness-roadmap-design.md` (Phase A / module M1 — the root unlock)
...
TDD throughout. Unit (auth, cors, refuse-boot, host bind) + integration (end-to-end authenticated turn via MockProvider). Full gate green. Update `docs/usage.md` (a "Remote gateway" section) + `docs/architecture.md` (the new surface). Commit/push; `sov upgrade`; cut a release (runtime change). Per `docs/conventions/autonomous-feature-builds.md`, this executes immediately into the build plan with no approval gate.
