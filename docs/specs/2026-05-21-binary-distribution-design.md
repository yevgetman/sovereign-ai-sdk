...
- One macOS install smoke test + one Linux install smoke test (manual; documented in `docs/testing-log.md`).
- New state snapshot `docs/state/YYYY-MM-DD-phase-21-m1.md`.
...
**Append to `docs/testing-log.md`** for each release attempt — pass/fail with notes.
...
**Spec:** `docs/specs/2026-05-21-binary-distribution-design.md` in the harness repo.
```
...
**Next step after this spec is approved:** invoke `superpowers:writing-plans` to produce the executable implementation plan at `docs/plans/2026-05-21-phase-21-m1-binary-distribution.md`. Plan execution happens when the author schedules it — likely after a small validation spike on `bun build --compile` with `bun:sqlite`.
