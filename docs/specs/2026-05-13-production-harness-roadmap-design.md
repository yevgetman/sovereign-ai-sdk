...
This spec is the umbrella roadmap. Per-phase implementation plans land in `docs/plans/YYYY-MM-DD-phase-NN-<feature>.md` when each phase is ready to execute, following the `superpowers:writing-plans` skill's TDD-task-by-task format.
...
The harness has shipped Phases 0–13.5 and Phase 16.0a (daemon skeleton, dormant). Phase 16.0b/c (Ink TUI + slash dispatch on Ink) were reverted on 2026-05-12 — see `docs/postmortems/2026-05-12-phase-16-revert.md`. Suite: 1809/1809 unit + 57/58 semantic. HEAD: `2ddf5fc`.
...
| 16.1 | Foreground TUI Rebuild | existing, reprio | ~400–600K (highest risk) | 24 prereqs in `docs/backlog/phase-16-rebuild-prereqs.md` | **ACTIVE — see linked spec & plan** |
...
**CLOSED 2026-05-13.** Decision: **Go + Bubble Tea (split-process architecture)**. See the 2026-05-13 split-process and Bubble Tea ADRs in `DECISIONS.md`, and `docs/specs/2026-05-13-phase-16-1-tui-rebuild-design.md`. The umbrella's prior claim that opencode uses OpenTUI/SolidJS was incorrect; opencode uses Bubble Tea.
...
Recommendation: **Option B (OpenTUI)**, with the four durable rules from `docs/postmortems/2026-05-12-phase-16-revert.md` enforced as plan-mode preconditions. Rationale:
...
- `docs/usage.md` — Providers section: expand from 4 to all supported; per-provider auth setup; example config.
- `docs/extending.md` — "Adding a provider" recipe updated to point at the wrapper pattern.
...
8. **Update docs.** `docs/usage.md` Providers section enumerates all ten with one-line auth setup each. `docs/extending.md` "Adding a provider" updated to the wrapper recipe (`@ai-sdk/<p>` install → `src/providers/aiSdk/<p>.ts` file → register).
...
- `docs/usage.md` — new "HTTP API" section.
- `docs/extending.md` — "Consuming the HTTP API" recipe.
...
11. **Documentation.** `docs/usage.md` adds "HTTP API" section with the curl examples above + endpoint reference. `docs/extending.md` adds "Consuming the HTTP API" recipe for an IDE-extension author.
...
- `docs/usage.md` — "MCP server mode" section.
- `docs/extending.md` — "Connect Claude Code / Cursor to Sov tools" recipe.
...
- `docs/usage.md` — "LSP tools" section.
- `docs/architecture.md` — LSP subsystem documented.
- `docs/extending.md` — "Adding LSP support for a new language" recipe.
...
9. **Documentation.** `docs/usage.md` LSP section with one example per tool. `docs/architecture.md` LSP subsystem diagram. `docs/extending.md` recipe for adding a new language.
...
- `docs/extending.md` — point to plugin-authoring guide.
...
4. Add an ADR-stub entry to `DECISIONS.md`: "Production polish track defined (Phases 15, 16.1, 18, 19, 20, 21). Phase 14 dropped per 2026-05-13 ADR. Hermes-layer extensions (16.5, 17, 22) deferred until polish track complete. See `docs/specs/2026-05-13-production-harness-roadmap-design.md`."

5. On the next state-snapshot session, the close-out snapshot at `docs/state/<YYYY-MM-DD>.md` references this spec as the active forward-looking plan.
...
- Each per-phase plan (saved to `docs/plans/YYYY-MM-DD-phase-NN-<feature>.md`) will fill in TDD task-decomposition with concrete code in every step per the writing-plans skill format. This spec is the umbrella; per-phase plans are the executable units.
