...
**✅ Shipped v0.6.20 (2026-06-06).** A real, polished **browser chat client** served by the gateway — a single self-contained page (`src/server/webui.html`, inline CSS + vanilla JS, no framework, no build) embedded into the binary (text-import) + served OPEN at `GET /` / `/ui` by `buildAppWithRuntime` (before the `/sessions/*` bearer-auth). It drives the same HTTP+SSE protocol as the TUI (token-prompt auth with the token client-side only; `?follow` fetch-stream; `Last-Event-ID` reconnect); renders streaming text, collapsible thinking, tool cards, inline permission Approve/Deny, auto-reconnect, new-chat, cancel. Same-origin → no CORS. Validated by a real-browser Playwright e2e (real model; tool-use → permission → Approve; reconnect — incl. fixing an SSE header-flush bug, `a77ed32`) and reviewed **SECURE + CORRECT** (XSS-clean, token-safe, reconnect correct). Two known-minor LOW follow-ups deferred (duplicate permission card on mid-turn reconnect-replay; SSE parser splits on LF). See `docs/specs/2026-06-05-phase-c-reference-web-ui-design.md` (spec, D1–D8), `docs/plans/2026-06-05-phase-c-reference-web-ui.md` (plan), and `docs/state/2026-06-06-phase-c-webui.md` (close-out).
