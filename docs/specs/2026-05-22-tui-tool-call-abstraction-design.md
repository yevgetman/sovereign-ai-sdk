...
After merge:
- Run `sov upgrade` to pick up the new binary (per the standing convention in `docs/conventions/sov-upgrade.md`).
...
The Phase 16.1 revert's Rules 1-4 (`docs/postmortems/2026-05-12-phase-16-revert.md`) apply primarily to foreground-surface refactors with active downstream consumers. This work is single-user UX polish, not a published-API change.
...
- `docs/conventions/tui-color-rendering.md` — body text inherits terminal default. The chevron + glyphs use accent colors per this rule.
- `docs/conventions/tui-ux-patterns.md` — flow layout, splash, spinner, tool events. Update after merge to reflect compact-mode tool rendering.
- `docs/state/2026-05-21-ux-fixes-r5.md` — round 5 architectural pivot (inline mode, no alt screen) that this work builds on.
- `docs/state/2026-05-22-phase-21-m1.md` — current canonical state snapshot.
