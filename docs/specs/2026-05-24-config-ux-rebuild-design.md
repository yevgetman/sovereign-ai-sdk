...
Per `docs/conventions/cutting-releases.md`, any change that touches `src/`, `bundle-default/`, or `packages/tui/` requires a same-session release cut so `~/.sov/bin/sov` picks up the changes. This work touches `src/` and `packages/tui/` — cut **v0.5.1** as the final task.
