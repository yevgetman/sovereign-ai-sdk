...
Supersedes: the M9 row in `docs/specs/2026-05-13-phase-16-1-tui-rebuild-design.md` §10 (refines that one-line scope into a per-task design)
...
| **T12** | Integration smoke + close-out | NEW `tests/server/m9Full.test.ts` (TS smoke for wire surface). NEW `packages/tui/internal/app/m9Full_test.go` (Go smoke for visible surfaces). Update `docs/state/2026-05-XX.md`; close ADRs M9-01 through M9-12; update CLAUDE.md/AGENTS.md pointers; close backlog #29 + #39. | TS + Go suites green; lint + typecheck clean; CLAUDE.md ≡ AGENTS.md byte-identical mirror. |
...
Per `docs/conventions/coding-style.md` (fail-explicit, no silent swallow):
...
| **`sov upgrade` after every `packages/tui/` change** | Per `docs/conventions/sov-upgrade.md`. | Final close-out `sov upgrade` verifies `bin/sov-tui` builds from postinstall. |
...
1. Write implementation plan at `docs/plans/2026-05-16-phase-16-1-m9-visual-polish.md` via `superpowers:writing-plans` skill.
2. Execute T1–T12.
