...
**Gate:** lint clean (765 files) + typecheck clean + **TS 4096 pass / 0 fail / 16 skip** (+100) + Go `build`/`vet`/`test` green. Behavioral smoke via the real dispatcher confirmed every scope's toast. Spec `docs/specs/2026-06-14-config-live-apply-design.md`; plan `docs/plans/2026-06-14-config-live-apply.md`; usage in `docs/usage.md` (the "Apply-scope" table).
...
Predecessor snapshot: `docs/state/2026-06-09-plugin-system-v1.md` (Plugin System v1). Find the latest via `ls docs/state/*.md | sort -r | head -1`.
