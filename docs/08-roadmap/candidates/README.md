...
|---|---|---|
| `docs/08-roadmap/candidates/` (this) | "We might build this. Here's the shape." | **None** — unscheduled |
| `specs/` | A design we intend to build, pending review | Pre-implementation |
| `plans/` | Phased, milestoned implementation work | Scheduled |
...
```
idea → candidate (here) → [greenlit] → specs/<dated>-design.md + plans/<dated>-<phase>.md
                        → [rejected]  → note the reason inline and leave it (or move to archive/)
```

A candidate graduating to real work gets a dated spec in `specs/` and milestones in
`plans/`; the candidate file then points at them and is left as provenance.
