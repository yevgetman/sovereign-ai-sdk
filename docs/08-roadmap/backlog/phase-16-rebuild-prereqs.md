...
> **Status (2026-05-16):** **24/24 prereq boxes complete; M10 audit-shipped; M10.5 dispatcher closed the audit's HIGH gap.** M4 closed 3 boxes, M5 closed 3, M6 closed 3, M7 closed 6, M8 closed the remaining 9. M9.x track shipped (M9 → M9.5 → M9.6). M10 parity audit (2026-05-16) independently verified subsystem wiring; surfaced 4 HIGH gaps (2 fixed inline, 1 scope-bounded mission-CLI-only, 1 deferred to #40). **M10.5 (2026-05-16) closed #40 via the new `POST /sessions/:id/commands` route** — bridges the existing `src/commands/registry.ts` into server-mode via a per-request CommandContext factory. **M11 is now unblocked.** Next milestone is M11 (default flip — `--ui tui` becomes default).

> **Authoritative phase plan:** `specs/2026-05-13-phase-16-1-tui-rebuild-design.md` §9. This backlog enumerates the 24 subsystems any new foreground must wire; the spec assigns them to milestones M4–M8.

When Phase 16 is retried (as Phase 16.1 per the open-question resolution in `docs/07-history/state/archive/2026-05-12.md`), it must follow Rule 1 of `docs/07-history/postmortems/2026-05-12-phase-16-revert.md`: the new foreground surface ships alongside the existing one (`src/ui/terminalRepl.ts`) behind an opt-in flag, not as a replacement. This file enumerates the subsystems any new foreground surface must wire — captured here so the next attempt doesn't re-discover them through a close-out audit.
...
| 14 | `[x]` (M8 — 2026-05-16) | **Local-model router** | Phase 10.6 — `RouterProvider` dispatches to cheap models when a router config is present; `RouterAuditLogger` records decisions | `src/router/provider.ts`, `src/router/auditLogger.ts` |
...
| 16 | `[x]` (M8 — 2026-05-16) | **Capture / replay for eval runner** | `--capture-fixture` writes a fixture; `--replay-fixture` reads it back without LLM calls | `src/eval/replay/capture.ts`, `src/eval/replay/loader.ts`, `src/eval/replay/provider.ts`, `src/eval/replay/toolPool.ts` |
| 17 | `[x]` (M8 — 2026-05-16) | **`@file:path` reference expansion** | Inline file references in user prompts get expanded to the file's contents | `src/context/references.ts` |
| 18 | `[x]` (M8 — 2026-05-16) | **Subdirectory hint state** | `CLAUDE.md` awareness from subdirectories — agent picks up CLAUDE.md files in cwd's parent chain | `src/context/subdirectoryHints.ts` |
| 19 | `[x]` (M8 — 2026-05-16) | **Skill-as-slash-command** | Skills with `whenToUse` triggers register as `/skillname` slash commands via `buildSkillCommands` | `src/skills/commands.ts` |
| 20 | `[x]` (M8 — 2026-05-16) | **Skill visibility filtering** | Per-turn `filterSkillRegistry` narrows the visible skill set to the active toolsets | `src/skills/visibility.ts` |
| 21 | `[x]` (M8 — 2026-05-16) | **Goodbye summary** | Session metrics block (cost, tokens, duration, turn count) renders on REPL exit | `src/ui/sessionSummary.ts` |
| 22 | `[x]` (M8 — 2026-05-16) | **Stall / no-op detection** | Phase 13.3 — sliding-window emits `stall_detected` trace events when the model loops or makes no progress | `src/review/stall.ts` |
...
| 24 | `[x]` (M8 — 2026-05-16) | **Tool-result expand registry** | `/expand [N]` re-renders the Nth-most-recent tool block with no truncation | `commandContext.expandToolBlock` wiring |
...
- `docs/07-history/postmortems/2026-05-12-phase-16-revert.md` — Rules 1-4 governing this work.
- `docs/07-history/state/archive/2026-05-12.md` — Phase 16.0a / revert close-out snapshot (historical).
