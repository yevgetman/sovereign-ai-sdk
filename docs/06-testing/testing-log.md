...
**Live conformance probe.** `tests/providers/router.live.test.ts` (new) — gated OFF unless BOTH `MANIFEST_LIVE=1` and `MANIFEST_API_KEY` are set (the sov.live.test.ts pattern; optional `MANIFEST_BASE_URL` override, default `http://localhost:2099/v1`). One real `model: "auto"` turn: asserts assistant text; asserts the `ResolvedRoute` shape only when route headers were observed (a proxy may strip them — logged either way). Run: `MANIFEST_LIVE=1 MANIFEST_API_KEY=mnfst_... bun test tests/providers/router.live.test.ts`. Verified it **skips cleanly** in the gate (and with `MANIFEST_LIVE=1` but no key).

**Live conformance — RUN AND PASSED (2026-07-06, post-merge, CEO-directed).** Manifest v6.13.5 installed self-hosted on this machine (Docker compose at `~/manifest`, loopback `127.0.0.1:2099` only, `restart: unless-stopped` added; admin + agent key minted via the dashboard, key in the login Keychain as `MANIFEST_API_KEY`; Anthropic connected as the sole usage-based provider; Default tier = Claude Sonnet 5 with Claude Haiku 4.5 fallback). Two live probe runs, both **1 pass / 0 fail**: (1) before routing was configured — turn completed with `route: {model: "manifest", provider: "manifest", tier: "simple", reason: "no_provider"}` (Manifest answered from its built-in fallback; proves the header parse + the don't-hard-require-headers posture); (2) after assigning the Default tier — `route: {model: "claude-sonnet-5", provider: "anthropic", tier: "default", reason: "default"}` — a real `"auto"` request routed to a real upstream with the routed model reported through `onRouteResolved`. End-to-end conformance of SOV wire ↔ Manifest v6.13.5 confirmed. (UI note for future maintainers: Manifest's dashboard now calls the keyed entity a **harness**, not an "agent" as the API docs say — keys are minted at harness creation; per-harness provider enablement + tier routing live under Harness → Providers/Routing, and an unrouted harness falls back rather than erroring.)
...
**Scope.** Build a deterministic multi-agent orchestration layer into the harness — the gap identified in the prior turn (model-driven fan-out only; a single global write-lock serialized write-capable fan-out). Founder-locked design: a DECLARATIVE engine (no arbitrary code exec) + path-granular locking, with **parallel fan-out as the headline**. Spec `specs/2026-06-15-multi-agent-workflows-design.md`, plan `plans/2026-06-15-multi-agent-workflows.md`. Built W1 (the concurrency core) by hand; W2–W6 via a 5-owner disjoint-file workflow (TDD + per-owner review); seams reconciled centrally.
...
**Scope.** Resolve the `/config` "unclear whether a saved setting applies now or needs a restart" UX gap. Replaced the binary hook-presence model (badge + toast derived independently, could disagree; many settings silently fell to "next session") with a single **apply-scope taxonomy** (`src/config/applyScope.ts`: `live` / `live-reload` / `other-process` / `restart`), maximized the live-apply set, and made the save confirmation always name the setting + state the exact outcome. Spec `specs/2026-06-14-config-live-apply-design.md`, plan `plans/2026-06-14-config-live-apply.md`. Built T1 (foundation) by hand; T2–T7 via a 5-owner disjoint-file workflow (TDD + per-owner review + a safety review of the provider re-resolution); integration seams + the safety-review HIGH reconciled centrally.
...
**Scope:** whole-repo audit (21 subsystem auditors + holistic data-leakage / lifecycle / consistency passes, all Opus) → triage → fixes across every region. Report: `docs/07-history/audits/2026-06-10-full-codebase-audit.md`.
...
**Scope.** Slice E / Task T8 (the closing slice) for the `/effort` reasoning-depth feature: documentation, a semantic test suite, and this log entry. No feature code touched — Slices A–D shipped the implementation. Spec `specs/2026-06-09-effort-reasoning-depth-design.md`.
...
**What I wrote.** `docs/03-cli-reference/usage.md` (Config-category command row + a `/effort — reasoning depth` subsection with the full level→provider mapping table + the `thinking.effort` config-fields-table row); `docs/02-architecture/runtime-architecture.md` (a Core-Contracts note on the provider-neutral `effort` knob + `runtime.effort`, linking the spec); `docs/04-extending/extending.md` ("Add A Provider" — a new reasoning-capable provider forks `req.effort` via `effort.ts`); `tests/semantic/suites/25-effort.cases.ts` (NEXT free suite number after 24-learning-recall; auto-discovered by `loadTestsFromDir` — no central registry).
...
**Scope.** Shipped **Plugin System v1** (the recommended v1 from the spec — founder-approved scope): the plugin *foundation* (manifest, loader, composition seams, consent + integrity infrastructure) plus the only extension types safe today — **skills + slash-commands** — installed as one consent-gated, integrity-hashed unit. CC-format skill/command packs install; a richer CC plugin's hooks/MCP/agents are **disclosed but inert** (deferred to v2/v3). Spec `specs/2026-06-08-plugin-system-design.md` (§5 v1 scope, §4 security model S1–S7); plan `plans/2026-06-08-plugin-system-v1.md` (T1–T9).

**Process.** Built **subagent-driven** (`superpowers:subagent-driven-development`): a fresh **Opus** implementer per task + **two-stage review** (spec-compliance, then code-quality) per task, TDD RED→GREEN throughout. The three load-bearing security tasks (T3 consent gate, T5 inline-shell-disable, T6 install-consent) each got an **additional dedicated adversarial security review**, and the integrated feature got a **whole-feature adversarial review** at T9.

**Prerequisite — backlog #55 fixed first (`79a1b7f`), to clean the gate.** Root cause was bigger than scoped: `buildRuntime` accepted `harnessHome` but did NOT thread it into `SessionDb.open` (so it opened the global 1.18 GB `~/.harness/sessions.db` → contention → turn timeout → the learning observer never drained → the #55 false failures) nor into `readConfig`. Fixed by threading `harnessHome` through both (backward-compatible `readConfig` overload). The previously-documented "3 known env-only learning-test fails" are now resolved; the named #55 tests pass on a deliberately-polluted `~/.harness`.

**What shipped (T1–T9).** New `src/plugins/` module: `types.ts`/`manifest.ts` (strict known-subset Zod, unknown CC keys → `ignored[]`) + `secretScan.ts` (best-effort literal-secret detector, field-targeted) [T1]; `integrity.ts` (deterministic NUL-separated, path-sorted SHA-256 tree-hash excluding `.consent.json`) + `consent.ts` (record + atomic write + strict `verifyConsent`) [T2]; `loader.ts` (the load-time consent gate — a plugin contributes NOTHING without a valid, hash-matching consent record; default-deny) [T3]; `compose.ts` (active plugins → `skillRoots` + `commands` + inert disclosures; M1 path-containment; `${CLAUDE_PLUGIN_ROOT}`) [T4]; the skills-loader edit disabling inline-shell for `source:'plugin'` skills + `extraRoots` precedence (`project > user > plugin > bundle`) [T5]; `install.ts`/`disclosure.ts` (TTY disclose-and-consent — the only consent minter; secret-scan + path-containment + symlink-reject + guard-scan all run BEFORE the prompt) [T6]; `pluginOps.ts` (`/plugins install|uninstall|enable|disable|list|info`) + opt-in `plugins` config + H3 hook/mcp merge extraction [T7]; runtime wiring (`loadPluginRuntime` into `buildRuntime` + `dispatchCommand`, HarnessInfo surfacing, single-readline TTY confirm) [T8]; `pathContainment.ts` consolidation + manifest drift-guard [T9].

**Notable review catches (found + fixed before ship).** T3 review found a **symlink-to-directory EISDIR DoS** that would crash the whole `loadPlugins` scan (boot-time DoS once wired) — fixed (`c873f68`: walk skips non-regular entries + a per-plugin gate try/catch). T5 proved **C2** RED→GREEN: a plugin skill's `` `!echo PWNED` `` *executed* on the pre-fix code and is inert after. T6 review found a **disclosure-fidelity gap** (install scanned `.md` bodies independently while the loader aggregates a directory-skill's reference files — a malicious `payload.txt` would be invisible at consent but block at load) — fixed (`8aa437a`: install now uses the loader's aggregating `guardSkillLoad`, parity exact).

**Whole-feature adversarial security review — VERDICT: SECURE-TO-SHIP.** No Critical/High. Every cross-task / emergent attack is blocked in the *integrated, wired* system (not just in isolation): drop→load→invoke (unconsented contributes nothing at the wiring layer), post-consent tamper (re-verified at every boot), cross-seam shell laundering (single gated sink; `source:'plugin'` forced + double-gated), command-precedence shadowing (built-ins win; plugin can't shadow user/project), consent minting (`installPlugin` the sole `writeConsent` caller; TTY-only confirm unreachable from server/gateway/channels), multi-user/channel leakage (plugins are an operator-shared install surface, no per-user namespace to cross, no remote install path), and DoS (symlink-cycle-safe walks, per-plugin fail-soft). The two known accepted-risk LOWs remain bounded — notably the lone-symlink-add-post-consent gap is *narrower* than documented: a symlink is invisible to BOTH the tree-hash AND the skill enumeration, so it never loads.

**Gate (final, clean `HARNESS_HOME`).** `bun run lint` clean (740 files) · `bun run typecheck` (`tsc --noEmit`) clean · `bun run test` **3555 pass / 0 fail / 16 skip** (3571 across 390 files, ~61 s). The `[WARN] blocked context file AGENTS.md … curl|bash` lines are pre-existing context-scanner noise about the repo's documented installer, not failures. New plugin coverage spans manifest/secretScan/integrity/consent/loader/compose/install/disclosure/snapshot/runtime/pathContainment unit suites + the inline-shell-disable security tests + the e2e consented-skill-through-the-live-server test + the dispatch subprocess tests.

**Commits.** `79a1b7f` (#55) then the T1–T9 chain `85ff11e … 76171d9` (15 commits total ahead of the prior `origin/master`, plus `fd6a096`, an unrelated parallel `sov`-provider T4-live gated test that landed on master during the build — gated/skipped, `tests/providers/` only, no conflict). No new ADRs, no bundle changes (built per ADR H-0010, canonical in `sovereign-ai-docs`). **Release v0.6.35** cut this session.
...
`specs/2026-06-08-sov-provider-design.md`, `plans/2026-06-08-sov-provider-bucket-a.md`.
...
**Diagnosis (empirical).** Reproduced headlessly in `sov drive` (same server path as the TUI, plain-text events) — confirms **server-side, not TUI-specific**: one `hi` → `322` `turn_complete` markers in ~12s. 322 turns in 12s ≈ 27/s is **impossible for real model calls** → it's a **replay loop**, not regeneration. Root cause chain: the per-turn contract closes the SSE stream on `turn_complete`; the TUI (`app.go`) and `sov drive` both **reconnect WITHOUT a `Last-Event-ID`**; the bus treats a no-cursor subscriber as fresh and replays `seq >= currentTurnStartSeq` (the documented POST-then-GET no-race current-turn replay) — but between turns `currentTurnStartSeq` still points at the just-completed turn (only the next `markTurnStart` overwrites it; `turnActive` flips false but the cursor doesn't move). So the fresh reconnect re-receives the WHOLE completed turn including its `turn_complete`, the stream ends on that replayed terminal, the client reconnects → re-receives it → ∞. The **web UI never looped** because it already sends `Last-Event-ID`.

**Fix decision — client-side, not server-side.** First attempt gated the bus's fresh-replay on `turnActive` (server-side); it broke **14 tests** because the no-cursor current-turn replay is a **load-bearing no-race guarantee** (MockProvider's instant turns make the POST-then-GET `/events` race past completion and rely on the replay — `turns.learning`, multi-user isolation, M7/M8 integration, etc.). Reverted. The correct fix is the standard SSE mechanism the server already supports (and the web UI already uses): **clients send `Last-Event-ID` on reconnect** so a post-turn reconnect replays only `seq > cursor` (nothing) and ends cleanly. **Server unchanged.**

**Change.** `src/cli/driveCommand.ts` — a shared `sseCursor`, the `Last-Event-ID` header in `drainSseStream`, advanced to each event's seq, reset on the compaction session-pivot. `packages/tui` — `transport.Consume(ctx, url, lastEventID)` sends the header when `> 0`; `app.go` tracks `m.sseCursor` in the `sseMsg` handler and threads it through `startSSE`, resetting on a session pivot (a new bus has its own seq space). New Go transport test `TestConsume_sendsLastEventIDHeaderOnReconnect` (header present on reconnect / absent on first connect; passes under `-race`).

**Verify.** `sov drive` `hi` → **1** `turn_complete` (was 322): local source (`bun src/main.ts drive`), and the **shipped installed** `sov drive` after upgrade. The Go fix is compiled into the source-mode `sov-tui` (postinstall `scripts/build-tui.ts` `go build`; `strings bin/sov-tui | grep Last-Event-ID` = 1) AND cross-compiled into the binary release. `go build ./...` clean; `go test ./internal/transport/ -race` green.
...
Operator decision: flip the subscription executor's default `permissionMode` from `plan` to **`bypass`** (→ `--dangerously-skip-permissions`). Rationale: a headless `claude -p` has **no interactive approver**, so under the old `plan` default any tool the subprocess needed permission for would auto-deny and stall — the executor was largely inert for real agentic work. With bypass it can actually do delegated work; `plan` | `acceptEdits` | `default` remain available for a constrained posture.

**Change set.** `src/config/schema.ts` — `permissionMode` enum gains `'bypass'` (the Claude-CLI spelling `bypassPermissions` is still NOT a valid token — the harness token is `bypass`); the security comment rewritten. `src/runtime/subprocessExecutor.ts` — `DEFAULT_PERMISSION_MODE` → `'bypass'`; `buildSubprocessArgs` now branches: `bypass` emits `--dangerously-skip-permissions` (and NO `--permission-mode`), the three constrained modes emit `--permission-mode <mode>`. `src/config/catalog.ts` — permissionMode choices lead with `bypass`; the `enabled` + `permissionMode` descriptions updated. **Safety bounding preserved:** the executor is reachable ONLY from the attended, interactive sub-agent seam — NOT cron/channels/gateway — and those remote surfaces keep their OWN bypass rejection (`src/channels/permission.ts`, untouched). So the bypass is the operator delegating to their own logged-in Claude Code, not a remote/automated RCE path. Still off by default (`enabled:false`).

**Tests (updated, RED-aware).** `tests/config/subscriptionExecutor.test.ts` — accepts all four modes incl. `bypass`; still rejects unknown tokens incl. `bypassPermissions`. `tests/runtime/subprocessExecutor.test.ts` — new posture block: unset defaults to `--dangerously-skip-permissions`; constrained modes emit `--permission-mode` and NO dangerous flag; `bypass` emits the dangerous flag and omits `--permission-mode`. `tests/config/catalog.test.ts` — permissionMode choices assert `['bypass','plan','acceptEdits','default']` with `bypass` leading. Docs: `docs/03-cli-reference/usage.md` (config example + permissionMode bullet + ToS-boundary note), the spike spec got a dated UPDATE note (default flipped; original design rationale preserved), and the CLAUDE.md/AGENTS.md index line corrected (was "bypass rejected").

**Concurrency.** A concurrent session was actively pushing to `master` throughout — rebased my 2 commits **twice** (onto a 7-commit skills review-batch `20cc30d…6afd095`, then a further `3ab0800` skills fix + `676f36a` MCP docs), re-running the FULL gate after each rebase. Per `cutting-releases.md`, v0.6.32 sweeps up those unreleased skills/MCP fixes alongside the executor change — all enumerated in the bump-commit body + the public CHANGELOG.
...
## 2026-06-08 — Skills review-cluster fixes (F1/F2/F9/F10/F14), TDD RED→GREEN

Fixed a confirmed cluster of code-review findings in the skills subsystem, each test-first (RED proving the bug → fix → GREEN). No release cut (the controller verifies + releases).

- **F1 — `splitCommaList` shredded Bash() patterns with commas** (`src/skills/frontmatter.ts`). A naive `split(',')` broke `Read, Bash(git log --pretty=format:%h,%an)` into unparseable fragments that later throw "missing closing ')'" in `parsePermissionRule`, failing the whole /skill turn. Made it paren/bracket-DEPTH aware (split only at depth 0). RED: 4/7 new `tests/skills/frontmatter.test.ts` cases failed (nesting cases); GREEN: 7/7.
- **F2 — a malformed skill `allowedTools` entry crashed the /skill turn** (`src/server/routes/turns.ts`). `buildToolScope` maps each entry through `parsePermissionRule`; one unparseable entry threw → caught by `runTurnInBackground` → `turn_error`. Added `filterParseableRules(entries, warn)` (`src/commands/toolScope.ts`) applied on the skill path ONLY (agent path unchanged); drops invalid entries with a warning (fail-closed). RED: the new skillScope test got `turn_error: invalid permission rule "Bash(git log"`; GREEN: turn completes, narrows to `FileRead`, malformed entry dropped (warned to stderr). +6 `filterParseableRules` unit cases in `tests/commands/toolScope.test.ts`.
- **F9 — import path duplicated the alias normalization** (`src/skills/install.ts` + `src/skills/loader.ts`). Exported `normalizeFrontmatterAliases` from the loader and had `normalizeImportedFrontmatter` delegate the `allowed-tools`→`allowedTools` alias + comma-split to it (single source), keeping only import-specific concerns (converted/warnings, ignored-key drop, :-glob warning, whenToUse synth) on top. `converted`/`warnings` output byte-identical (pinned by `import.test.ts`, still 29 pass). RED: new `tests/skills/normalizeAliases.test.ts` failed to import (symbols not exported + not single-sourced); GREEN: 4/4 (both surfaces produce identical `allowedTools` incl. the F1 comma-in-parens case).
- **F10 — install dup-check regex mis-read quoted/commented names** (`src/skills/install.ts`). Replaced the bespoke line-regex name-scrape with the real `yaml` parser (already imported) so install's dup-check + landed dir match the loader's registered name. RED: `name: commented-name # comment` → regex scraped the literal `commented-name # comment` (invalid name) → install wrongly refused; GREEN: lands under `commented-name`, loader registers the same. +3 cases in `install.test.ts`.
- **F14 — skillScope tests asserted too little** (`tests/server/skillScope.test.ts`, test-only). (a) The deny test now parses the `tool_result` OUTPUT (the proposed input is always echoed on the wire, so a raw-stream scan is wrong) and asserts the denied command's stdout marker is ABSENT while the deny reason IS the output. (b) The narrowing/full-pool tests now assert `turn_complete` is in the drained stream AND `MockProvider.lastTools` is defined before asserting the set (so a no-op turn fails instead of coalescing `?? []` to `[]`). **Both guards verified to FAIL against the regressions they protect** (temporary mutations, then reverted: deny→allow made the marker appear in tool_result output; `lastTools = undefined` made `toBeDefined()` fail) and PASS against current correct behavior.

**Gate (post-rebase onto `origin/master` @ e108553).** `bun run lint` clean (biome, 705 files) + `bun run typecheck` clean (`tsc --noEmit`) + `bun run test` **3312 pass / 0 fail / 14 skip** (9268 expect() calls, 3326 tests across 370 files, ~110 s) — pre-rebase full run; the rebase was conflict-free against disjoint upstream (config catalog + v0.6.31 release), and the 9 touched test files re-ran post-rebase at **101 pass / 0 fail**. No `console.log`; no new `process.env` mutation in tests; `CLAUDE.md ≡ AGENTS.md` (untouched). The `[skill:malformed] dropping unparseable rule` line is the F2 warning under test (expected), not a failure. Pre-existing AGENTS.md guard-scanner `[WARN]` lines (installer `curl | bash` doc snippet) unrelated.

**Commits (atomic, per F-group; pushed `origin/master`).** `20cc30d` F1 · `057c5bd` F9 · `a92f688` F10 · `06dea79` F2 · `9e3485a` F14 · `2c042e0` F9-lint-wrap. (Original pre-rebase SHAs: b48d3a8/3eeb037/7d7ad48/20acaf6/7ef09c7/0336371.)



Closed a catalog gap: the `subscriptionExecutor` block (the opt-in headless Claude Code sub-agent executor, v0.6.27) was in `SettingsSchema` + `docs/03-cli-reference/usage.md` but never added to the curated config catalog (`src/config/catalog.ts`), so it was editable ONLY by hand-editing `~/.harness/config.json` — it never appeared in `/config` or `sov config`. Added a **Subscription executor** group (placed after Task routing, its mutually-exclusive sibling) covering all six fields (enabled / engine / binary / permissionMode / timeoutMs / maxTurns).

**Design call — next-session semantics (no live-apply hook).** The scheduler captures `subscriptionExecutor` config at construction (`src/runtime/scheduler.ts:349` reads `this.opts.subscriptionExecutor`) and never refreshes it; `rebuildTaskRouting` re-reads `subscriptionExecutor.enabled` ONLY for the tool-pool AgentTool enum (role visibility), not the routing branch. A live-apply hook would therefore be half-applied (enum flips, routing stays stale), so every field is intentionally left out of `LIVE_APPLY_HOOKS` → automatic `⟳ next session` badge. Matches the router/learning/gateway groups. permissionMode uses Claude Code's own `plan|acceptEdits|default` vocabulary (a distinct choice list from the top-level `default|ask|bypass`; `bypass*` deliberately absent — the schema rejects it). No Go TUI change needed (the catalog is data-driven; the dispatcher emits pickerOpen items).

**Tests.** New `subscription-executor` describe block in `tests/config/catalog.test.ts` (4 cases: root-menu placement immediately after task-routing; all six paths present + owned by the group; editor shapes incl. the distinct permissionMode enum with a `bypass`-absent assertion; every field has NO live-apply hook). The pre-existing `every catalog path is a valid SettingsSchema dotpath` test auto-validates the new paths against the schema (it already unwraps the `subscriptionExecutor × taskRouting` superRefine ZodEffects). Added a semantic case to `tests/semantic/suites/23-config-ux.cases.ts` (`config-subscription-executor-submenu-reload-badge`). `docs/03-cli-reference/usage.md` "Enable it" now points at the TUI path first.

**Concurrency note.** On push, `origin/master` had advanced by **5 unreleased MCP review-fix commits** (`2222ff8` F3 → `08b1535` F7/F8/F12) with the remote version still at 0.6.30 (no bump). Rebased my 2 commits on top (clean), **re-ran the full gate on the combined tree**, and per `cutting-releases.md` ("body lists every behavioral change since the last release tag") v0.6.31 sweeps up those MCP fixes alongside the catalog change — both enumerated in the bump-commit body + the public CHANGELOG.
...
## 2026-06-08 — RELEASE v0.6.30: remote MCP transport (HTTP/SSE) + cross-origin redirect header-stripping

Cut **v0.6.30** off `master` — the remote-MCP-transport increment since v0.6.29 (`a25f7d7` config union → `b9c9f52` HTTP/SSE client transports + env-first auth → `c5f9338` docs → `7aabba8` strip auth on cross-origin redirect → `f30c074` reject MCP alias env-var collisions → `dcd9600` redirect-stripping docs). Followed `docs/05-conventions/cutting-releases.md` (tag-driven CI). Only `package.json` carries the version stamp (build-time JSON import via `src/version.ts`); the CLAUDE.md/AGENTS.md "shipped" lines + ACTIVE FOCUS banner are historical doc references, left untouched.

**Gate (pre-bump-commit).** `bun run lint` clean (biome, 702 files, no fixes) + `bun run typecheck` clean (`tsc --noEmit`) + `bun run test` **3270 pass / 0 fail / 14 skip** (9156 expect() calls, 3284 tests across 367 files, ~109 s). Matches the expected baseline exactly — no new failures, no regressions; the known MockProvider/telegram env-flakes did not surface this run (fully green). The pre-existing AGENTS.md guard-scanner `[WARN]` lines (the installer `curl | bash` doc snippet) are unrelated.
...
## 2026-06-08 — Remote MCP transport — review fixes (cross-origin secret leak, SSE coverage, env purity, alias collision, SSRF heuristic)

Applied two reviews' findings (spec+quality, adversarial security) on the just-landed remote MCP transport (`a25f7d7`+`b9c9f52`+`c5f9338`): **2 HIGH + 1 MEDIUM + 2 LOW**. Built RED→GREEN where behavior is testable; **no release** (the controller reviews + releases). Committed atomically; pushed `origin/master`.

**HIGH-1 (security) — cross-origin redirect leaked `X-API-Key` + custom headers.** Bun's fetch strips `Authorization` on a cross-origin redirect but NOT `X-API-Key` or operator `headers`, so a malicious/open-redirecting configured server could exfiltrate them. Fix: new `src/mcp/safeFetch.ts` — `buildSafeFetch(configuredUrl, attachedHeaderNames, fetchImpl?)` does **manual redirect handling** (`redirect:'manual'`, cap 5, modeled on `WebFetchTool`'s per-hop loop) and, on **every hop whose target origin differs from the original configured server origin**, strips the FULL resolved header set we attached (`Authorization` + `X-API-Key` + every operator custom header name) before issuing the next request; same-origin (incl. same host:port) keeps them. Wired into BOTH transports — `StreamableHTTPClientTransport`'s top-level `fetch` and the SSE `eventSourceInit.fetch` (GET stream) **and** the SSE POST `fetch`. **Proof:** new `tests/mcp/fixtures/redirect-server.ts` (two real `Bun.serve` listeners on distinct ports → distinct origins; the "configured" one 307s every request to the "attacker" one, which records inbound headers) + two `remoteClient.test.ts` tests (HTTP + SSE) asserting the attacker receives NONE of `Authorization`/`X-API-Key`/`X-Tenant`, with a `attackerHits.length > 0` guard proving the redirect path actually ran. **Verified the tests catch the vuln:** temporarily neutering `isCrossOrigin` → both tests RED with the attacker capturing `Bearer secret-bearer`; restored → GREEN. Plus 6 fast unit tests in `tests/mcp/safeFetch.test.ts` (same-origin keeps headers; cross-origin strips all; different-port = cross-origin; no re-add after a bounce back to origin; redirect-cap throw; `redirect:manual` asserted).

**HIGH-2 — SSE GET-stream Headers merge bug dropped the SDK's `Accept`/`mcp-protocol-version`.** `{ ...(init.headers as Record) }` yields `{}` because `init.headers` is a `Headers` instance (entries aren't own-enumerable), clobbering `Accept: text/event-stream`. Fix: merge via `new Headers(init.headers)` then `.set()` our resolved headers on top, composed with the HIGH-1 `safeFetch`. **HIGH-3 — SSE had zero runtime coverage (why HIGH-2 shipped undetected).** **SSE was FIXED, not deferred.** New `tests/mcp/fixtures/sse-echo-server.ts` — a faithful in-process SSE MCP server over a real `node:http` listener driven by the SDK's own `SSEServerTransport` (the SDK's SSE server is `node:http`-shaped, not `Bun.serve`/Web-standard, so `node:http` — which Bun supports — is the faithful route; no hand-rolled JSON-RPC). New `tests/mcp/sseClient.test.ts` (3): connect+list+call+isError round-trip; **the regression guard** asserting the GET stream received BOTH `Accept: text/event-stream` AND the resolved `Authorization: Bearer …`; env-token reaches the GET stream. **Verified the guard catches the bug:** temporarily reverting to the buggy spread-merge → the guard RED (`Accept` observed as `*/*`, which is exactly why the connect still "worked" enough to ship undetected); restored → GREEN.

**MEDIUM — env purity.** `buildRemoteTransport` read `process.env` directly. Threaded an optional `env` through `BuildMcpClientPoolOpts → connectOne → buildTransport → buildRemoteTransport`, defaulting to `process.env` at the boundary (the `runtime.ts` pool-build call site now passes `env: process.env` explicitly). The env-override `remoteClient.test.ts` test now injects `env: { SOV_MCP_REMOTE_TOKEN }` (no more `process.env` save/mutate/restore); the config-bearer test passes `env: {}` so an ambient var can't pollute it. **No `process.env` mutation remains in the MCP tests.**

**LOW-1 — alias→env-var collision.** `foo-bar` and `foo_bar` both normalize to `SOV_MCP_FOO_BAR_TOKEN`, so one server's token could be applied to another host. `loadMcpServerSettings` (`src/config/settings.ts`) now tracks the normalized fragment per alias and throws (mirroring the existing dup-alias throw) when two distinct aliases collide. Test in `tests/config/settings.test.ts` (RED→GREEN). **LOW-2 — `isPrivateHost` SSRF-warning gaps.** Extended the warn-only heuristic (now exported for test) to catch IPv6 loopback `::1`, unspecified `::`/`[::]`, ULA `fc00::/7`, link-local `fe80::/10`, and IPv4-mapped `::ffff:a.b.c.d` (recursing on the embedded IPv4). Non-dotted IPv4 encodings (decimal/octal/hex) left out with a code comment noting the dotted-quad scope (warn-only, kept cheap). 3 `client.test.ts` tests (IPv4 set / IPv6 set / public-not-flagged incl. public IPv6 + `::ffff:8.8.8.8`).

**Docs.** `docs/03-cli-reference/usage.md` MCP security notes now document the redirect behavior (auth headers dropped on a cross-origin hop; configure the canonical same-origin endpoint; prefer `bearerToken`/`Authorization` / `SOV_MCP_<ALIAS>_TOKEN`).

**Gate.** `bun run lint` clean (biome; `bun run format` reflowed 2 test files first) + `bun run typecheck` clean (`tsc --noEmit`; `HeadersInit`/`RequestRedirect` aren't global under `lib:ESNext`+bun-types — used `new Headers(init.headers)` directly and `string` for the redirect assertion) + `bun run test` **3270 pass / 0 fail / 14 skip** (9156 expect() calls, 3284 tests across 367 files, ~107 s) — baseline ~3255 + 15 net-new test cases, no new failures, no regressions; the known MockProvider/telegram env-flakes did not surface (fully green). No `console.log` added. CLAUDE.md ≡ AGENTS.md unchanged (verified identical; not touched). The pre-existing AGENTS.md guard-scanner `[WARN]` lines (installer `curl | bash` doc snippet) are unrelated. **Not exercised:** the spec's manual smoke against a public Streamable HTTP server (deferred to the controller's pre-release pass, kept out of CI by design).

## 2026-06-08 — Remote MCP transport (HTTP / SSE) — TDD

Arc C of the ecosystem-openness work, per `specs/2026-06-08-remote-mcp-transport-design.md`. Opens the harness to remote/hosted MCP servers (Streamable HTTP + legacy SSE) alongside the existing stdio servers. **No release** (the controller reviews + releases). Built RED→GREEN; committed atomically; pushed `origin/master`.

**What shipped (all decisions per the spec).** Config `McpServerConfigSchema` (`src/config/settings.ts`) became a backward-compatible `z.union` of three `.strict()` variants (http, sse, stdio — type-required variants first, permissive stdio with `type.default('stdio')` last) fronted by a `z.preprocess` that emits a friendly "set `type:'http'` or `type:'sse'`" error when a config has `url` but no transport. `src/mcp/types.ts` mirrors the union (discriminated `McpStdioServerConfig | McpHttpServerConfig | McpSseServerConfig`); stale "deferred" comment fixed. New `src/mcp/auth.ts` — `resolveMcpHeaders(alias, cfg, env)` (pure, **injectable env**; env > config, trimmed, empty→absent; `SOV_MCP_<ALIAS>_TOKEN`→`Authorization: Bearer`, `SOV_MCP_<ALIAS>_API_KEY`→`X-API-Key`, never overwriting an explicit header), `normalizeAliasForEnv`, `redactUrlAuth` (origin-only), `serializeMcpServerConfig` (status projection). `src/mcp/client.ts` — extracted `buildTransport` branching on `cfg.type ?? 'stdio'`: http→`StreamableHTTPClientTransport`, sse→`SSEClientTransport` (headers injected on both POST `requestInit` and the GET stream via an `eventSourceInit.fetch` override — **SSE SHIPPED, not deferred**, the eventsource v3 shape was clean); `ActiveConnection.transport` widened to the base SDK `Transport`; connect-error log sanitized (`sanitizeConnectError` → HTTP status / short class, never a token-bearing URL); non-https + private/loopback URLs warn (never block; no insecure-TLS hatch). Status serializer (`src/server/runtime.ts`) + `HarnessInfoSnapshot` (`src/tools/HarnessInfoTool.ts`) branch on transport: remote → `{transport,url}` (origin-only, no headers/command), stdio → `{transport,command,args}`.

**Fixture approach.** Smoke-proved the fixture against a real `StreamableHTTPClientTransport` round-trip BEFORE writing the suite (this surfaced + fixed the SDK's stateless "transport cannot be reused across requests" constraint → fresh `Server`+transport per request, `enableJsonResponse:true`). New `tests/mcp/fixtures/http-echo-server.ts` is an in-process Streamable HTTP MCP echo server over a real `Bun.serve` listener (SDK's `WebStandardStreamableHTTPServerTransport`, no Hono needed) that records inbound `Authorization`/`X-API-Key` headers.

**RED→GREEN tests.** All four new test groups failed RED first (module-not-found / schema-rejects-type / stdio-only connectOne), then GREEN. `tests/config/settings.test.ts` +8 (legacy `{command}`→stdio unchanged; empty config→`{}`; http parses; sse parses; `{type:'http'}` no-url throws; `{command,url}` throws; `{url}` no-type throws friendly; http unknown-key throws). `tests/mcp/auth.test.ts` (new, 14: env>config for token + apiKey, config fallback, trim/empty→absent, alias normalization, no-overwrite of explicit Authorization/X-API-Key, passthrough headers, stdio→no auth, `redactUrlAuth` strips query/userinfo + invalid-url placeholder). `tests/mcp/remoteClient.test.ts` (new, 6, real HTTP round-trip: connect+list+call+isError; config bearer reaches server; **env token overrides config + reaches server**; dead-URL logged-and-skipped + good server still connects + **log has no token**; mixed stdio+http pool spans both; connect-timeout fires). `tests/mcp/statusSerializer.test.ts` (new, 4: stdio→command/args, legacy no-args→`[]`, http→redacted url with no leaked headers/secret/command, sse→redacted url). `tests/tools/harnessInfoTool.test.ts` +1 (remote renders `transport: http, url: …`, never `command:`). Existing stdio test literals in `client.test.ts`/`integration.test.ts`/`harnessInfoTool.test.ts` updated to the typed `type:'stdio'` shape (runtime back-compat unaffected — the schema still coerces a bare `{command}`).

**Gate.** `bun run lint` clean (biome, 697 files — `bun run format` first reflowed 6 files; one `noDelete` flag in a test fixed via `Reflect.deleteProperty` rather than the unsafe `=undefined` autofix) + `bun run typecheck` clean (`tsc --noEmit`; the SDK's strict-optional `get sessionId()` quirk handled with a narrow `as Transport` at the two remote-transport constructors) + `bun run test` **3255 pass / 0 fail / 14 skip** (9091 expect() calls, 3269 tests across 365 files, ~109 s) — baseline ~3222 + 33 new tests, no new failures, no regressions; the known MockProvider/telegram env-flakes did not surface this run (fully green). `process.env` is never mutated in pure tests (`resolveMcpHeaders` takes `env` as a param; the one remote-client test that exercises a real env var save/restores it). The pre-existing AGENTS.md guard-scanner `[WARN]` lines (installer `curl | bash` doc snippet) are unrelated. No Go changes (Go gate skipped). **Not exercised:** the spec's one manual smoke against a public Streamable HTTP server (deferred to the controller's pre-release pass, kept out of CI by design). CLAUDE.md ≡ AGENTS.md unchanged (no edits there).

## 2026-06-08 — RELEASE v0.6.29: import Claude Code skills + enforce skill `allowedTools` on the `/skill` path

Cut **v0.6.29** off `master` HEAD `ae72bd3` — the ecosystem-openness A+B increment since v0.6.28 (`9e3d611` CC-skill import + `c6cc375` enforce skill `allowedTools` on `/skill` + `ce302d4` docs + `ae72bd3` import hardening / review polish). Followed `docs/05-conventions/cutting-releases.md` (tag-driven CI). Only `package.json` carries the version stamp (build-time JSON import via `src/version.ts`); the CLAUDE.md/AGENTS.md "shipped" lines + ACTIVE FOCUS banner are historical doc references, left untouched.
...
Two related features shipped per `specs/2026-06-08-skill-import-and-tool-scope-enforcement-design.md`. **No release** (the controller reviews + releases). Both built RED→GREEN; committed atomically (A separate from B); pushed `origin/master`.

**Feature A — CC-skill import.** A1: wrapped `SkillFrontmatterSchema` (`src/skills/loader.ts`) in a `z.preprocess` that aliases hyphenated `allowed-tools` → `allowedTools` (no clobber when the native key is present) and splits a comma-separated string into a trimmed list; exported the schema. A2: `importSkill()` (`src/skills/install.ts`) — distinct verb from byte-faithful `installSkill` — parses with the real YAML parser, normalizes (alias + comma-split, synthesize `whenToUse` from `description` when absent, drop CC keys `model`/`license`/`argument-hint`), validates the normalized frontmatter against the exported loader schema (fail loud), copies the source tree, overwrites the target `SKILL.md` with canonical content; returns `{ ok, name, installedAt, converted, warnings }`. Decision held: CC `:`-globs (`Bash(git status:*)`) are NOT auto-translated — warned + left verbatim. Wired 3 layers mirroring `install`: server route `POST /sessions/:id/skills/import` (`src/server/routes/skills.ts`); Go transport `ImportSkill` (`packages/tui/internal/transport/skills.go`); Go verb parser `case "import"` + converted/warnings rendering + usage strings (`packages/tui/internal/app/app.go`). Both `normalizeFrontmatterAliases` (loader) and `normalizeImportedFrontmatter` (install) are immutable — no `delete` (object-omission + shared `splitCommaList` helper), satisfying the `noDelete` lint.

**Feature B — enforce skill `allowedTools` (turn-scoped, `/skill` path).** Seam entirely in `src/server/routes/turns.ts`: the `/skill` resolution retains `skill.allowedTools` and threads it via a new `skillScope?: readonly string[]` param (passed only for `kind:'skill'` with a non-empty list). After `sessionCanUseTool` is built, `scope = buildToolScope({ allowedTools: skillScope, tools: runtime.toolPool, canUseTool: sessionCanUseTool })` — `runtime.toolPool` is READ, never mutated (`buildToolScope` returns a fresh filtered copy). `scope.tools` + `scope.canUseTool` feed BOTH `query()` AND `buildSessionToolContext` (new `effectivePool` opt DEFAULTING to `runtime.toolPool`, so all 7 existing callers + the pinned subagent test are byte-unchanged) so forked sub-agents inherit the narrowed pool. Empty/undefined `allowedTools` → identity (same refs) → non-skill/unscoped turns byte-identical. `SkillTool` (`src/tools/SkillTool.ts`) stays **advisory** — its rendered result now says so. **No-mutation proof:** test asserts `runtime.toolPool.length` unchanged after a scoped turn; `buildToolScope`'s identity branch returns the same `tools`/`canUseTool` references for unscoped turns.

**RED→GREEN tests.** `tests/skills/loader.test.ts` +3 (list-form alias, comma-string alias, no-clobber). `tests/skills/import.test.ts` (new, 7: canonical rewrite + converted/warnings, refs copied, schema refusal, `whenToUse` synthesis loads warning-free, `:`-glob warning kept verbatim, file source, force-overwrite). `tests/server/routes/skills.test.ts` +2 (import route 200 + envelope; 400 on schema failure). `tests/server/skillScope.test.ts` (new, 7: `query()` tools narrowed to `[FileRead]` for `allowedTools:[Read]`; out-of-scope Bash INPUT denied with `tool is outside slash-command scope` via `Bash(git status)`-allowed-but-`echo`-attempted; empty `allowedTools` + plain turn run full pool; intersection drops unknown names; `runtime.toolPool.length` unchanged; sub-agent inherits `parentToolPool === scope.tools`; back-compat default). `tests/tools/skillTool.test.ts` updated for the advisory rendering. New `MockProvider.lastTools` snapshot (mirrors `lastMessages`).

**Note on the deny path (informative, not a divergence):** a tool entirely ABSENT from the allow-list is removed from the pool, so the orchestrator reports `unknown tool: <name>` (a distinct, also-correct enforcement outcome); the `'tool is outside slash-command scope'` `canUseTool` deny fires for a tool that survives the pool filter but whose INPUT is out of scope (content-restricted rule). The deny-reason test targets that path explicitly.

**Gate.** `bun run lint` clean (biome, 690 files) + `bun run typecheck` clean (`tsc --noEmit`) + `bun run test` **3217 pass / 0 fail / 14 skip** (+20 from the ~3197 baseline; no new failures, no regressions). Go: `cd packages/tui && go build ./... && go test ./...` — all packages green (build exit 0; app/components/render/style/theme/transport all `ok`). The pre-existing AGENTS.md guard-scanner `[WARN]` lines (the installer `curl | bash` doc snippet) are unrelated. Docs updated: `docs/03-cli-reference/usage.md` (`/skills import` verb + "Porting a Claude Code skill" + `:`-glob caveat + `/skill` enforcement note + API route table) and `docs/04-extending/extending.md` (`allowedTools` enforced on `/skill`; `SkillTool` advisory; importing CC skills).

## 2026-06-08 — RELEASE v0.6.28: drive delegation-summary fix + subscriptionExecutor/taskRouting mutual exclusion

Cut **v0.6.28** off `master` HEAD `f38663c` (the two SPIKE-follow-up fixes from the entry below — `3f5de12` drive delegation-summary fix + `38549d2`/`f38663c` subscriptionExecutor⊕taskRouting mutual exclusion). Followed `docs/05-conventions/cutting-releases.md` (tag-driven CI). Only `package.json` carries the version stamp (build-time JSON import via `src/version.ts`); CLAUDE.md/AGENTS.md "shipped v0.6.27" lines are historical doc references, not stamps.
...
**Fix 1 — `sov drive` rendered `[result AgentTool] (no summary)` for delegations.** Root cause traced precisely (not guessed): the delegated answer DID reach the model, but the drive *display* read the wrong shape. The orchestrator renders an `AgentTool` tool_result as a STRING — an observation header (`status:`/`summary:` lines, `renderObservationHeader` in `src/core/orchestrator.ts`) followed by a `<subagent_result>…delegated text…</subagent_result>` block — and the SSE `tool_result.output` carries that string verbatim (`src/server/routes/turns.ts` ~990, `output: contentBlock.content`). But `renderToolOutput` in `src/cli/driveCommand.ts` only mined a `summary` from a JSON *object* envelope; for ANY string output it returned `summary:''` → drive printed `(no summary)`. The model still got the full string (so it answered "3"), but the display field was empty. **This was independent of `runSubprocessExecutor`** — its `finalAssistant` is correctly populated (existing tests) and `scheduler.delegate()` correctly returns the summary (existing scheduler test asserts `'SPIKE-OK'`). The native-vs-subscription "divergence" was really a TUI-vs-drive split: the TUI's `compactline.go` renders `Dispatched <subagent_type>` from the always-present `input`, so it never showed `(no summary)`; drive depended on the empty summary field. **Fix:** `renderToolOutput` now recovers a one-line summary from a string output — preferring the `<subagent_result>` body (the delegated answer, whitespace-flattened) and falling back to the observation-header `summary:` line. A bare string with no header still passes through summary-less (deny path unchanged). **Tests:** `tests/cli/driveCommand.test.ts` +4 (subagent_result body → summary; multi-line flatten; empty-body → header fallback; header-only string; bare-string stays summary-less) + `tests/cli/driveCommand.delegator.test.ts` +1 (EventRenderer integration: an AgentTool `tool_result` with the real wire string shape renders the delegated text, not `(no summary)`) + `tests/runtime/subprocessExecutor.test.ts` +1 (canned stream-json final message "There are 3 files." → non-empty extractSummary-equivalent) + `tests/runtime/scheduler.subscriptionExecutor.test.ts` +1 (end-to-end: REAL `runSubprocessExecutor` parsing canned stream-json through `delegate()` → `summary === 'There are 3 files.'`, non-empty). RED reproduced the empty-summary case first (5 fails), then GREEN.

**Fix 2 — `subscriptionExecutor` and `taskRouting` are mutually exclusive.** They are two conflicting cost strategies on the same delegation path (taskRouting forces parent→delegator→API cost-lanes; subscriptionExecutor offloads to a flat-rate subscription subprocess; the `delegator`'s `allowedSubagents` can't even reach the subscription-executor role; ToS postures opposite). Added a top-level `.superRefine` on `SettingsSchema` (`src/config/schema.ts`) that errors when `subscriptionExecutor?.enabled === true` AND `taskRouting?.enabled === true`, with an actionable message. Note: wrapping the object in `.superRefine` makes `SettingsSchema` a `ZodEffects`, so `tests/config/catalog.test.ts` (the only `_def.shape()` consumer) was updated to unwrap `_def.schema` first; all other usages are `.parse()` (unaffected). `tests/config/schema.test.ts` +8 (both enabled → throws with the message; only-subscription / only-routing / each-with-other-absent / neither / both-blocks-absent / bare-`taskRouting:{}`+subscription-on all OK). The live dogfood config (`subscriptionExecutor.enabled:true` + `taskRouting.enabled:false`) stays valid. `computeToolVisibleAgents`'s "both flags on" test is unaffected (it takes direct booleans, not a parsed schema).

**Docs.** `docs/03-cli-reference/usage.md` — added a mutual-exclusion note (with cross-reference) to BOTH the task-routing section and the subscription-executor section. `specs/2026-06-08-subscription-executor-spike.md` — added an "Interaction with task routing (mutual exclusion)" section (the conflict, the parse-time decision, and the future "subscription-as-a-routing-lane" compose explicitly out of scope).

**Gate.** `bun run lint` clean (biome, 688 files) + `bun run typecheck` clean (`tsc --noEmit`) + `bun run test` **3198 pass / 0 fail / 14 skip** (+16 from the 3182 baseline; no new failures, no regressions). Fix 1 + Fix 2 committed separately, plus the docs; pushed `origin/master`.

## 2026-06-08 — RELEASE v0.6.27: opt-in subscription executor (docs + index + release)

Close-out + release of the opt-in headless Claude Code sub-agent executor — the spike (`9686029`) + the learning-replay increment (`6b84822`) + the tool-vocabulary canonicalization (`6789c7c`), shipped as **v0.6.27**. The capability is **off by default** (`subscriptionExecutor.enabled: false`, absent block ⇒ today's behavior, byte-identical) and wired ONLY to the interactive sub-agent delegation seam — never cron / channels / gateway — because driving a Claude *subscription* credential as an automated / multi-tenant backend crosses the personal-use ToS boundary (the per-token API stays the path for everything else).

**Docs.** Added a "Subscription executor (opt-in)" subsection to `docs/03-cli-reference/usage.md` (what it is — delegate to a headless `claude -p` under your local `claude`, subscription flat-rate vs per-token API; the `subscriptionExecutor` config block with JSON + per-field semantics, verified field-for-field against `src/config/schema.ts`; how to use it via `subagent_type:'subscription-executor'` needing `claude` installed+logged-in; the per-tool learning replay canonicalized to native tool names with the brief residual gaps — `durationMs:0` no per-tool timing + success/error-only status; and a prominent **Terms-of-service boundary** subsection — personal/attended use of the official binary on your own subscription ONLY, automated/unattended/multi-tenant/client-product use against subscription terms enforced early 2026 → stays on the per-token API, which is WHY it's interactive-seam-only; safe `--permission-mode` posture, never bypass). Added one bullet to `docs/02-architecture/runtime-architecture.md`. Both edits were authored by a prior interrupted run; verified accurate against the source (`src/runtime/subprocessExecutor.ts` canonicalization map + residual-gap comments; `src/server/runtime.ts` role-gating via `filterAgentRegistry`; schema config keys/defaults) and kept — section anchors cross-check (`usage.md#subscription-executor-opt-in`, `#terms-of-service-boundary`). Indexed the spike doc in `CLAUDE.md` + `AGENTS.md` Forward-looking specs table (`diff` empty via `cp`).

**Gate.** `bun run lint` clean + `bun run typecheck` clean + `bun run test` **3182 pass / 0 fail / 14 skip** (matches the canonicalization-increment baseline; no new failures). Docs committed atomically, version bumped 0.6.26 → 0.6.27, pushed `origin/master`; tag `v0.6.27` → CI release; public `sov-releases` CHANGELOG + tag. **Post-upgrade smoke:** `~/.sov/bin/sov --version` → 0.6.27; the released binary boots cleanly against an isolated temp `HARNESS_HOME` with `subscriptionExecutor:{enabled:true,engine:'claude-code'}` (config accepted, no parse/validation error).

## 2026-06-08 — SPIKE fix: canonicalize replayed Claude Code tool vocabulary to native names for corpus co-clustering — TDD

Closed a corpus-fidelity gap in the learning-replay path: the replay recorded Claude Code's tool NAMES + input keys verbatim (`Read`/`{file_path}`, `Write`, `Edit`, `Bash`/`{command,description}`), which diverge from the harness native vocabulary (`FileRead`/`{path}`, `FileWrite`, `FileEdit`, `Bash`/`{command}`) — so the synthesizer split a delegated file-read from a native one. **Verified the authoritative native names/keys first** from `src/tools/` (`FileReadTool name:'FileRead' aliases:['Read'] key:path`; `FileWriteTool`/`FileEditTool` likewise; `Bash`/`Grep`/`Glob` already match) and the real Claude shape live (`claude -p` v2.1.168: `Read` emits `{file_path, limit}`; `Bash` emits `{command, description}`). Added a pure immutable `canonicalizeToolForObservation(name, input)` at the replay boundary in `src/runtime/subprocessExecutor.ts` (map `Read→FileRead`/`Write→FileWrite`/`Edit→FileEdit`; rename `file_path→path`; drop Claude-only `Bash.description`; unmapped tools pass through unchanged). Applied to the **OBSERVATION + `distinctToolNames` only** — `messages[]` and the trace stay byte-verbatim (fidelity record). 11 new tests in `tests/runtime/subprocessExecutor.test.ts` (the pure map across all six tools + unmapped pass-through + immutability + non-object input; an end-to-end unmapped-tool `WebFetch` replay; co-clustering assertions: a Claude `Read{file_path}`+`Bash{command,description}` transcript → observations `FileRead{path}`/`Bash{command}` matching native, trace stays `Read`/`Bash`). RED confirmed first (export missing), then GREEN. **Gate:** `bun run lint` clean (biome, 688 files) + `bun run typecheck` clean + `bun run test` **3182 pass / 0 fail / 14 skip** (+11 from the 3171 baseline; the scheduler subscription-executor threading tests stay green — they inject a fake executor, so canonicalization in the real parser doesn't perturb them). Spec `specs/2026-06-08-subscription-executor-spike.md` updated (tool identity now co-clusters; parity holds for tool identity; trace/messages verbatim). No release (controller handles it).

## 2026-06-08 — SPIKE increment: stream-json → learning observer + trace replay (delegated turns feed the soak) — TDD + live smoke

Closed the spike's "dark to per-step learning" gap: a delegated headless `claude -p` task's per-tool use is now **replayed into the same learning observer + trace the orchestrator would feed for a native tool call**, so a subscription-executor delegation participates in the active learning-loop soak. Spec updated: `specs/2026-06-08-subscription-executor-spike.md` (the "NEXT increment" section is now "IMPLEMENTED").

**The fidelity (field-for-field with `src/core/orchestrator.ts` ~601–623).** `runSubprocessExecutor` gained optional `learningObserver?` + `traceRecorder?`. The parser captures each `tool_use` (stream order) + indexes each `tool_result` by `tool_use_id`; on a **completed** run it replays per tool: `observe({ toolName = tool_use.name, toolInput = tool_use.input verbatim, status = is_error ? 'error' : 'success', durationMs = 0, traceId = tool_use.id })` + a `tool_start` then `tool_end` (`outputBytes` = byte length of the result content, mirroring the orchestrator) XOR `tool_error` (result content as message). A failed run replays nothing (parity with a native child that errored skipping its hooks). `messages[]` was already faithful (assistant tool_use + tool_result user + final text). **Residual gaps:** `durationMs:0` (stream-json has only an aggregate `duration_ms` on the terminal `result`, no per-tool timing); status is `success`|`error` only (Claude Code resolves `denied`/`cancelled` internally); no harness `ToolObservation` envelope; no per-event "from subprocess" marker (the `TraceEvent` schema has none — child sessionId tagging is the attribution, same as native children).

**Same destination as a native delegation.** The scheduler threads the **child ToolContext's `learningObserver`** (the object the native AgentRunner reads off `toolContext`, inherited from the parent, sessionId-bound) + its **`wrappedTraceRecorder`** (tags each event with the child sessionId, forks to both the parent recorder and the child's per-session `TraceWriter`) into the subprocess executor. Both optional → clean no-op when learning is disabled / no trace sink.

**TDD build (RED→GREEN), 7 new tests.** `tests/runtime/subprocessExecutor.test.ts` +5 (per-tool observation parity on a real-shaped two-tool transcript with one error; faithful `messages[]`; no-op without sinks; orphan tool_use still observed; no-replay-on-error-terminal). `tests/runtime/scheduler.subscriptionExecutor.test.ts` +2 (child observer/trace threaded → observation lands on the parent observer + trace tagged with child sessionId; no observer on the parent context → none threaded). RED confirmed first (observer not wired → 0 observations), then GREEN.

**Grounded on REAL output.** Re-captured `claude -p "List the files…" --output-format stream-json --verbose --permission-mode plan --max-turns 3` (claude v2.1.168) in a temp dir: confirmed the `tool_use` block (id/name/input + a `caller` field we drop) and the matching `tool_result` riding a `type:'user'` frame (`{tool_use_id, type:'tool_result', content:<string>, is_error}`), and that **only the terminal `result` carries `duration_ms`** (grounds the `durationMs:0` decision). The canned two-tool transcript in the test mirrors this exact shape.

**LIVE SMOKE (the proof).** Drove `SubagentScheduler.delegate()` with the **real** `runSubprocessExecutor` (default `Bun.spawn`) + a **real** `LearningObserver` pointed at a temp `HARNESS_HOME`, isolated temp cwd (read-only; never touched `~/.harness`): task `"List the files in the current directory and tell me how many there are"` → terminal `completed`, `toolCallCount:1`, `distinctToolNames:["Bash"]` (Claude chose `ls -la`), summary "3 files". Parent trace recorder saw `tool_start:Bash` + `tool_end:Bash`. **`<harnessHome>/learning/<projectId>/observations.jsonl` got exactly 1 line: `tool=Bash status=success session=smoke-parent-session duration_ms=0` with the real `ls -la …` command in `tool_input_summary`.** Per-tool learning from a delegated headless turn reached the corpus. (One-off smoke script removed after the run.)

**Gate.** `bun run lint` clean (biome, 688 files) + `bun run typecheck` clean (`tsc --noEmit`) + `bun run test` **3171 pass / 0 fail / 14 skip** (clean run — the usual env-only learning flakes all passed this run; +7 from the 3164 baseline). The existing `tests/runtime/scheduler.test.ts` + the spike's original subprocessExecutor tests stay green (normal-path + no-sink no-op unchanged). No release (the controller handles it).

## 2026-06-08 — SPIKE: opt-in headless Claude Code sub-agent executor (off by default) — TDD + live smoke

Built the **subscription-executor** spike — proving the harness can drive a headless `claude -p` session as an opt-in, task-delegating sub-agent executor that round-trips a summary through the UNCHANGED scheduler tail. **Off by default; not productized; NO release** (per the spike's opt-in posture). Spec: `specs/2026-06-08-subscription-executor-spike.md`.
...
**The SMS channel build (commits `08d4797` config + `ec624d3` adapter+route).** SMS is the Slack adapter's shape (open webhook + signature verify + ack-fast-then-async reply via an injectable transport) PLUS an SMS-specific second gate. A phone number is publicly textable and SMS sender-IDs are spoofable, so: (1) the **Twilio `X-Twilio-Signature`** authenticates the TRANSPORT (HMAC-SHA1/base64 over the public URL + alphabetically-sorted params; constant-time; **pinned against Twilio's official documented test vector** in the suite — `url + CallSid/Caller/Digits/From/To` + authToken `12345` → `L/OH5YylLD5NRKLltdqwSvS0BnU=`); a bad/missing signature → **403**. (2) The **sender allow-list** (`gateway.channels.sms.senders`, a `From → principalId` map) is the SENDER gate — an unlisted number is ACKed **200** with no turn/session/reply (no existence leak). `accountSid`/`authToken` are env-first secrets (`SOV_TWILIO_ACCOUNT_SID`/`SOV_TWILIO_AUTH_TOKEN`, never logged); the schema `superRefine` requires a non-empty `senders` map (every value a declared principal) + the three creds for an enabled channel. Carrier-mandated **STOP/UNSUBSCRIBE/CANCEL/END/QUIT → opt-out, START/UNSTOP → re-opt-in, HELP/INFO → static helpText**, all handled before any turn; opt-outs persist durably to `<harnessHome>/channels/sms/optouts.json`. A valid/allow-listed/non-keyword/non-opted-out message acks fast then runs the safe-posture turn on the shared Phase-F `runChannelTurn` pipeline and posts the reply async via the Twilio Messages REST API. Each allowed sender is isolated to its own principal (sessions/memory/learning). The adapter is built + tested against an injected transport (no live Twilio account needed).

**Security review → SECURE-TO-SHIP, two LOW hardenings applied (this session, TDD).**
- **L1 — sender-lookup robustness.** The route's sender gate used `senders[from] === undefined`; while the upstream E.164 regex already neutralizes prototype-shaped keys, the gate is now **self-evidently** prototype-safe via a new pure `resolveSenderPrincipal(senders, from)` that uses `Object.hasOwn` (only an OWN string-valued entry resolves). So a `From` of `__proto__`/`constructor`/`toString`/`hasOwnProperty` never resolves to a principal regardless of upstream ordering. **TDD:** a route-level test (signed `From=__proto__/constructor/toString` → 400, no turn) + a pure-unit test (`resolveSenderPrincipal` returns undefined for every prototype key, resolves a real sender). RED before / GREEN after.
- **L2 — opt-out store write serialization + atomicity.** `writeOptOut`/`clearOptOut` did a synchronous read-modify-write; under concurrent STOPs from different numbers a naive async version is last-writer-win (both read the pre-write file → one opt-out lost). Now each mutation is **serialized through a per-harness-home in-process promise chain** (mirrors `serializePerSession` in `pipeline.ts`) AND **written atomically** (temp file + `rename`, mirrors `delivery.ts`); the route awaits the persist before its ACK. `readOptOuts` stays synchronous (off the write path; a stale read is harmless). The write functions became `Promise<void>` (only the route consumed them; updated to `await`). **TDD:** two concurrent `writeOptOut` for different numbers → both persist, `set.size === 2`. **Verified the test catches the regression:** with serialization defeated the test FAILS (the two un-serialized writes lost a write / collided on the temp path); restored → GREEN.
- Commit `3322059` (`fix(channels): SMS sender-lookup hardening + atomic serialized opt-out store (review LOWs)`).

**iMessage feasibility spike (already-pushed doc `specs/2026-06-06-imessage-channel-feasibility.md`, commit `8da6332`).** Verdict: **NO-GO by default.** Apple ships no official inbound-message API (the official path is a dead end); a **BlueBubbles-bridge** route (a Mac running a bridge server) is technically feasible but ops-heavy (a dedicated always-on Mac) and ToS-risky; the cheaper substitutes that already ship are **Telegram** (no public endpoint) or **webhook + an Apple Shortcut**. Documented, not built.

**Docs + index.** `docs/03-cli-reference/usage.md` gained a full **SMS** subsection (config shape with the `senders` allow-list, env-first secrets, Twilio setup steps incl. the exact `POST /channels/sms` webhook URL + A2P 10DLC note + automatic STOP/HELP/START, the two-gate security model + safe-by-default backstop, v1 limits) + SMS rows in the channels config example/table + boot-summary mention. `docs/02-architecture/runtime-architecture.md` gained the SMS-adapter bullet (Twilio webhook → signature + sender allow-list → safe-posture turn → async Messages-REST reply) + config/hosting mentions. `CLAUDE.md` + `AGENTS.md` Forward-looking index gained the SMS spec + plan + the iMessage feasibility doc; `diff CLAUDE.md AGENTS.md` EMPTY (cp).

**Gate (before the release commit).** `bun run lint` clean (`biome check src tests`, 683 files) + `bun run typecheck` clean (`tsc --noEmit`) + `bun run test` **3138 pass / 0 fail / 14 skip** (clean run — even the 3 known env-only learning flakes passed; includes the +4 L1/L2 tests). All 150 channels tests green.

**Post-upgrade binary smoke (v0.6.26, isolated temp `HARNESS_HOME`, real config untouched).** Built + released v0.6.26, `sov upgrade`'d, then booted the upgraded `~/.sov/bin/sov gateway` with a temp config (`gateway.principals:[{id:'sms-user',token:'t'}]` + `gateway.channels.sms` enabled with `accountSid:'ACtest'`, `authToken:'faketok'`, `fromNumber:'+15550000000'`, `senders:{'+15551234567':'sms-user'}}` + the anthropic key copied from real config). Boot log showed `channels: sms`. Via curl against `POST /channels/sms` (form body, computed `X-Twilio-Signature` = HMAC-SHA1/base64 of the exact URL + sorted params with `faketok`): a **valid signature + allowed sender** `From=+15551234567` → **200** (session created for the allowed sender; the reply send to fake Twilio failed + was swallowed — fine, the turn ran); the **same POST with a bad signature** → **403**; an **unlisted `From=+19999999999` with a valid signature** → **200** with **no session created**. (Results below in the release section.) `~/.sov/bin/sov --version` → **0.6.26**.

## 2026-06-06 — Two independent fixes: CI Node-24 bump (backlog #49) + cron learning-loop participation (TDD)

Two unrelated logical changes in one session, separate commits.

**1. CI — bumped GitHub Actions off Node 20 (`ci:` commit, closes backlog #49).** `.github/workflows/release.yml` pinned several Node-20 actions; GitHub forces Node 24 from 2026-06-16. Bumped each to its current latest Node-24 major, verifying each target version exists via the action's GitHub releases page (WebFetch + WebSearch): `actions/checkout@v4 → v5` (v5 is the first node24 major; v6 also exists), `actions/setup-go@v5 → v6` (v6.0.0 upgraded the runtime node20→node24; v5 is still node20), `actions/upload-artifact@v4 → v5`, `actions/download-artifact@v4 → v5`. Left `oven-sh/setup-bun@v2` as-is — v2.2.0 already runs on Node 24 (no v3 exists). YAML re-parsed clean via the `yaml` package (all four jobs intact); confirmed no `@v4`/`setup-go@v5` remain. Marked #49 CLOSED in `docs/08-roadmap/backlog/post-phase-13-4.md` (open backlog 7 → 6). Docs/YAML only — no test run needed for this commit (no `src/` change).

**2. Cron — cron turns now inject MEMORY.md + run recall (`fix(cron):` commit, TDD).** `src/cron/wiring.ts` built its headless `AgentRunner` WITHOUT `memoryManager`/`recall`, so a scheduled job never injected MEMORY.md, never ran recall (`<learned-context>`), and never wrote memory back — the EXACT omission the Phase-F channel fix (`e23b869`) flagged as still-open for cron ("cron has the SAME omission — pre-existing, out of scope here"). Fix mirrors the channel pipeline: read the SAME cached per-session context the toolContext was built from (`sessionCtx = runtime.getSessionContext(sessionId)` — `buildSessionToolContext` already calls `getSessionContext`, so this is the cached instance, no second build) and thread `memoryManager: sessionCtx.memoryManager` + `...(sessionCtx.recall ? { recall } : {})` into the `AgentRunner`. Cron sessions are owner-null → legacy/global memory + learning namespace (correct for operator-scheduled jobs). Existing cron behavior otherwise unchanged (auto-deny permissions, `extractFinalText`, `disposeSession` in `finally`, `[SILENT]` delivery — all untouched). **TDD:** new `tests/cron/wiring.recall.test.ts` (3 cases, mirrors `turns.recall.test.ts` + channel `pipelineFixes.test.ts` Fix-1): a cron job (MockProvider) with a seeded legacy MEMORY.md asserts `<memory-context>` + the body reach the provider; a seeded `_global` instinct asserts `<learned-context>` + the action reach the provider; a `syncTurn` spy (via a `sessionContextFactory` wrapper, since cron mints the session id internally) asserts write-back fired. **RED before the wiring (3 fail: no `<memory-context>`, no `<learned-context>`, syncCalls=0) / GREEN after (3 pass).**

**Gate.** `bun run lint` clean (`biome check src tests`; one import-order nit in the new test auto-flagged + fixed) + `bun run typecheck` clean (`tsc --noEmit`) + `bun run test` **3100 pass / 0 fail / 14 skip** (+3 from the ~3097 baseline = the new cron tests; the 3 known env-only learning flakes happened to pass this run). All 63 cron tests green (no regression in the existing outbox e2e). Two atomic commits; pushed to `origin/master`.

## 2026-06-06 — Run-anywhere (A–F) holistic E+F review, channel/gateway fixes, and a documentation audit

Three threads in one pass over the run-anywhere program (Phases A–F): a holistic correctness review of the late phases (E multi-user + F channels), the code fixes that review (and an adversarial pass) surfaced, and a docs-only discoverability/organization/completeness audit that brings the doc set current to the post-fix behavior. No bundle changes, no new ADRs.

**1. Holistic E+F review (findings).** Reviewed the multi-user (E) + channels (F) surfaces against their specs/plans and the rest of the gateway. The feature docs were independently verified ACCURATE — **no doc-vs-code drift** in the shipped behavior. The review's High/Medium findings were correctness/UX defects in the channel pipeline + a per-user learning-path bug + gateway-auth-config hardening (below), not documentation errors. Verdict on the surface after fixes: **VERIFIED.**

**2. The fixes (committed, then verified).** Three fix commits landed before this docs pass:
- `e23b869` — **four channel-pipeline defects** (`src/channels/pipeline.ts`, TDD): (1) channel turns silently opted OUT of the learning loop → now inject MEMORY.md + run recall (`<learned-context>`) + write memory back, scoped to the channel principal; (2) unbounded re-seed could overflow + permanently brick a conversation → now a **bounded recent window** (`capSeededHistory`, ~40 messages; older turns drop from in-context, memory/learning carry the rest) + a user-facing **error fallback** on a non-completed terminal (never pure silence); (3) concurrent messages from one `(channel,sender)` raced the shared SessionContext (double-dispose) → **serialized per sessionId**; (4) empty/whitespace inbound text ran a billable turn → **ignored** before any upsert/turn. (Detailed sub-entry below.)
- `08fd2c4` — **per-user synthesis path fix** (the learning synthesizer wrote/looked under the wrong corpus path for a per-principal turn) + **gateway auth-config hardening**.
- `61fb67f` — **real Telegram long-poll** (`getUpdates` with a server-side `timeout`, holding the connection open rather than busy-polling) + **failure backoff** + an **actionable bad-token/network log line** (never the token); **idle channel-session sweep** after 30 days (`cleanupOldChannelSessions`, called from `buildRuntime`); **source-id hardening** (Slack/Telegram inbound ids validated as safe segments at the source via the shared `isSafeSegmentId`); a **1 MiB inbound body cap** on all `/channels/*` routes.

**Verification — VERIFIED.** Each fix was confirmed present in the code during this pass: `pipeline.ts` threads `memoryManager` + conditional `recall` into AgentRunner, guards empty text, and funnels the turn through `serializePerSession`; `seedHistory.ts` caps the seed at `DEFAULT_MAX_SEEDED_MESSAGES = 40` (pairing-safe, repaired); `telegram.ts` long-polls with `timeout: LONG_POLL_TIMEOUT_SECS`, backs off `DEFAULT_FAILURE_BACKOFF_TICKS`, and logs an actionable line; `sessionDb.cleanupOldChannelSessions(maxAgeMs = 30d)` is invoked in `runtime.ts`; `channels.ts` enforces `MAX_CHANNEL_BODY_BYTES = 1_048_576` and the webhook/telegram/slack adapters validate ids via `isSafeSegmentId`. (The prior adversarial security review had already returned SECURE-TO-SHIP after fixing the CRITICAL path-traversal write in `260de9f`.)

**3. The documentation audit (this pass — docs-only).** A discoverability/organization/completeness sweep over the run-anywhere docs, PLUS reflecting the fixes above so the docs describe CURRENT behavior (notably: channel conversation context is a **bounded recent window**, NOT "uncompacted / can overflow"). Applied:
- **README.md** — H1: new `## Surfaces` section naming the four run modes (TUI / `sov drive` / `sov serve` / `sov gateway`) with one line + usage anchor each; added `src/server/` (updated) + `src/channels/` (new) rows to the contents table. H2: stale `Latest snapshot` Status pointer → `docs/07-history/state/2026-06-06-phase-f-channels.md` (A–F COMPLETE). (`9403611`)
- **docs/03-cli-reference/usage.md** — H3: `sov gateway` row in the CLI Subcommands table. H4: full `gateway.*` group (host/port/token/corsOrigins/eventBufferSize/idle*/maxConcurrentSessions/principals/channels.*) in the master config-fields table, defaults verified against `src/config/schema.ts` + the call sites. H5: consolidated the trust model into `### Security model` (within-org vs founder-reserved hostile-multi-tenant; the "every principal/token = full tool powers" invariant stated once; channel safe-by-default pointer) + cross-linked Phase E back to it. M2: nested `## Channels` → `### Channels` under `## Remote gateway`. M3: gateway overview arc paragraph (A→…→F) with per-subsection links. M4: surfaces capabilities matrix. M5: a single `### Known limitations (v1)` at the end of `## Remote gateway`, reflecting the fixes (bounded window not overflow; 30-day sweep; per-user trace/CLI caveats; hostile-multi-tenant out of scope). L1/L3/L4 (present-tense framing / fuller config block / webhook reserved id `default`) + fix-reflection prose in the channel sections (learning loop on; bounded window; serialization; empty-text; error fallback; telegram long-poll+backoff; body cap; source-id validation). All new intra-doc anchors verified to resolve. (`64ccd3d`)
- **docs/04-extending/extending.md** — M6: `## Add A Channel Adapter` recipe (adapter contract verify→parse→deliver with source-level safe-segment id validation; the shared `runChannelTurn`; the safe-by-default `buildChannelCanUseTool` posture that must not be weakened; `channelsRoute`/`buildChannelListeners` wiring; `gateway.channels` schema + env-first secrets; the path-traversal source/sink defense-in-depth lesson) + a config-only `### Add A Principal` note. (`0e6c02f`)
- **docs/02-architecture/runtime-architecture.md** — L1: present-tense "run-anywhere A–F complete" framing at the two stale "Phase A" spots (intro + Native Gateway section), naming the per-phase modules layered over the protocol seam. (`1c01954`)
- **CLAUDE.md + AGENTS.md** — M1: the roadmap spec (marked ✅ COMPLETE A–F) + six per-phase specs + six plans added to the Forward-looking index. L2: appended "OpenAI + native gateway server surfaces (auth, multi-client transport, supervisor, multi-user isolation, channels)" to the architecture.md design-reference row. `diff CLAUDE.md AGENTS.md` EMPTY (cp after edit). (`f7d2780`)

**Gate (docs-only).** `bun run lint` clean (`biome check src tests` — docs untouched in src/tests, as expected) + `bun run typecheck` clean (`tsc --noEmit`). `git diff --stat` for the docs commits shows only `README.md`, `docs/**`, `CLAUDE.md`, `AGENTS.md` — no code file touched. Full test suite not re-run (docs-only); the prior run logged below stands at 3082 pass / 0 fail / 14 skip. Committed in logical groups; pushed to `origin/master`.

## 2026-06-06 — Phase F channel-pipeline defect fixes (holistic-review follow-up, 4 fixes, TDD)

Fixed four real correctness/UX defects in `runChannelTurn` (`src/channels/pipeline.ts`) found by a holistic review of the Phase F channel surface. NOT security (Phase F is already SECURE-TO-SHIP). Each fix was TDD'd (failing test first, then green). New test files: `tests/channels/pipelineFixes.test.ts` (10 cases across the four fixes) + `tests/channels/seedHistory.test.ts` (5 unit cases for the new bounded-seed helper).

**Fix 1 [High] — channel turns got NO recall + NO MEMORY.md (silently opted out of the learning loop).** `runChannelTurn` built its `AgentRunner` without `memoryManager`/`recall`, so a channel turn never injected MEMORY.md, never ran recall (`<learned-context>`), never wrote memory back — contradicting the spec's D2 (memory+learning scoped) and the recall-on-by-default soak. (Capture/observe already ran via the toolContext.) Added optional `memoryManager?` + `recall?` to `AgentRunnerOpts` and threaded both into AgentRunner's internal `query()` call (conditional spreads, so cron + sub-agents + every test that omits them are byte-unchanged). In the pipeline, read the SAME per-session `sessionCtx = runtime.getSessionContext(sessionId)` the toolContext was built from and pass `memoryManager: sessionCtx.memoryManager` + `...(sessionCtx.recall ? { recall } : {})` — mirrors the turns route. Tests (MockProvider, inspect `lastMessages`): owner-scoped MEMORY.md → `<memory-context>` reaches the provider; a seeded global instinct under `users/{principal}/learning/_global` → `<learned-context>` + the action reach the provider; a `syncTurn` spy fires (write-back). RED before / GREEN after. **Flagged: cron (`src/cron/wiring.ts`) has the SAME omission — pre-existing, out of scope here.**

**Fix 2 [High] — context overflow permanently bricked a channel conversation, silently.** The pipeline re-seeded the WHOLE per-(channel,sender) history every turn with no cap and ignored `result.terminal.reason` — so on overflow `finalAssistant` was undefined → `{ silent: true }` → the user got silence, and since the overflowing message was already persisted EVERY later message also overflowed (bricked, no recovery). Compaction isn't viable (it pivots to a new child session id → breaks `buildSessionKey` continuity). **(a)** New `src/channels/seedHistory.ts` `capSeededHistory(history, max=40)`: takes the last N (default **40** messages — ≈20 turns, comfortably under any context limit), then PAIRING-SAFE truncation (drops leading orphan tool_result messages so the seed never starts with a dangling tool_result the provider would reject), then `repairMissingToolResults` over the retained window. A long conversation now "forgets" the oldest turns (acceptable chat-window behavior; memory/learning carry longer context). **(b)** On a non-`completed` terminal with no usable text, return a user-facing fallback (`"Sorry — I hit an error handling that. Please try again."`) instead of `{ silent: true }` (mirrors cron `wiring.ts:165-176`). Tests: a 200-message history is capped (provider sees ≤60, < full history); the seed never leads with an orphan tool_result; a scripted provider `throw` → non-silent fallback text. RED/GREEN. **This directly resolves the v1 limitation (a) documented in the Phase F close-out entry below ("channel turns are uncompacted").**

**Fix 3 [High] — concurrent messages from the same sender raced on the shared SessionContext (double-dispose).** Two messages from one sender map to the same deterministic `sessionId`; Slack (fire-and-forget) + webhook (concurrent HTTP) could drive two `runChannelTurn` on that id at once → both shared the cached SessionContext and whichever's `finally` fired first called `disposeSession` (closing the trace writer + draining the observer + writing the trajectory) WHILE the other turn was live → corrupted/lost trace+learning, double trajectory write, a context disposed under a running turn. **Serialized per `sessionId`** via a module-level `Map<sessionId, Promise>` chain (`serializePerSession`): each turn chains on the prior via `.then(task, task)` (a thrown turn never wedges the queue); the map entry is reclaimed when its chain drains (guarded so a newer turn that replaced the entry isn't deleted) → no unbounded growth. `runChannelTurn` is now a thin guard wrapper (bypass + empty-text checks run BEFORE the lock) over the serialized `runChannelTurnInner`. Tests: two concurrent turns on the SAME session → exactly TWO trajectory lines (one per turn), 4 persisted messages (turn 2 saw turn 1's reply), no "disposed context" error; two concurrent turns on DIFFERENT sessions still run (serialization is per-session, not global). RED (raced/double-wrote) / GREEN.

**Fix 4 [Medium] — empty/whitespace inbound text ran a billable turn.** Central guard at the top of `runChannelTurn`: `if (msg.text.trim() === '') return { silent: true }` BEFORE upsert/persist/turn — so all three adapters are consistent (the webhook accepted `""`; slack/telegram only rejected length-0, not whitespace-only). Tests: empty + whitespace-only inbound → `MockProvider.streamCalls === 0`, no persisted row, `{ silent: true }`. RED/GREEN.

**One existing test updated to the new (correct) behavior:** `tests/channels/telegram.test.ts` "resilience — one throwing update does not abort the batch" asserted the throwing update sent nothing (old silent-on-error behavior); now Fix 2(b) sends a fallback reply, so it asserts TWO sends (fallback + the real reply) with the offset still advancing — the resilience contract is unchanged, only the silent→fallback behavior flipped.

**Gate.** `bun run lint` clean (`biome check`), `bun run typecheck` clean (`tsc --noEmit`), `bun run test` **3082 pass / 0 fail / 14 skip** (clean full run; up from the ~3067 Phase F baseline by the 15 new tests; the 3 known env-only learning-test flakes all passed in this run — no new failures). Backward-compat verified: scheduler + cron AgentRunner callers omit `memoryManager`/`recall` → byte-unchanged. No bundle changes, no new ADRs. Committed; NOT pushed/released per instruction.

## 2026-06-06 — Phase F close-out: channel framework + Slack/Telegram/webhook adapters shipped (v0.6.23) — the run-anywhere roadmap A–F is COMPLETE

Close-out of **Phase F — Channel framework + first adapters** (modules M6 + M7; the final module of the run-anywhere roadmap). Covers the F-T1…F-T9 build, the adversarial security review that found + fixed a CRITICAL path-traversal write, the full gate, and the release.

**The build (F-T1…F-T8, 9 commits, subagent-driven TDD per task).**
- F-T1 — safe-by-default channel permission posture (`buildChannelCanUseTool` never loads local allow-rules + auto-denies; `bypass` rejected) + the extended `ChannelAdapter` contract.
- F-T2 — the channel-agnostic `runChannelTurn` pipeline (per-`(channel,sender)` session via `buildSessionKey`+`upsertSession`, owned by the channel principal → Phase-E isolation; headless turn; reclaim-in-finally) + F-T2 follow-up: hydrate prior history so multi-message conversations are coherent.
- F-T3 — `gateway.channels` config (webhook/telegram/slack; `permissionMode` enum excludes `bypass`; superRefine requires secret(s) + `principalId` ∈ `gateway.principals`).
- F-T4 — generic webhook adapter + open route (HMAC-SHA256 over the raw body, synchronous reply) — the keystone, no external deps.
- F-T5 — Telegram adapter (`getUpdates` long-poll over an injectable transport, `unref`'d loop, offset advance + per-update try/catch).
- F-T6 — Slack Events adapter (signing-secret `v0=` HMAC + 300 s replay window + `url_verification` challenge + ack-fast-then-async `chat.postMessage` + retry dedupe).
- F-T7 — gateway wiring (mount the open routes + start/stop the Telegram worker in the lifecycle; env-first secret resolution pre-parse) — see the entry below.
- F-T8 — channel isolation + security suite (per-principal isolation; a channel can't run `Bash`/`Write` even with a seeded local `allow Bash(*)`; bad webhook HMAC / bad-or-stale Slack signature rejected; `bypass` rejected) + the adversarial review.

**The adversarial security review (F-T8 hard gate) — found + fixed a CRITICAL, re-reviewed SECURE-TO-SHIP.** A hard adversarial review over the whole Phase F surface found a **CRITICAL path-traversal arbitrary-file-write**: an attacker-controlled webhook `chatId` (or `sender`/`threadId`) containing `../` flowed through `buildSessionKey` into the session id → the trace FILENAME (`<sessionId>.jsonl`), so a forged-but-valid-signature request could escape the trace dir and write attacker-chosen bytes to an attacker-chosen path **before the model ran**. Fixed defense-in-depth at both boundaries (`260de9f`): the **source** (`parseWebhook` validates the inbound ids against a safe-segment allowlist → 400) and the **sink** (`TraceWriter` in `src/trace/writer.ts` collapses `..` runs, replaces path separators/control chars with `_`, and containment-asserts the resolved path stays under the traces dir). A re-review over the fixed surface returned **SECURE-TO-SHIP — no remaining Critical/High**: the conservative posture holds (proven with a seeded local allow rule), auth is constant-time + replay-resistant (webhook HMAC; Slack signing secret + 300 s window; Telegram token env-first + never logged), the channel→principal isolation holds for sessions/memory/learning, inbound parse is bounded, secrets are never logged.

**Known v1 limitations (documented, not fixed):** (a) channel turns are uncompacted (a very long conversation could overflow context — same caveat as cron); (b) channel sessions use colon-delimited ids → not individually `/sessions/:id*`-addressable (fail-closed/secure, channel-managed, rows accrue like cron); (c) LOW hardening follow-ups (no inbound body-size cap, gateway-wide; Slack/Telegram inbound ids rely on the airtight trace-sink sanitizer rather than source-level validation); (d) live Slack/Telegram need real external credentials (operator setup documented in `usage.md`, **not live-verified here** — adapters built + tested against injected transports + a real-HMAC webhook e2e); (e) a minor `StartServerOptions.channels` type carries the `telegram` key (harmless, structural).

**The gate (F-T9, pre-release).** `bun run lint` clean (`biome check`), `bun run typecheck` clean (`tsc --noEmit`), `bun run test` **~3067 pass / 0 fail / 14 skip** (clean run; up from the Phase-E ~2957 baseline; gate criterion unchanged — no new failures beyond the known env-only learning-observer set, which passes on a clean `HARNESS_HOME` / in CI). No bundle changes, no new ADRs.

**Release v0.6.23 + post-upgrade binary smoke.** Pushed all Phase F commits to `origin/master`; bumped `package.json` 0.6.22 → 0.6.23; tag-driven CI built + published the 4 release artifacts to `yevgetman/sov-releases`; `sov upgrade` pulled it onto `~/.sov/bin/sov`. **Smoke (proves channels ship in the binary):** an isolated temp `HARNESS_HOME` with `gateway.principals: [{ id:'wh', token:'t' }]` + `gateway.channels.webhook: { enabled:true, secret:'whsec', principalId:'wh' }`; booted the upgraded `~/.sov/bin/sov gateway` on loopback; `POST /channels/webhook/default` with a VALID `X-Signature: sha256=<hmac>` over the raw body → a reply (200); the SAME POST with a BAD signature → **401**. `~/.sov/bin/sov --version` → 0.6.23. The real config was NOT mutated; the temp home was cleaned up. (Full smoke result recorded in the release run below.)

**Roadmap status: A–F COMPLETE.** With Phase F shipped the run-anywhere roadmap is done — A (v0.6.17/18) · B (v0.6.19) · C (v0.6.20) · D (v0.6.21) · E (v0.6.22) · F (v0.6.23). The learning-loop soak ran in parallel throughout, untouched (recall ON by default since v0.6.16 + capture on). Cross-repo `sovereign-ai-docs` sync (D + E + F shipped + roadmap COMPLETE) is still pending for a future docs session.
...
`package.json` check in [`conventions/semantic-tests.md`](docs/05-conventions/semantic-tests.md) (override
...
**Release.** Cut **v0.6.20** (CI-driven, per `docs/05-conventions/cutting-releases.md`): bumped
...
- **Security review:** Phase A reviewed (exposing a tool-running agent off-loopback is the
  ...
  "run behind a constrained permission policy + TLS when exposed" guidance in `docs/03-cli-reference/usage.md`.
...
- **Release:** cut **v0.6.17** (CI-driven, tag-push pipeline) per
  `docs/05-conventions/cutting-releases.md`. Runtime change (new `sov gateway` surface) → release
...
Final verification for the Phase 1 close-out (docs-only change in this pass — `docs/02-architecture/runtime-architecture.md`,
`docs/03-cli-reference/usage.md`, the new state snapshot `docs/07-history/state/2026-06-04-learning-loop-spike-phase-1.md`,
`CLAUDE.md`/`AGENTS.md` reconciled byte-identical + state pointer, `docs/08-roadmap/backlog/post-phase-13-4.md`,
...
- **Fixes (12 files, doc-only):** new canonical state snapshot
  `docs/07-history/state/2026-05-31-post-m2-hardening.md`; re-pointed README + CLAUDE.md/
...
- Record-of-truth doc at `docs/05-conventions/visual-tui-qa.md` covers configuration, scenario authoring, runner semantics, workflow patterns, and limitations.
...
**Scope:** Phase 2 of multi-provider task routing per `specs/2026-05-23-multi-provider-task-routing-design.md`. Adds full observability + lane timeoutMs enforcement + env-var override. 10 implementation tasks (T1-T10) shipped. T11 (this entry) + T12 (release).
...
**Scope:** Phase 2 T7 from `plans/2026-05-23-phase-2-task-routing.md`. The chat completions streaming branch (`POST /v1/chat/completions` with `stream: true`) now subscribes to the per-session event bus before driving `translateStream` and emits each delegator_* event as a `event: hermes.delegator.progress\ndata: <json>\n\n` SSE side-channel frame interleaved with the OpenAI-shaped main stream. `unsubscribe()` runs in the route's `finally` block so a closed stream never receives writes from a late publish. Purely additive — the existing `translateStream` consumer is untouched.
...
**Scope:** Phase 2 T3 from `plans/2026-05-23-phase-2-task-routing.md`. Wired the per-call lane timeout override path through the scheduler. `SubagentScheduler.DelegateInput` now accepts an optional `perChildTimeoutMsOverride` consumed ahead of the existing `opts.perChildTimeoutMs ?? agent.maxTurns * DEFAULT_PER_TURN_TIMEOUT_MS` fallback. `ToolContext` carries an optional `laneRegistry` so `AgentTool.call` can resolve `agent.role → lane.timeoutMs` at dispatch time and thread it onto `scheduler.delegate()`. The Phase 1 deferred `atomTimeout.test.ts` is now unskipped and verifies a 50ms `cheap-task` lane timeout cancels a slowMode-throttled mock stream.
...
**Scope:** Phase 18 (OpenAI-compatible HTTP API server) closed. T11 is the docs close-out for the phase: created `docs/07-history/state/2026-05-23-phase-18-openai-api-server.md` (full close-out snapshot mirroring the Phase 17 structure), updated `CLAUDE.md` + `AGENTS.md` (byte-identical mirror — `diff` confirmed `IDENTICAL`) so the Session-boot pointer + Current-state table row both reference the new snapshot at the top, added a complete `## OpenAI-compatible HTTP API (`sov serve`)` section to `docs/03-cli-reference/usage.md` (quickstart with curl + Python SDK + Open WebUI; CLI flags table; endpoints table; session continuity semantics; tools + side-channel doc; abort propagation; error envelope semantics; configuration; deployment), added an `## OpenAI HTTP Server` section to `docs/02-architecture/runtime-architecture.md` (sits parallel to TUI / drive / dispatch / cron; ASCII boot diagram; per-request flow; tool execution invariant D9; statelessness invariant D10; abort propagation chain; session observability; cron co-deployment), added an `## Add An OpenAI Route` recipe to `docs/04-extending/extending.md` (4-step pattern: route module → mount in `/v1` auth group → Zod schemas → two-layer testing; tool execution invariant + auth scope + SessionDb conventions).

**Phase 18 chain (T1-T10 + 3 audit fixes):** T1 bearer auth + module skeleton (`8892304`) → T2 chat completions non-streaming with mock provider (`f04bd68`) → T3 `sov serve` subcommand (`67abfa6`) → T4 text-delta streaming translator (`4efdb0f`) → T5 SSE streaming on chat completions (`9468c7d`) → T6 tool-use chunks + `hermes.tool.progress` events (`ae6f7c5`) → T7 GET /v1/models (`f872388`) → T8 X-Session-Id header + DB persistence (`b8b1a8a`) → T9 per-request provider resolution from `req.model` (`063635e`) → T10 abort in-flight turn on client disconnect (`22e20f5`) → audit fix H1 namespace X-Session-Id to prevent cross-surface session pollution (`7303e96`) → audit fix H2 wrap non-streaming drain in catch + return OpenAI error envelope (`573a45b`) → audit fix H3 collapse non-standard `finish_reason` values to `'stop'` (`ad74f0c`) → (this docs close-out, TBD).

**3 HIGH audit findings — all fixed inline pre-close-out (verified no regressions):**

- **H1 (`7303e96`)** — Client-supplied `X-Session-Id` headers were passed verbatim to `runtime.sessionDb.upsertSession()`. `upsertSession` silently returns the existing row's `session_id` if a row with that PK already exists, regardless of `metadata.kind`. A careless or malicious client could send `X-Session-Id` matching a TUI/cron/drive session UUID and have its messages appended to that surface's transcript. **Fix:** prefix all client-supplied values with `openai:` before use as the row PK (defensive 256-char cap preserved). The openai-api session keyspace is now structurally disjoint from any other surface. Wire response (`chatcmpl-<id>`) echoes the CLIENT's unprefixed view (public contract unchanged); client's supplied id also preserved in `metadata.clientSessionId` for observability. 7 tests in `tests/openai/session.test.ts` (2 new + 5 updated to look up rows under the prefix).
- **H2 (`573a45b`)** — Two distinct failure modes were swallowed: (1) `query()` catches provider exceptions internally and surfaces a `Terminal{reason: 'error', error}` rather than propagating — the non-streaming route happily returned 200 OK with empty assistant message and `finish_reason: 'error'`; (2) anything escaping `query()` (provider 401, network error, malformed credentials in a path `query()` doesn't catch) bubbled to Hono's default 500 with no body. SDK clients had no structured way to know the call failed. **Fix:** single classifier `buildProviderErrorResponse` invoked from both paths produces a consistent OpenAI-shaped envelope. `CredentialUnavailableError` / `ProviderHttpError 401|403` / credential-related message heuristic → 401 + `invalid_api_key`. `ProviderHttpError` / SDK-shaped errors with a `.status` field → mirror upstream status + `upstream_error` (a real 429 stays 429, not 500). Else → 500 + `api_error` generic. 3 new tests + `MockProvider.throwOnNext` static field (auto-resets after one throw) for deterministic catch-path exercise.
- **H3 (`ad74f0c`)** — `mapTerminalToFinishReason` returned `'error'` when the terminal reason was `'error'`, `'interrupted'`, or `'checkin'`. OpenAI's spec only defines `'stop' | 'length' | 'content_filter' | 'tool_calls' | 'function_call'`, so SDK clients (openai-python, openai-js) raise a validation error during response parsing on any other value. **Fix:** collapse all non-length terminals to `'stop'`. Return type narrowed at the type level (`'stop' | 'length'`) — TypeScript enforces the contract. D9 reaffirmed: never `'tool_calls'` — the harness runs tools internally. Note: H2 short-circuits `reason === 'error'` into an HTTP error envelope BEFORE this function runs; H3 keeps the function defensive for future callers that bypass the H2 envelope. 7 new tests in `tests/openai/finishReason.test.ts` covering every `Terminal['reason']` value + static-type guarantee on the return shape.

**Semantic tests — none added this round; rationale:**

The semantic test framework at `tests/semantic/` is shaped around driving `sov drive` (the line-driven stdin/stdout headless surface, see `tests/semantic/framework/driver.ts`) with an LLM judge evaluating the resulting transcript. Adding a `sov serve` semantic test would require either (a) expanding the driver to spawn `sov serve`, talk HTTP via `fetch`/`curl`, and feed transcripts to the same judge — which is a meaningful framework extension, OR (b) writing an inline `bun test` integration that boots `Bun.spawn`, exercises curl-equivalent flows, and asserts on the wire shape — but **this is exactly what `tests/openai/serve.cli.test.ts` already does** (it forks `bun src/main.ts serve`, hits `/health` + `POST /v1/chat/completions`, asserts response shape, sends SIGTERM, and verifies clean exit; second test exercises the missing-API-key refusal path). The existing 16 test files under `tests/openai/` cover every wire path with both Hono in-memory `app.request()` (fast unit) and real-Bun.serve socket integration (`abort.test.ts`, `serve.cli.test.ts`) — adding a semantic-suite layer on top would duplicate coverage without exercising any new code path. The decision: **skip semantic for Phase 18, document this rationale, and revisit if a future audit surfaces a behavioral category our unit/integration tests don't catch.** T12 (binary release) will hand-smoke a real-Anthropic Haiku call against `sov serve` via the Python `openai` SDK as part of the release verification — that's the moment the real-LLM exercise lands.

**Stale-reference deep-crawl results.** Grepped `CLAUDE.md` / `AGENTS.md` / `docs/` for `most recent close`, `canonical current`, `2026-05-22-phase-17-cron`, `harness-build-plan`, `Phase 17`, `Phase 18`. Three stale pointers fixed:
1. `CLAUDE.md` + `AGENTS.md` Session-boot list item 3 — pointer flipped from Phase 17 close-out to Phase 18 close-out with a new one-paragraph description of the OpenAI HTTP API surface.
2. `CLAUDE.md` + `AGENTS.md` Current-state table — new top row added for Phase 18 close-out; prior Phase 17 row demoted to second slot with a "T11 (cut v0.3.0 release) followed; Phase 18 then opened" trailer; lower rows preserved.
3. `CLAUDE.md` + `AGENTS.md` docs/plans pointer — "Latest" flipped from `2026-05-22-phase-17-cron-scheduled-jobs.md` to `2026-05-23-phase-18-openai-api-server.md`.
4. `CLAUDE.md` + `AGENTS.md` backlog table cell — appended "Phase 17 (cron / scheduled jobs) closed 2026-05-22 evening; Phase 18 (OpenAI HTTP API server) closed 2026-05-23" to the trailer.
5. `docs/08-roadmap/backlog/post-phase-13-4.md` first paragraph — "freshest concretely-specced next phase is Phase 21" updated to "freshest closed phases are Phase 17 + Phase 18; next concretely-specced phases are Phase 21 M2 plus demand-gated 16.5/19/20 channels."

**Final suite numbers:** TS **2192 pass / 0 fail / 14 skip** (+117 from morning Phase 17 close-out's 2075 baseline; +196 from prior-session's 1996 at the start of Phase 17 T1). Lint+typecheck clean (the 2 pre-existing warnings in `src/permissions/shellSemantics.ts` are unrelated to Phase 18).

**ADRs:** none new. Phase 18 is purely additive; all design decisions are captured inline in the plan at `plans/2026-05-23-phase-18-openai-api-server.md` (D1-D14 + OQ1-OQ5 in the "Locked design decisions" table at the top). No foreground refactor, no surface removal, no architectural pivot.

**No binary release cut** — T12 owns the v0.4.0 release. Harness stays on v0.3.4 until then. **AGENTS.md ≡ CLAUDE.md confirmed `IDENTICAL`** post-edit.

## 2026-05-23 — Phase 18 T10 (abort on client disconnect)

**Scope:** T10 of the Phase 18 plan (`plans/2026-05-23-phase-18-openai-api-server.md`). When the OpenAI client disconnects mid-request (closes its fetch context), the in-flight `query()` must abort via `AbortSignal` so the harness stops wasting provider tokens — matches the OpenAI SDK / `openai-python` documented behavior (aborting the `fetch()` context interrupts the backend).
...
**Scope:** T9 of the Phase 18 plan (`plans/2026-05-23-phase-18-openai-api-server.md`). Until now, `resolveModelForRequest` recognized only `harness-default` (and empty string) and threw `InvalidModelError` for every other name — including the explicit catalog entries (`claude-haiku-4-5-20251001`, `gpt-4o`, etc.) that `GET /v1/models` had been advertising. T9 wires the explicit-name branch: known SUPPORTED_MODELS entries now call `resolveProvider(family, model, { harnessHome })` to build a per-request transport. The result is used for **this request only**; runtime state is untouched.
...
**Scope:** T7 of the Phase 18 plan (`plans/2026-05-23-phase-18-openai-api-server.md`). Adds the OpenAI-standard `/v1/models` list endpoint so OpenAI-SDK clients (Open WebUI, LibreChat, the `openai` Python/JS SDKs) can populate their model pickers.
...
**Scope:** T6 of the Phase 18 plan (`plans/2026-05-23-phase-18-openai-api-server.md`). Extends the streaming translator and chunk builders to surface tool execution on the OpenAI wire:
...
**Scope:** Phase 17 closed. New `src/cron/` subsystem + `sov cron` CLI surface + delivery hook + recursion guard rename. T10 docs work: created `docs/07-history/state/2026-05-22-phase-17-cron.md`, updated `CLAUDE.md` + `AGENTS.md` (byte-identical mirror) to point at the new snapshot, committed previously-untracked plan file `plans/2026-05-22-phase-17-cron-scheduled-jobs.md`.
...
**Follow-ups for the run:** T11 (cut v0.3.0 binary release per `docs/05-conventions/cutting-releases.md`).
...
Suite is functional; flakes are documented in `docs/07-history/state/2026-05-22-semantic-suite-revival.md` as separate follow-up work.
...
**Spec:** None — this is restoration + a localized server bug fix, not a new architectural surface. **State snapshot:** `docs/07-history/state/2026-05-22-semantic-suite-revival.md`.
...
**Spec:** `specs/2026-05-22-tui-tool-call-abstraction-design.md`. **State snapshot:** `docs/07-history/state/2026-05-22-tui-tool-call-abstraction.md`.
...
**Files touched:**
- `docs/07-history/state/2026-05-21-ux-fixes-r5.md` — replaced the "Phase 13.5 (scheduled-mission sub-agents) — the original next-phase candidate pre-Phase-16.1" bullet with Phase 21 as the freshest concretely-specced direction; appended a note that Phase 13.5 already shipped.
- `docs/08-roadmap/backlog/post-phase-13-4.md` — corrected the opening paragraph ("The build plan's next phase is Phase 13.5") to point at Phase 21 instead and flagged Phase 13.5 as complete.
...
- `docs/03-cli-reference/usage.md:66, 108, 109` — "(Phase 13.5.)" attribution on the `--state-dir` flag and `mission init/run` subcommands documents which phase shipped the feature; keep.
- `specs/2026-05-21-binary-distribution-design.md:312` — "Orthogonal to Phase 13.5 and any later phase" describes Phase 21's dependency-freeness; keep.
- `specs/2026-05-13-production-harness-roadmap-design.md:741` — historical roadmap context; keep.
- `docs/07-history/state/2026-05-20-m13.md:84` — historical snapshot using past tense; snapshots are frozen records, do not edit.
...
**Files touched:**
- `docs/07-history/state/2026-05-20-m13.md` — new state snapshot (canonical current-state).
...
- `docs/08-roadmap/backlog/post-phase-13-4.md` — "Last sync" header refreshed with M13 close-out; predecessor chain preserved.
- `docs/06-testing/testing-log.md` — this entry.
...
**Smoke:** **4/4 PASS** at `docs/07-history/state/2026-05-20-m13-smoke/` (scripted during T11; this close-out commit doesn't re-run them — they're committed artifacts):
...
**Scope:** Docs-only commit removing `--ui` / `SOV_UI` / `ui.surface` references from README and `docs/03-cli-reference/usage.md`, and cleaning code comments that pointed at `src/ui/terminalRepl.ts:NNN` (now deleted in M13).

**Files touched (no behavior changes):**
- `README.md`, `docs/03-cli-reference/usage.md` — dropped surface-flag rows + REPL deprecation prose; renamed the "REPL UX" section to "Session UX"; replaced "REPL"-as-noun with "session" / "TUI" where it referred to the runtime surface.
...
**Verification:** `grep -rn "terminalRepl\|readline REPL\|--ui repl\|SOV_UI=repl\|ui\.surface=repl" src tests` returns one match — the intentional M13 marker comment in `src/permissions/prompt.ts:3` explaining why the readline-based asker is gone. README and `docs/03-cli-reference/usage.md` are similarly clean of stale surface references.
...
**Why no separate close-out snapshot:** these closures don't represent a milestone or substantive design change — they're a hygiene pass on the backlog. The M12 close-out (`docs/07-history/state/2026-05-19-m12.md`) remains the canonical "current state" snapshot; this testing-log entry + the backlog header is sufficient record.
...
- `README.md` + `docs/03-cli-reference/usage.md` — `--ui` flag descriptions updated to note the deprecation and the `SOV_NO_DEPRECATION_WARNING` env-var escape hatch.
...
**Smoke:**
- `docs/07-history/state/2026-05-19-m12-smoke/run-smoke.ts` — 6 scenarios pinning the predicate end-to-end:
...
**Docs updated:** `docs/08-roadmap/backlog/post-phase-13-4.md` (#41 marked closed with strikethrough + "Last sync" line refreshed); `docs/07-history/state/2026-05-19-m11-5.md` (open-backlog list shortened, #41 struck through); CLAUDE.md/AGENTS.md (state-doc table row revised to reflect 8 open items, mirror verified byte-identical).
...
- `docs/08-roadmap/backlog/post-phase-13-4.md` — added Item 45 under a new "P3 (TUI architecture)" subsection; updated "Last sync" line to 2026-05-19 with the new open backlog list (17, 29, 38, 39, 41, 43, 44, 45).
...
- (pending close-out commit) — state snapshot, audit report, smoke transcripts, ADRs M11-01..03, backlog header, CLAUDE.md/AGENTS.md mirror, LOW-fix polish in `docs/05-conventions/sov-upgrade.md` + `src/server/commandContext.ts` (M11 opt-out messaging) + `src/cli/tuiLauncher.ts` (consistent warning text)

**Audit:** Independent Opus subagent reading the code, not by recall, per Postmortem Rule 3. Report at `docs/07-history/state/2026-05-17-m11-parity-audit.md`. Disposition: **PASS-with-followups** (0 CRITICAL / 0 HIGH / 0 MEDIUM / 1 LOW fixed inline). Verified that:
...
- Full transcripts at `docs/07-history/state/2026-05-17-m11-smoke/` (13 scenarios × stdout + stderr + exit code + 1 real-API rerun + README summary table). Cost total: ~$0.005.

**Adaptation note:** The spec called for a single interactive Haiku 4.5 session running ~10 dispatcher commands inside the TUI. The autonomous-execution environment cannot drive an interactive TUI through arbitrary keystrokes, so the smoke split into (a) the boot-decision scenarios (verifying which surface gets reached) + (b) the dispatcher-command rerun (verifying the commands themselves work end-to-end against the live API). Both adaptations are documented in `docs/07-history/state/2026-05-17-m11-smoke/README.md`.
...
- Rule 3 — audit is independent + mechanical + checked-in. The audit report at `docs/07-history/state/2026-05-16-tui-parity-audit.md` is the Rule 3 attestation.
...
- **`docs/03-cli-reference/usage.md`** — Updated the `--ui tui` flag coverage table. The two M8 capture/replay rows moved from `**Warn**` to `Wired (M8)` with status text noting the mutex behavior. The four remaining `**Warn**` rows (transcript / agent / state-dir / verbose) carry the same milestone targets as before.
...
- [x] `docs/03-cli-reference/usage.md` reflects the new wiring + remaining warnings
...
- [x] 9 prereq boxes flipped (rows 14, 16, 17, 18, 19, 20, 21, 22, 24) — verified `grep 'M8 — 2026-05-16' docs/08-roadmap/backlog/phase-16-rebuild-prereqs.md` returns 9 lines; zero `[ ]` remaining
...
- [x] State snapshot `docs/07-history/state/2026-05-16.md` created (follows 2026-05-15 template structure)
...
**Status:** Phase 16.1 M8 closed. Phase 16 rebuild prerequisites at 24/24 — every subsystem listed in `docs/08-roadmap/backlog/phase-16-rebuild-prereqs.md` is wired. M9 visual polish + M10 parity audit are the gates to M11 default flip. User has not yet given the go-ahead to start M9 planning.
...
**Scope:** Final task of M7 (Hermes-layer parity group). Integration smoke test drives all six subsystems (MCP, DaemonEventBus, trace, trajectory, learning, review) through one end-to-end scenario via the public POST /sessions + POST /sessions/:id/turns + SSE drain pattern. Six prereq boxes (rows 2, 5, 10, 11, 12, 13) flipped in `docs/08-roadmap/backlog/phase-16-rebuild-prereqs.md`; backlog item #28 (DaemonEventBus → server-mode TaskManager) closed; two new backlog items added (#38 reviewAutoPromote* parentToolContext snapshot gap + #39 Go TUI mirror for SessionSummaryEvent). Six ADRs added to DECISIONS.md (M7-01, M7-02, M7-03, M7-05, M7-06, M7-08). M7-04 and M7-07 are scope/sequencing decisions, noted in the state snapshot, not promoted to ADRs. Old `docs/07-history/state/2026-05-14.md` archived as `docs/07-history/state/archive/2026-05-14-pm.md` (the existing `2026-05-14.md` archive — covering M4 + M5 + M5.1 — is preserved untouched). New canonical snapshot at `docs/07-history/state/2026-05-15.md`. CLAUDE.md / AGENTS.md state-snapshot pointer updated; byte-identical mirror preserved.
...
2. **Prereq box flips.** Rows 2, 5, 10, 11, 12, 13 in `docs/08-roadmap/backlog/phase-16-rebuild-prereqs.md` updated from `[ ]` to `[x] (M7 — 2026-05-15)`. Format matches the M4/M5/M6 precedent (`(MX — YYYY-MM-DD)`). Remaining `[ ]` count drops from 15 to 9 — all in M8 polish surfaces.

3. **Backlog #28 closure.** Item #28 in `docs/08-roadmap/backlog/post-phase-13-4.md` marked complete with the T2 commit SHA (`bfaeaad`). Priority-order line at the top of the file updated. The "Last sync" paragraph rewritten to reflect the M7 close-out (1965/1965 tests, items 1-11, 14-16, 18-23, 25-28, 31-37 closed; open: 17, 29, 30, 38, 39).
...
5. **Six ADR stubs added to `DECISIONS.md`.** M7-01 (Per-session subsystems on Runtime Map), M7-02 (Trace writer rebuilt on compaction), M7-03 (Trajectory disposal-driven, not per-turn), M7-05 (Review manager same lifecycle as trace; scheduler-dispatched), M7-06 (DaemonEventBus plumbing-only), M7-08 (`runtime.dispose()` order — per-session → MCP → approvals → sessionDb). Each links back to the T1–T6 commits that implemented the decision and to the plan at `plans/2026-05-15-phase-16-1-m7-hermes-layer.md`. M7-04 (direct-call observation pattern) and M7-07 (real-Anthropic smoke deferred) are scope/sequencing decisions, noted in the snapshot but not promoted to ADRs.

6. **State snapshot.** New `docs/07-history/state/2026-05-15.md` (~200 lines) follows the M6 snapshot template: HEAD SHA + suite numbers + sov binary version + what shipped (newest-first table) + per-task narrative + scope decisions noted + ADRs added + what does NOT work / known gaps + behavioral notes + what's open / what's next + manual smoke status + resolutions of prior open questions + pointers to deeper M7 narrative. Old `docs/07-history/state/2026-05-14.md` archived to `docs/07-history/state/archive/2026-05-14-pm.md` (a non-colliding name — the existing `2026-05-14.md` archive covers M4 + M5 + M5.1, the new `-pm` one covers M6 + 2026-05-15 hardening + autonomous smoke + PM #32/#37). `git mv` used so the move shows in git as a rename.
...
- `docs/08-roadmap/backlog/phase-16-rebuild-prereqs.md` — 6 box flips
- `docs/08-roadmap/backlog/post-phase-13-4.md` — #28 closed + 2 new items (#38, #39) + priority-order list updated + "Last sync" paragraph rewritten
- `DECISIONS.md` — 6 new ADR stubs prepended (M7-01, M7-02, M7-03, M7-05, M7-06, M7-08)
- `docs/07-history/state/2026-05-15.md` — new (~200 lines)
- `docs/07-history/state/2026-05-14.md` → `docs/07-history/state/archive/2026-05-14-pm.md` (git mv)
...
**Scope:** Closed the M5 milestone group — 3 prereq boxes (hooks system / permission prompt UI / sub-agent scheduler) flipped from `[ ]` to `[x] (M5 — 2026-05-14)` in `docs/08-roadmap/backlog/phase-16-rebuild-prereqs.md`. Ten T-commits (T1–T9 plus T4-cleanup) landed the wiring; T10 (this commit) is integration smoke + docs close-out.
...
**Scope:** Verified the harness was unaffected by the docs-only modernization of the deprecated `sov chat` keyword. Bulk `replace_all` across `docs/03-cli-reference/usage.md`, `docs/02-architecture/runtime-architecture.md`, `evals/README.md`, and one occurrence in `docs/08-roadmap/backlog/phase-16-rebuild-prereqs.md`. The deprecation row in `docs/03-cli-reference/usage.md:108` is preserved (documents the deprecation itself); historical files (`testing-log.md`, `CHANGELOG.md`, `state/archive/`, `plans/`, `specs/`, `post-phase-13-4.md`) left untouched.
...
**Scope:** Verified the harness was unaffected by the docs-only Progressive Disclosure restructure (commits `5264aaa`, `2c3ec74`, `d7e2201`): new `docs/05-conventions/` directory, new `docs/01-overview/design-principles.md`, lean CLAUDE.md/AGENTS.md (~100 lines, down from 216), pruned README.md, stale state-file path fixes.
...
**Scope:** Drove 8 of the 11 manual smoke scenarios from `docs/07-history/state/2026-05-14.md` autonomously without the user typing into the TUI. The remaining 3 (#2 resume hydration render, #7 `--max-tokens 100` truncation against real Anthropic, #11 legacy REPL visual) still require the user's eyeball.
...
**Result:** M4 critical correctness group complete. 3 of 24 prereq boxes flipped to `[x]`; 21 remain for M5+. State snapshot rolled to `docs/07-history/state/2026-05-14.md`; `docs/07-history/state/2026-05-13.md` archived. `usage.md` documents the `--ui tui` flag-coverage matrix. `DECISIONS.md` records ADR M4-01 (hydrate-then-subscribe). CLAUDE.md + AGENTS.md updated (mirror invariant holds).
...
**Follow-ups:** M4 — 24-prereq Group 1 (critical correctness). Spec §10 milestone M4 and `docs/08-roadmap/backlog/phase-16-rebuild-prereqs.md`.
...
1. **Phase 16 revert** (force-push from a worktree that reset to commit `e9d5445`, the last green pre-Ink-TUI state). Discarded Phase 16.0b (Ink TUI) + Phase 16.0c (Wave 1 slash dispatch on Ink). Preserved at `origin/archive/ink-tui-2026-05-12` (commit `fe0f44b`). Rationale + durable rules in `docs/07-history/postmortems/2026-05-12-phase-16-revert.md`.
...
- New `docs/07-history/state/2026-05-12.md` (canonical close-out snapshot).
...
- `docs/03-cli-reference/usage.md` — added subcommand entries for `dispatch`, `mission init`, `mission run`, `daemon`; added `--agent` and `--state-dir` flags; documented `sov chat` deprecation.
- New `docs/08-roadmap/backlog/phase-16-rebuild-prereqs.md` (see Q7 below).
...
- Q7 deferred Phase 16 rebuild prereqs: captured in new `docs/08-roadmap/backlog/phase-16-rebuild-prereqs.md`.
...
**Semantic audit:** Phase 16.0a adds no agent-facing tools, slash commands, permission rules, or context surfaces — all code is headless daemon infrastructure. Semantic suite unchanged at 58/58. Audit noted in `docs/06-testing/semantic-testing.md`.
...
- Result:
  ...
  - Two new semantic cases under existing categories: `tools.mcp-tool-search-then-invoke` (end-to-end MCP discovery + invocation) and `permissions.mcp-permission-rule-blocks-server` (mcp__server prefix denial). Inventory headline updated 32 → 34 in `docs/06-testing/semantic-testing.md`.
- Regressions / follow-ups:
...
- Result:
  ...
  - Two new semantic cases land under a new `hooks` category: `hook-pretooluse-blocks-bash` and `hook-posttooluse-additional-context`. Inventory headline updated 30 → 32 in `docs/06-testing/semantic-testing.md`.
...
- Regressions / follow-ups:
  ...
  - Backlogs (`docs/08-roadmap/backlog/archive/phase-10-5.md`, `docs/08-roadmap/backlog/archive/post-phase-10-5-repl.md`) intentionally left as historical records — they describe specific testing sessions in 2026-04-27 and shouldn't be edited retroactively.
...
- Commands:
  ...
  - `docs/06-testing/semantic-testing.md` updated: headline 38 → 39, slash-command pipeline 5 → 6, new row in the coverage table.
...
- Commands:
  ...
  - Doc updates same set: `docs/06-testing/semantic-testing.md` headline 39 → 41 + new sections + 4 mapping-table rows; `docs/03-cli-reference/usage.md` headline 38 → 41, new "Secret Redaction (Defense in Depth)" subsection under Tool Permissions, new "Default-bundle skills" subsection under Skills; `README.md` headline 38 → 41 with the two new categories named; `CLAUDE.md` + `AGENTS.md` 38/38 → 41/41, unit 1130/1130 → 1181/1181, plus a new sentence noting the redactor and security-audit skill location; `CHANGELOG.md` new top entry.
...
- Result: Suite headline 51 → 54. `docs/06-testing/semantic-testing.md` updated in same commit.
- Regressions / follow-ups: None.
...
- Files updated: `CLAUDE.md`, `AGENTS.md`, `README.md`, `DECISIONS.md`, `CHANGELOG.md`, `docs/02-architecture/runtime-architecture.md` (new review pipeline section + REVIEW_ONLY_TOOLS description + updated semantic counts), `docs/03-cli-reference/usage.md` (new `/review` slash command table + `settings.review.*` config block + updated semantic count), `docs/04-extending/extending.md` (review-* agent special role + REVIEW_ONLY_TOOLS note), `docs/06-testing/semantic-testing.md` (stale count fixes), `docs/06-testing/testing-log.md` (this entry + all prior close-out entries).
...
- Regressions / follow-ups: None. `docs/08-roadmap/backlog/archive/phase-10-5.md` and `docs/08-roadmap/backlog/archive/post-phase-10-5-repl.md` are historical backlog records and intentionally left unchanged.
...
**Files:** `src/compact/microcompact.ts`, `src/core/query.ts`, `tests/compact/microcompact.test.ts`, `docs/08-roadmap/backlog/post-phase-13-4.md`.

## 2026-05-08 — Memory retrieval gaps spec

**Scope:** Added `specs/memory-retrieval-gaps.md` as a draft anchor for future memory retrieval improvements. The spec records the gaps identified while comparing the harness's current memory system with the 2026-05-08 memory article review: need detection, ranking/packing, semantic retrieval, temporal validity, write redaction, retrieval evals, API surface, cross-scope policy, and observation-corpus retrieval.
