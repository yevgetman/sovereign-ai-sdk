...
47. **Retire dead `packages/tui/internal/components/transcript.go`** — the inline-mode refactor in round 5 routes all permanent content through `m.print` + `tea.Println` and all live content through `LiveRegion`. The Transcript struct + its viewport/`lines[]`/`toolCards` map are unused production code, kept only because `recomputeLayout`'s legacy `SetSize` call and `New()`'s initializer still reference them. Cleanup: delete `transcript.go` + `transcript_test.go` (~450 LoC); drop the `transcript` field + initialization + the SetSize call. Tests should already pass with no transcript reference once `New()` is trimmed. Effort: ~30 min. Risk: low (no consumers). [ux-fixes round 5 close-out 2026-05-21]

P3 (Phase 21 follow-up):
48. **Phase 21 M2 — GitHub Actions release automation + optional code-signing.** The manual `bun run release v0.x.y` flow shipped in Phase 21 M1 works end-to-end (~3 min wall time for compile + ~30s for upload). M2 moves the release pipeline into a GitHub Actions workflow inside `sov-releases` (or a private workflow that pushes to it via deploy key); optionally adds macOS code-signing + notarization (requires Apple Developer Program enrollment, ~$99/yr, removes the Gatekeeper "cannot verify the developer" warning); optionally adds a homebrew tap. Trigger: when manual releases start feeling like friction. Spec already covers M2 in `docs/specs/2026-05-21-binary-distribution-design.md` §2 + §10. Effort: ~3-4 wall hours per the spec. [Phase 21 M1 close-out 2026-05-22]
