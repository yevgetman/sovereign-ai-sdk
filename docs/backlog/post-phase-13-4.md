...
**Last sync:** 2026-05-19 — Post-M11.5 polish + docs/tests audit pass. Two UX issues addressed (`uxissue1.png` popup-below-input, `uxissue2.png` Enter-selects-with-args-guard) in commit `ca8f670`. Conventions doc `tui-ux-patterns.md` extended with the inline picker card pattern and the slash-autocomplete dropdown behavior; iteration narrative extended through M11.14–18 + formal M11.5. Three new explicit tests added in `slashautocomplete_keys_test.go` (Enter-fills-and-submits, args-preservation, popup-below-prompt layout). Item 46 filed for the `/theme` migration follow-up (previously tagged F1 in the spec). Predecessor (2026-05-19 M11.5 close-out): Picker-driven `/model`, `/resume`, `/export` emit `pickerOpen` side-effect; Go TUI renders inline `PickerCard`; T8 spacing fix; 3 ADRs M11.5-01..03; suite 2061/2061 green; real-Anthropic smoke 2/2 pass. Open backlog: **17, 29, 38, 39, 41, 43, 44, 45, 46**. Predecessor (2026-05-19 popup-discoverability): staticEntries 4 → 25 entries; Item 45 filed. Predecessor (2026-05-17 M11): Default-flip shipped; 3 ADRs M11-01..03; suite 2033/2033. Next milestone: M12 — terminalRepl deprecation. Predecessor sync (M10.5): slash-command dispatcher shipped; #40 closed. New M10-cascading items #41 (createClearedChildSession), #43 (createDefaultMemoryManager), #44 (appendProjectLocalPermissionRule persistence) added as deferred-from-M10.5 P2/P3 items. Predecessor (M10): independent mechanical parity audit per Postmortem Rule 3; 2 HIGH gaps fixed inline, 1 HIGH scope-bounded as CLI-only, 1 HIGH deferred as new backlog item #40 BLOCKING M11; M9.5 theme regression also fixed inline. Items 1-11, 14-16, 18-23, 25-28, 30-37 closed across sixteen batches. Items 25-30 added 2026-05-14 from Phase 16.1 M5 close-out + M5.1 review. Items 31-33 added 2026-05-14 from Phase 16.1 M6 final whole-branch review. Items 34-36 added 2026-05-15 from M6 pre-smoke critical bug-hunt. Item 37 added 2026-05-15 from M6 smoke pre-flight. Items 38-39 added 2026-05-15 from Phase 16.1 M7 T5/T6 reviews. **Item 40 added 2026-05-16 from Phase 16.1 M10 parity audit slice 1 HIGH — server-side built-in slash-command dispatcher route; CLOSED in M10.5.** Items 18-24 originated from the 2026-05-07 ad-hoc 7-agent REPL soak (41/41 cases passed). Items 25-30 originated from the Phase 16.1 M5 T10 / M5.1 reviews. Items 31-33 originated from the Phase 16.1 M6 final whole-branch review. Items 34-36 originated from the M6 pre-smoke deep-dive review (three parallel Opus reviewers). Item 37 originated from M6 smoke pre-flight. Items 38-39 originated from Phase 16.1 M7 reviews. **Item 40 originated from the Phase 16.1 M10 parity audit (4 parallel Opus subagents reading 92 imports of `terminalRepl.ts` literally).**
...
41. `createClearedChildSession` server wiring — backs /clear, /rollback **[M10 audit 2026-05-16, deferred from M10.5 per ADR M10-04]**
43. `createDefaultMemoryManager` + `resolveProjectScope` server wiring — backs /memory **[M10 audit 2026-05-16]**

P3 (small persistence gap):
44. `appendProjectLocalPermissionRule` server-side persistence path — "yes & remember (project)" approval persistence **[M10 audit 2026-05-16]**

P3 (TUI architecture):
45. TUI slash-command discovery endpoint — `staticEntries` in `slashautocomplete.go` is a compile-time hand-mirror of TS `COMMAND_REGISTRY`; add `GET /sessions/:id/commands` so the popup syncs automatically **[2026-05-19]**

P4 (M11.5 follow-up):
46. Migrate `/theme` to use `pickerOpen` side-effect — currently uses dedicated client-side dispatch (one of the original 4 `staticEntries`). Consistency-only; no user impact today **[M11.5 close-out 2026-05-19, formerly tagged F1 in the spec]**
