...
61. **`workflow_run` TOOL not wired into the tool pool.** **OPEN (P3).** The agent-invocable surface — `src/tools/WorkflowRunTool.ts` (`buildWorkflowRunTool({ runtime })`) + its `SUBAGENT_EXCLUDED_TOOLS` entry — landed as scaffolding, but it is NOT in `assembleToolPool` (`src/server/runtime.ts`), so the model can't call `workflow_run` yet. The blocker is a runtime self-reference: the tool needs the live `Runtime`, but the pool is assembled (runtime.ts:801) BEFORE the runtime object exists. **Fix:** mirror the `harnessInfoSnapshot` lazy-thunk pattern (a `getRuntime` ref filled post-construction), or push `buildWorkflowRunTool({ runtime })` into `runtime.toolPool` after the runtime object is built (the array is already mutated in place by the reload hooks at runtime.ts:1455). Until then, workflows run via `sov workflow run` (CLI) + `/workflow` (slash). Surfaced 2026-06-15. Likely areas: `src/server/runtime.ts`, `src/tool/registry.ts`, `src/tools/WorkflowRunTool.ts`.
...
- Source: Learning-loop spike Phase 1 close-out (2026-06-04); spec `docs/specs/2026-06-03-portable-learning-layer-adapter-1-design.md` D1/D15
