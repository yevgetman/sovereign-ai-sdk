...
P3 (CI maintenance):
49. **Node-20 GitHub Actions deprecation.** **OPEN (P3).** The release workflow (`.github/workflows/release.yml`) pins `actions/checkout@v4`, which runs on Node 20. GitHub forces these actions to Node 24 starting **2026-06-16** and removes the Node-20 runner on 2026-09-16 — the deprecation annotation has surfaced on every release run since ~v0.6.0. Action: bump `actions/checkout` (and any other Node-20-based actions) to a Node-24-compatible major before 2026-06-16, or set `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true` on the runner as an interim. Low effort, pure CI hygiene — no runtime impact. (Originally noted in the frozen `docs/state/2026-05-25-phase-21-m2.md` follow-ups; promoted here so it lives in the living backlog.)
