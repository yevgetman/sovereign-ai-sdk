...
**The SDK substrate (`createAgent`) and the open/proprietary boundary.** As of the SDK open-core extraction (2026-06-30), the turn loop is driven through an importable open-core SDK rather than ad-hoc per-surface wiring. `createAgent()` (`src/agent/createAgent.ts`, re-exported from the `sdk.ts` barrel as `@yevgetman/sov`'s `./sdk` entry) is the standing-config + per-turn-override (`PerTurn`) front door over `query()`; its `run()` yields `query()`'s `StreamEvent | Message` stream unchanged and in order (the stream-passthrough invariant). It composes **only open primitives** — `query()`, `resolveProvider`, the `Tool`/`ToolContext` shapes, and five injected ports (`SessionStore`, `TranscriptStore`, recall, observe→`LearningObserverPort`, trace) whose proprietary impls stay closed; absent a `SessionStore`, a turn touches **no disk** (the embeddable default). Every in-process surface (gateway/OpenAI turn-exec, `sov mission run`, cron, channels, the sub-agent scheduler) runs on `createAgent()`. A **file-level boundary lint** (`bun run boundary`, part of `bun run lint`) enforces zero open→proprietary imports against an explicit exception manifest (`scripts/boundary-manifest.json`). The gateway wire protocol is Contract #2, a pure `.d.ts` `sov-protocol` surface. Full rationale + the §15 acceptance gate: [`specs/2026-06-29-sdk-open-core-extraction-design.md`](specs/2026-06-29-sdk-open-core-extraction-design.md).
