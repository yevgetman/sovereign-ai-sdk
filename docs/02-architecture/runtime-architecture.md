...
5. The turns route (`src/server/routes/turns.ts`) drives the turn through the open-core SDK assembler `createAgent().run()` (`src/agent/createAgent.ts`), which calls `query()` in `src/core/query.ts`; `query()` calls the selected `LLMProvider.stream()` with internal content-block messages and segmented system prompt. Because `run()` yields `query()`'s stream unchanged (the stream-passthrough invariant), steps 6–9 are identical whether a turn is SDK-driven or, in older code, called `query()` directly.
...
**Reasoning depth (`/effort`).** `ProviderRequest` carries a provider-neutral `effort?: ReasoningEffort` (`off | low | medium | high | max`), threaded from `QueryParams.effort` → `query()` → `provider.stream(req)`. The pure module `src/providers/effort.ts` owns the level→wire translation; each adapter forks it in `buildKwargs`: Anthropic → `thinking.budget_tokens` (+ raised `max_tokens`, dropped `temperature`, interleaved-thinking beta), OpenAI reasoning models → `reasoning_effort`, sov → `enable_thinking`. A `modelSupportsReasoning` gate means a non-reasoning model never gets a thinking parameter. ollama reasoning is gated off for v1 (its native `think: true` switch differs from sov's `enable_thinking` chat-template flag and needs per-model capability data not yet wired), so `/effort` is a no-op on ollama models — a planned fast-follow. The level is per-session mutable runtime state — `runtime.effort` (parallel to `runtime.model`), seeded from the `thinking.effort` config default and mutated live by the `/effort` command via `CommandContext.setEffort`. The default `off` leaves the request body byte-identical to a no-thinking turn. Spec: [`specs/2026-06-09-effort-reasoning-depth-design.md`](specs/2026-06-09-effort-reasoning-depth-design.md).

## The SDK substrate (`createAgent`) & the open/proprietary boundary

As of the SDK open-core extraction (2026-06-30), the turn loop is driven through an importable **open-core SDK** rather than ad-hoc per-surface wiring. This is the substrate the whole harness composes over: the runtime is now a thin wrapper around `createAgent()` plus the proprietary packages.

`createAgent()` (`src/agent/createAgent.ts`, re-exported from the `src/sdk.ts` barrel — the `./sdk` entry of the `package.json` `exports` map alongside `./protocol`) is the standing-config (`AgentConfig`) + per-turn-override (`PerTurn`) front door over `query()`. Its `run()` yields `query()`'s `StreamEvent | Message` stream **unchanged and in order** (the stream-passthrough invariant), and returns a structured `RunResult`. `query()` itself is untouched (Invariant #1) — `createAgent` is a composition over it, not a fork. It adds only the three net-new engine responsibilities the spec (§5.2) names: per-turn override merging, optional persistence via the store ports, and an `observe`→`LearningObserverPort` adapter.

It composes **only open primitives** — `query()`, `resolveProvider`, the `Tool`/`ToolContext` shapes (assembled by the open `buildToolContext()`, `src/tool/buildToolContext.ts`), and **five injected ports** whose proprietary implementations stay closed:

- **`SessionStore`** (`src/persistence/sessionStore.ts`) — persistence. `createInMemorySessionStore()` is the default; the `bun:sqlite` `SessionDb` (`src/agent/sessionDb.ts`) is a *closed* implementation bound behind it. **Absent a `SessionStore`, a turn touches no disk** — the embeddable default.
- **`TranscriptStore`** (`src/persistence/transcriptStore.ts`) — transcript writes; a no-op default (`createNoopTranscriptStore`).
- **recall** (`RecallTurn` → `RecallResult`), **observe** (`LearningObserverPort` / `ObserveInput`), and **trace** (`TraceEvent`) — the learning/observability seams.

**Every in-process surface runs on `createAgent()`** — the gateway/OpenAI turn-exec, `sov mission run`, cron, channels, and the sub-agent scheduler — each migrated behind a field-level parity test. Two parity *gains* rode along: cron and channels picked up microcompaction (and cron, transcript writes) through the new ports, closing a gap the old per-surface wrapper structurally could not carry.

The open/proprietary line is **machine-enforced**, not conceptual: a **file-level boundary lint** (`bun run boundary` = `depcruise` against `.dependency-cruiser.cjs`, part of `bun run lint`, and re-run in `ci.yml`) fails the build on **any** open→proprietary import, measured against an explicit exception manifest (`scripts/boundary-manifest.json`). The `src/sdk.ts` barrel is itself classified open, so it actively self-gates — re-exporting anything proprietary fails the lint. The gateway wire protocol ships as **Contract #2**, a pure-`.d.ts` `sov-protocol` surface (`src/protocol/`) — the wire event/endpoint types plus a fetch-based typed client, the single source of truth the gateway, the Go TUI, and external clients all adopt. Full rationale + the §15 acceptance gate: [`specs/2026-06-29-sdk-open-core-extraction-design.md`](specs/2026-06-29-sdk-open-core-extraction-design.md).
...
Session persistence lives in `src/agent/sessionDb.ts` and uses `bun:sqlite` with WAL, schema migrations, FTS5, and a jittered busy retry wrapper. Since the SDK extraction (2026-06-30) `sessionDb.ts` is a **closed implementation behind the open `SessionStore` port** (`src/persistence/sessionStore.ts`): the harness wrapper binds the SQLite store, but `createAgent()` only ever sees the port interface. The default when no store is injected is `createInMemorySessionStore()` — and a turn run with *no* store touches no disk at all (the embeddable default; see "The SDK substrate" above).
...
See [`usage.md`](docs/03-cli-reference/usage.md#mcp-servers) for the full remote-config + `SOV_MCP_*` auth reference.
...
**Disclosure surface.** `buildRuntime` returns every *discovered* plugin (active and inert) on `runtime.plugins` and the active plugins' commands on `runtime.pluginCommands`; `buildPluginSnapshots` (`src/plugins/snapshot.ts`) projects them into the rows `HarnessInfo` and `/plugins list` (`src/commands/pluginOps.ts`) render — one shared status precedence (`tampered` > `needs-consent` > `disabled` > `active`) so the two surfaces agree. Config is the opt-in `plugins: { enabled?, disabled? }` block (`.strict()`, `src/config/schema.ts`): `enabled` is an allow-list, `disabled` wins, an absent block activates every consented plugin. `/plugins install` is TTY-only (the consent prompt is injected via `ctx.confirm`; absent on server/TUI surfaces, which refuse). See [`usage.md`](docs/03-cli-reference/usage.md#plugins) and [`extending.md`](docs/04-extending/extending.md#authoring-a-plugin).

## OpenAI HTTP Server

`src/openai/` carries an OpenAI-compatible HTTP API surface (Phase 18). It sits parallel to the TUI / drive / dispatch / cron surfaces — all five share the same `buildRuntime()` factory, but each owns its own entry point. The OpenAI server boots via `sov serve`, runs on its own `Bun.serve` binding (default port 8765, configurable via `--port` / `SOV_OPENAI_PORT` / `openaiServer.port`), and is fully stateless per request: each call drives one turn through the open-core SDK assembler `createAgent().run()` over the request body's full `messages[]` (the wire shape carries history natively, so nothing is hydrated from disk). Bearer auth gates `/v1/*` (the API key is required at boot — no anonymous mode); `/health` is auth-exempt for container liveness probes.
...
**The SDK driver (`createAgent` + `drainRunner`).** The scheduler drives a child turn through the open-core SDK assembler `createAgent()` (`src/agent/createAgent.ts`) — the same turn-driver every in-process surface (gateway, OpenAI, cron, channels, missions) now runs on, rather than a bespoke per-surface wrapper. `createAgent().run()` builds the user message from the string prompt, wires the per-turn `query()` params, and yields `query()`'s stream unchanged (Invariant #1 — `query()` is never forked). `drainRunner` (`src/runtime/scheduler.ts`) is the small kept stream-drain helper that collects that stream into the `RunResult` the scheduler tail consumes: the final assistant message, iteration count, tool-call count, and parent-child lineage carry. Its result shape is pinned by `src/runtime/executorPort.ts` (so the subscription-executor subprocess branch can return a byte-compatible result). The former standalone `AgentRunner` wrapper class (`src/runtime/agentRunner.ts`) was **removed** once every surface re-seated onto `createAgent()` — it had zero callers; only `drainRunner` and `RunResult` survive from that lineage.
...
**Subscription executor (opt-in, off by default — spike).** A single scheduler branch (`src/runtime/subprocessExecutor.ts`, gated by `subscriptionExecutor.enabled` + `agent.role === 'subscription-executor'`) hands a delegated task to a **headless `claude -p` subprocess** instead of the native `createAgent` loop. The subprocess runs Claude Code's own agentic loop and returns a result in the **exact shape `drainRunner` produces**, so the whole scheduler tail (summary, trajectory, `on_delegation` hook, review-fork, SSE events) is byte-unchanged. Per-tool work from the subprocess `stream-json` is **replayed into the learning corpus + trace** — observations + `distinctToolNames` are **canonicalized to the native tool vocabulary** (`Read`→`FileRead`, `Write`→`FileWrite`, `Edit`→`FileEdit`, `file_path`→`path`, drop `Bash.description`; unmapped tools pass through) so delegated turns co-cluster with native ones, while `messages[]` + the trace stay verbatim. Permission posture is safe-by-default (`--permission-mode`, `bypassPermissions` rejected at config-parse). It is wired **only** to the interactive sub-agent delegation seam — **not** cron / channels / the gateway — because driving a Claude *subscription* credential as an automated / multi-tenant backend crosses the personal-use ToS boundary; only attended personal use of the local `claude` install is defensible. See [`specs/2026-06-08-subscription-executor-spike.md`](specs/2026-06-08-subscription-executor-spike.md) and [`usage.md`](docs/03-cli-reference/usage.md#subscription-executor-opt-in).
...
Before this phase the learning loop was **open**: the Phase 13.4 pipeline synthesized instincts to disk but nothing ever read them back into the main agent. The spike closes the loop and seats the whole learning concern behind a portable contract (ADR H-0010; spec at `specs/2026-06-03-portable-learning-layer-adapter-1-design.md`; plan at `plans/2026-06-03-learning-loop-spike-phase-1.md`).
...
- `src/agents/` for agent definitions (loader + types + global exclusion set), `src/agent/createAgent.ts` for the open-core SDK assembler (the turn driver every surface runs on), and `src/runtime/` for the sub-agent runtime (scheduler, `drainRunner`, semaphores)
...
- `src/sdk.ts` for the open-core SDK barrel (Contract #1 — `createAgent` + the open tool/provider/port surface) and `src/protocol/` for the gateway wire types + typed client (Contract #2); both are gated open by `scripts/boundary-manifest.json` — an open-core file may not import proprietary code, enforced by `bun run boundary`
- `src/persistence/` for the injectable `SessionStore` / `TranscriptStore` ports (in-memory + no-op defaults) the SDK persists through; the SQLite `SessionDb` (`src/agent/sessionDb.ts`) is the closed implementation bound behind the port

See `docs/04-extending/extending.md` for concrete recipes.

---

## Read next

- [`docs/02-architecture/subsystems-overview.md`](docs/02-architecture/subsystems-overview.md) — the component-level atlas (where each piece of the flow lives).
- [`docs/01-overview/design-principles.md`](docs/01-overview/design-principles.md) — the 9 locked principles behind these choices.
- [`docs/04-extending/extending.md`](docs/04-extending/extending.md) — concrete recipes for adding to the runtime.
- [`docs/03-cli-reference/usage.md`](docs/03-cli-reference/usage.md) — how the runtime is driven day to day.
