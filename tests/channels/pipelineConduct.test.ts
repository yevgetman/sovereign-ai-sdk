// Task 10 — conduct threading through `buildChannelAgentConfig`.
//
// Mirrors the recall conditional-spread precedent in `pipeline.reseat.test.ts`
// (the exact unit-level `buildChannelAgentConfig` field-parity harness): a
...
/** The minimal standing input `pipeline.reseat.test.ts` uses, plus any override
 *  (here: a `conduct` sentinel). Distinct sentinel objects so the assertion can
 *  prove reference-identity pass-through, not a copy. */
function makeInput(overrides: Partial<ChannelAgentConfigInput> = {}): ChannelAgentConfigInput {
