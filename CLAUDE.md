...
Phases 0 through 10 complete (2026-04-26). Phase 10.5b–e (REPL polish) complete (2026-05-03). Phase 11 (shell hooks) and Phase 12 (MCP client + deferred tool loading) complete (2026-05-03). Phase 9.6 (skill trigger rigor), Phase 12.5 (tool observation envelope), Phase 12.6 (context budget audit + `/context-budget`), Phase 13.1 (trajectory capture — the Sovereign moat), Phase 10.7 (profile system: `-p/--profile`, `sov profile [list|create|use|show|import-default]`, profile-aware path resolution + atomic PID lock helper), Phase 10.5 part 1 (operational traces + `sov trace show` + multi-heuristic loop detection), and Phase 10.6 part 1 (local-model router: `--provider router`, deterministic classifier, audit log, `route_decision` StreamEvent) shipped 2026-05-04. Phase 10.5 part 2 is complete (2a + 2b-i + 2b-ii + 2c). Phase 10.6 part 2a (router polish — REPL banner for `route_decision`, recent-error tracking that feeds classifier triggers, splash cosmetic) shipped 2026-05-05. Eval-runner `--capture <dir>` / `--replay <dir>` CLI follow-up shipped 2026-05-05 (also `sov chat --capture-fixture <path>` / `--replay-fixture <path>`). Phase 10.6 part 2b interactive `ask` mode shipped 2026-05-05. Phase 10.8 (default bundle / bundleless invocation + `sov init`) shipped 2026-05-05 — `sov` no longer requires a bundle on disk; bundle resolution falls through to a vendor-neutral shipped default at `bundle-default/`, with user-override at `<harness-home>/default-bundle/`. **Phase 10 lane fully closed (2026-05-05).** Two items originally listed under 10.6 part 2b — capability profile lookup and per-lane concurrency guards — were re-scoped into Phase 13 (sub-agent runtime, build items 4 and 10) where they have a real consumer; nothing remains deferred under a 10.x label. Defense-in-depth secret redactor (Write/Edit/NotebookEdit InputTransformer) and `/security-audit` skill shipped 2026-05-05 — see `src/permissions/secretRedactor.ts` + `bundle-default/skills/security-audit.md`. Semantic test suite is 41/41; unit suite is 1181/1181. See [`~/code/sovereign-ai-docs/harness/docs/runtime/phase-10x-status.md`](~/code/sovereign-ai-docs/harness/docs/runtime/phase-10x-status.md) for the full 10.x scoreboard.
