...
1. This file.
2. `README.md`.
3. `docs/state-of-build-2026-05-11.md` — **most recent close-out snapshot.** Read this BEFORE the build plan to know what shipped, what's open in the backlog, and where to start. Replaced each session.
...
**Phase 16.0a (daemon infrastructure skeleton) shipped 2026-05-11** — channel types (`InboundMessage`, `ChannelAdapter`, `SecretTarget`, `DeliveryResult`) in `src/channels/types.ts`; `buildSessionKey()` in `src/channels/sessionKey.ts`; `send()` with local outbox in `src/channels/delivery.ts`; LRU `SessionCache` in `src/daemon/sessionCache.ts`; `ApprovalQueue` with TTL expiry in `src/daemon/approvalQueue.ts`; `DaemonEvent` 7-variant union + `DaemonEventMap` in `src/daemon/types.ts`; typed `DaemonEventBus` over Node `EventEmitter` in `src/daemon/eventBus.ts`; `startDaemon()` runner (PID lock via Phase 10.7 `tryAcquireLock`, idempotent `shutdown()` with try/finally lock release) + `harness daemon` CLI command with SIGTERM/SIGINT handling. Unit suite at close: **1805/1805**. Semantic suite unchanged at 58/58 — Phase 16.0a adds no agent-facing tools, slash commands, or context surfaces. Phase 16.0b (Ink TUI) is the logical next step.

**Phase 16.0b (Ink TUI + task event bus subscription) shipped 2026-05-11** — Ink 5 + React 18 deps; new `src/ui/ink/` directory with App + Transcript + Prompt + StatusLine components, pure UiState reducer, `useBusSubscription` + `useAgentTurn` hooks; TaskManager now emits `task_update` to the daemon bus via a safeEmit wrapper that isolates listener throws (Task 2 + safeEmit fix); `src/cli/missionRun.ts` extracted from `terminalRepl.ts` as the non-interactive scheduled-mission wake runner; bare `sov` and `harness` now open the Ink TUI as the default action; `sov chat` command removed; `sov mission run --state-dir <path>` subcommand added for the launchd path; `src/ui/terminalRepl.ts` and 11 helper UI modules deleted. Final-review follow-ups (post-cascade): profile-name bug fixed in `startInkTUI` (the status line was rendering the home path instead of the active profile name); concurrency guard added to `<Prompt>` so Enter is suppressed while the agent turn is in flight; ~8,000 LoC of dead modules orphaned by the `terminalRepl.ts` deletion purged in a single commit (17 production modules + 21 test files — see commit message for the audit). Suite: **1443/1443 unit** (the drop from 1700 is fully accounted for by orphan-test deletions alongside their source modules; no live tests regressed). Semantic unchanged at 58/58 — Phase 16.0b ships no new agent-facing tools, slash commands, permission rules, or context surfaces.

**Deferred to Phase 16.0c (P0 headline: slash command dispatch):** The Ink TUI ships without `/help`, `/cost`, `/tasks`, `/compact`, etc. — the slash command registry (`src/commands/registry.ts` and friends) was orphaned by the `terminalRepl.ts` deletion and is now removed from the codebase. Phase 16.0c rebuilds dispatch on top of the Ink TUI: parsing `/`-prefixed input in `useAgentTurn`, routing to a registry that's reconstructed inside the new Ink architecture, rendering results in the transcript. **Also deferred:** daemon-level compression threshold (build item 5); TaskManager construction lift into `startInkTUI` (subscription side is wired — `task_create` is exposed but throws "no task manager" if invoked); full agent-loop knobs at the CLI (`--provider`, `--model`, `--max-tokens`, `--permission-mode`, `--resume`, `--db`, `--no-cache`, `--no-preflight`, `--transcript`, `-v`, `--legacy-input`, `--capture-fixture`, `--replay-fixture`, `--agent` — currently only `-b/--bundle` forwards; `--state-dir` remains valid on `sov mission run`); eval-runner re-wire (`sov eval run` broken — spawned deprecated `sov chat`; NOTE comment in `src/main.ts:290`); `daemon_stopping`-after-unmount timing; `process.exit(0)` memory-flush leak on Ctrl-C in the Prompt component.

**Next high-leverage targets** per the build plan: Phase 16.0c (P0: slash command dispatch on top of the Ink TUI; P1: daemon compression threshold, TaskManager construction lift, agent-loop CLI knob forwarding, eval-runner re-wire, daemon_stopping timing, Ctrl-C memory flush). Consult `~/code/sovereign-ai-docs/harness/docs/runtime/harness-build-plan.md` for the full Phase 16.0 spec. The P3+ backlog item (17 — see `docs/post-phase-13-4-backlog.md`) remains open. Do not start any phase unless explicitly requested. See `runtime-scaffold-plan.md` for the Phase-0 layout this repo was seeded against.
