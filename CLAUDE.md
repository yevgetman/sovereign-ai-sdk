...
3. **`docs/state/2026-05-22-phase-21-m1.md`** — most recent close-out snapshot (Phase 21 M1 shipped 2026-05-22 — **binary distribution: `sov` ships as installable compiled binaries for `darwin-arm64` / `darwin-x64` / `linux-x64` via public `github.com/yevgetman/sov-releases` with `curl | bash` installer.** First release `v0.2.0` live; `~/.sov/bin/sov` + `~/.sov/bin/sov-tui` + `~/.sov/bundle-default/` layout. Runtime changes: `shippedBundlePath()` + `findTuiBinary()` learn `process.execPath`-relative discovery; `sov upgrade` auto-detects binary mode and re-runs the public installer; `src/version.ts` switched to build-time JSON import (the runtime `readFileSync(package.json)` broke in `bun --compile` mode). ADRs P21-A + P21-B. Suite **1972/0/14** TS green; +14 from baseline; lint+typecheck clean. macOS smoke PASS; Linux-binary structural check PASS; Linux container smoke deferred (Docker daemon issue on dev host); darwin-x64 deferred to first Intel-Mac beta user.) Predecessor: `docs/state/2026-05-21-ux-fixes-r5.md` (ux-fixes rounds 3-5 close-out). Read this BEFORE the build plan. Replaced each session — find the latest via `ls docs/state/*.md | sort -r | head -1`. The M11 audit report at `docs/state/2026-05-17-m11-parity-audit.md` and the M10 audit at `docs/state/2026-05-16-tui-parity-audit.md` both remain canonical.
...
| File | What's in it |
|---|---|
| [`docs/state/2026-05-22-phase-21-m1.md`](docs/state/2026-05-22-phase-21-m1.md) | **Canonical current-state snapshot — Phase 21 M1 shipped 2026-05-22.** Binary distribution: per-platform Bun `--compile` + Go cross-compile + side-car `bundle-default/` + public `github.com/yevgetman/sov-releases` repo with `curl ... | bash` installer; first release `v0.2.0` live (darwin-arm64 31.9 MB / darwin-x64 34.8 MB / linux-x64 47.5 MB tarballs + SHA256SUMS). Runtime: `shippedBundlePath()` + `findTuiBinary()` learn `process.execPath`-relative discovery; `sov upgrade` auto-detects binary mode and re-runs the public installer; `src/version.ts` switched to build-time JSON import. ADRs P21-A + P21-B. TS suite **1972/0/14** (+14 from baseline). macOS smoke PASS; linux-x64 binary structural check PASS; linux container smoke deferred (Docker daemon issue on dev host — re-run separately); darwin-x64 deferred to first Intel-Mac beta user. |
