...
export function buildHookRunner(opts: BuildHookRunnerOpts): HookRunner {
  ...
  // De-dup the awaiting-consent notice per (event, command) for the lifetime of
  // this runner closure. The runner is built once per long-lived runtime, so a
  // `matcher: "*"` hook with no recorded consent would otherwise emit one
  // identical line per tool invocation per turn — thousands over a gateway
  // session. We log the actionable notice the FIRST time each hook is skipped
  ...
  return async function run<N extends HookEventName>(
    ...
    for (const { spec } of matching) {
      ...
      if (decision === 'skip') {
        ...
        // dead with no signal — this is that signal. De-duped per (event,
        // command) so a wildcard hook doesn't flood stderr (one line per call).
        const skipKey = `${event} ${spec.command}`;
        if (!warnedSkips.has(skipKey)) {
          warnedSkips.add(skipKey);
          log(
