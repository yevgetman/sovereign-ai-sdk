...
export type ResolveProviderOpts = {
  ...
  /** Pre-loaded settings (read-only). A partial literal (e.g. `{}`) is valid
   *  because every field on `Settings` is optional; resolveProvider only reads
   *  optional fields off it. */
  settings?: Settings;
